...
#define PEONY_SENDTO_LAST_MEDIUM	"last-medium"
#define PEONY_SENDTO_LAST_COMPRESS	"last-compress"
#define PEONY_SENDTO_STATUS_LABEL_TIMEOUT_SECONDS 10

#define UNINSTALLED_PLUGINDIR "plugins/removable-devices"
#define UNINSTALLED_SOURCE "peony-sendto-command.c"
...
pack_files (NS_ui *ui)
{
	...
	tmp_dir = g_strdup_printf ("%s/peony-sendto-%s", 
				   g_get_tmp_dir(), g_get_user_name());	
	g_mkdir (tmp_dir, 0700);
	tmp_work_dir = g_strdup_printf ("%s/peony-sendto-%s/%li",
					g_get_tmp_dir(), g_get_user_name(),
					time(NULL));
...
send_button_cb (GtkWidget *widget, NS_ui *ui)
{
	...
	if (p->info->validate_destination != NULL) {
		...
		if (p->info->validate_destination (p, w, &error) == FALSE) {
			...
			g_free (message);
			ui->status_timeoutid = g_timeout_add_seconds (PEONY_SENDTO_STATUS_LABEL_TIMEOUT_SECONDS,
								      status_label_clear,
								      ui);
...
option_changed (GtkComboBox *cb, NS_ui *ui)
{
	...
	p = (NstPlugin *) g_list_nth_data (plugin_list, option);
	supports_dirs = (p->info->capabilities & PEONY_CAPS_SEND_DIRECTORIES);
...
set_model_for_options_combobox (NS_ui *ui)
{
	...
	for (aux = plugin_list; aux; aux = aux->next) {
		...
		if (last_used != NULL && !strcmp(last_used, p->info->id)) {
			option = i;
			last_used_support_dirs = (p->info->capabilities & PEONY_CAPS_SEND_DIRECTORIES);
		}
		i++;
	}
	g_free(last_used);
...
peony_sendto_create_ui (void)
{
	...
	app = gtk_builder_new ();
	if (!gtk_builder_add_from_file (app, UIDIR "/" "peony-sendto.ui", &error))	{
	...
	ui->dialog = GTK_WIDGET (gtk_builder_get_object (app, "peony_sendto_dialog"));
...
static void
peony_sendto_plugin_dir_process (const char *plugindir)
{
	GDir *dir;
	const char *item;
	NstPlugin *p = NULL;
	...
	if (g_file_test (UNINSTALLED_PLUGINDIR, G_FILE_TEST_IS_DIR) != FALSE) {
		...
		while ((item = g_dir_read_name(dir))) {
			...
			if (g_file_test (plugindir, G_FILE_TEST_IS_DIR) != FALSE)
				peony_sendto_plugin_dir_process (plugindir);
			g_free (plugindir);
		}
		g_dir_close (dir);
	}
...
int main (int argc, char **argv)
{
	...
	settings = g_settings_new ("org.mate.Peony.Sendto");
	peony_sendto_init ();
	if (peony_sendto_plugin_init () == FALSE) {
