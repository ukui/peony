...
/*
   peony-mime-application-chooser.c: an mime-application chooser
...
#include <config.h>
#include "peony-mime-application-chooser.h"
...
struct _PeonyMimeApplicationChooserDetails
{
    char *uri;
...
G_DEFINE_TYPE (PeonyMimeApplicationChooser, peony_mime_application_chooser, GTK_TYPE_BOX);
#else
G_DEFINE_TYPE (PeonyMimeApplicationChooser, peony_mime_application_chooser, GTK_TYPE_VBOX);
#endif

static void refresh_model             (PeonyMimeApplicationChooser *chooser);
static void refresh_model_soon        (PeonyMimeApplicationChooser *chooser);
static void mime_type_data_changed_cb (GObject                        *signaller,
                                       gpointer                        user_data);

static void
peony_mime_application_chooser_finalize (GObject *object)
{
    PeonyMimeApplicationChooser *chooser;

    chooser = PEONY_MIME_APPLICATION_CHOOSER (object);
    ...
    g_signal_handlers_disconnect_by_func (peony_signaller_get_current (),
    ...
    G_OBJECT_CLASS (peony_mime_application_chooser_parent_class)->finalize (object);
}

static void
peony_mime_application_chooser_class_init (PeonyMimeApplicationChooserClass *class)
{
    GObjectClass *gobject_class;

    gobject_class = G_OBJECT_CLASS (class);
    gobject_class->finalize = peony_mime_application_chooser_finalize;
}
                    ...
                    gpointer user_data)
{
    ...
    chooser = PEONY_MIME_APPLICATION_CHOOSER (user_data);
...
static GAppInfo *
get_selected_application (PeonyMimeApplicationChooser *chooser)
{
    GtkTreeIter iter;
...
static GtkWidget *
create_tree_view (PeonyMimeApplicationChooser *chooser)
{
    GtkWidget *treeview;
    GtkListStore *store;
                ...
                gpointer user_data)
{
    ...
    {
        dialog = peony_add_application_dialog_new_for_multiple_files (chooser->details->extension,
                 chooser->details->orig_mime_type);
    }
    else
    {
        dialog = peony_add_application_dialog_new (chooser->details->uri,
                 chooser->details->orig_mime_type);
    }
...
static void
peony_mime_application_chooser_init (PeonyMimeApplicationChooser *chooser)
{
    GtkWidget *box;
    GtkWidget *scrolled;
    GtkWidget *button;

    chooser->details = g_new0 (PeonyMimeApplicationChooserDetails, 1);
...
{
    PeonyMimeApplicationChooser *chooser = data;
...
static void
refresh_model_soon (PeonyMimeApplicationChooser *chooser)
{
...
static void
refresh_model (PeonyMimeApplicationChooser *chooser)
{
    GList *applications;
...
static void
set_extension_and_description (PeonyMimeApplicationChooser *chooser,
...
static gboolean
set_uri_and_type (PeonyMimeApplicationChooser *chooser,
                  const char *uri,
                  const char *mime_type)
{
    char *label;
    ...
    set_extension_and_description (PEONY_MIME_APPLICATION_CHOOSER (chooser),
                                   extension, mime_type);
    g_free (extension);
...
static char *
get_extension_from_file (PeonyFile *nfile)
{
    char *name;
    char *extension;

    name = peony_file_get_name (nfile);
    extension = get_extension (name);
...
static gboolean
set_uri_and_type_for_multiple_files (PeonyMimeApplicationChooser *chooser,
                                     GList *uris,
                                     const char *mime_type)
{
    char *label;
    ...
    chooser->details->orig_mime_type = g_strdup (mime_type);
    same_extension = TRUE;
    first_extension = get_extension_from_file (PEONY_FILE (uris->data));
    iter = uris->next;
    ...
    while (iter != NULL)
    {
        ...
        extension_current = get_extension_from_file (PEONY_FILE (iter->data));
...
GtkWidget *
peony_mime_application_chooser_new (const char *uri,
                                   const char *mime_type)
{
    GtkWidget *chooser;

    chooser = gtk_widget_new (PEONY_TYPE_MIME_APPLICATION_CHOOSER, NULL);

    set_uri_and_type (PEONY_MIME_APPLICATION_CHOOSER (chooser), uri, mime_type);

    return chooser;
}

GtkWidget *
peony_mime_application_chooser_new_for_multiple_files (GList *uris,
        const char *mime_type)
{
    GtkWidget *chooser;
    ...
    set_uri_and_type_for_multiple_files (PEONY_MIME_APPLICATION_CHOOSER (chooser),
                                         uris, mime_type);
