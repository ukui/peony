...
static PangoLayout *get_label_layout                 (PangoLayout               **layout,
    						      ...
    						      const char                *text);
#if !GTK_CHECK_VERSION(3,0,0)
static void     draw_label_layout                    (PeonyIconCanvasItem        *item,
    						      GdkDrawable               *drawable,
    						      PangoLayout               *layout,
    						      gboolean                  highlight,
...
#else
GdkPixmap *
peony_icon_canvas_item_get_image (PeonyIconCanvasItem *item,
                                 GdkBitmap **mask,
...
static void
draw_frame (PeonyIconCanvasItem *item,
            GdkDrawable *drawable,
            guint color,
            gboolean create_mask,
            int x,
                 ...
                 EelIRect icon_rect)
{
    ...
    if (have_editable)
    {
        ...
        peony_icon_container_get_label_color
             (PEONY_ICON_CONTAINER (canvas_item->canvas),
...
real_map_pixbuf (PeonyIconCanvasItem *icon_item)
{
            ...
            || icon_item->details->is_highlighted_for_drop)
    {
        ...
        color =  gtk_widget_has_focus (GTK_WIDGET (canvas)) ? PEONY_ICON_CONTAINER (canvas)->details->highlight_color_rgba : PEONY_ICON_CONTAINER (canvas)->details->active_color_rgba;
                            ...
                            cairo_region_t *region)
{
    PeonyIconContainer *container;
#else
peony_icon_canvas_item_draw (EelCanvasItem *item, GdkDrawable *drawable,
...
#if !GTK_CHECK_VERSION(3,0,0)
static void
draw_label_layout (PeonyIconCanvasItem *item,
                   GdkDrawable *drawable,
                   PangoLayout *layout,
                   gboolean highlight,
                   GdkColor *label_color,
                   ...
                   int y)
{
    ...
    if (!highlight && (PEONY_ICON_CONTAINER (EEL_CANVAS_ITEM (item)->canvas)->details->use_drop_shadows))
    {
        /* draw a drop shadow */
