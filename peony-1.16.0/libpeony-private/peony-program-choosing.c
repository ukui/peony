...
/* peony-program-choosing.c - functions for selecting and activating
                                  ...
                                  GtkWindow *parent_window)
{
    ...
    if (peony_mime_has_any_applications_for_file (file))
    {
        if (application != NULL)
        {
        ...
        if (response == GTK_RESPONSE_YES)
        {
            ...
            peony_choose_application_for_file
            (file,
             parent_window,
 ...
 */
void
peony_launch_application (GAppInfo *application,
                         GList *files,
                         GtkWindow *parent_window)
{
    GList *uris, *l;
    ...
    {
        uris = g_list_prepend (uris, peony_file_get_activation_uri (l->data));
    }
    uris = g_list_reverse (uris);
    peony_launch_application_by_uri (application, uris,
                                    parent_window);
...
void
peony_launch_application_by_uri (GAppInfo *application,
                                GList *uris,
                                GtkWindow *parent_window)
{
    char            *uri, *uri_scheme;
    ...
    file = peony_file_get_by_uri (uris->data);
    icon = peony_file_get_icon (file, 48, 0);
    peony_file_unref (file);
    if (icon)
    {
        gdk_app_launch_context_set_icon_name (launch_context,
                                              peony_icon_info_get_used_name (icon));
        g_object_unref (icon);
    }
    ...
    if (!result)
    {
        ...
        {
#ifdef NEW_MIME_COMPLETE
            peony_program_chooser_show_invalid_message
            (UKUI_VFS_MIME_ACTION_TYPE_APPLICATION, file, parent_window);
#else
    ...
    else
    {
        for (l = uris; l != NULL; l = l->next)
        {
            file = peony_file_get_by_uri (l->data);
            peony_recent_add_file (file, application);
            peony_file_unref (file);
        }
    }
 ...
 */
void
peony_launch_application_from_command (GdkScreen  *screen,
                                      const char *name,
                                      ...
                                      ...)
{
    ...
    if (use_terminal)
    {
        eel_ukui_open_terminal_on_screen (full_command, screen);
    }
    else
    {
#if GTK_CHECK_VERSION (3, 0, 0)
 ...
 */
void
peony_launch_application_from_command_array (GdkScreen  *screen,
        const char *name,
        const char *command_string,
        gboolean use_terminal,
...
void
peony_launch_desktop_file (GdkScreen   *screen,
