...
/* peony-tree-view-drag-dest.c: Handles drag and drop for treeviews which
 *                                 contain a hierarchy of files
 */

#include <config.h>
#include "peony-tree-view-drag-dest.h"
...
struct _PeonyTreeViewDragDestDetails
{
...
static void peony_tree_view_drag_dest_init (PeonyTreeViewDragDest      *dest);
static void peony_tree_view_drag_dest_class_init    (PeonyTreeViewDragDestClass *class);

static guint signals[LAST_SIGNAL] = { 0 };

G_DEFINE_TYPE (PeonyTreeViewDragDest, peony_tree_view_drag_dest,
               G_TYPE_OBJECT);
#define parent_class peony_tree_view_drag_dest_parent_class
...
static void
remove_scroll_timeout (PeonyTreeViewDragDest *dest)
{
...
static void
remove_expand_timeout (PeonyTreeViewDragDest *dest)
{
...
static void
set_widget_highlight (PeonyTreeViewDragDest *dest, gboolean highlight)
{
...
static void
set_drag_dest_row (PeonyTreeViewDragDest *dest,
                   GtkTreePath *path)
{
    if (path)
    {
...
static void
clear_drag_dest_row (PeonyTreeViewDragDest *dest)
{
...
static gboolean
get_drag_data (PeonyTreeViewDragDest *dest,
               ...
               guint32 time)
{
    ...
    if (target == gdk_atom_intern (PEONY_ICON_DND_XDNDDIRECTSAVE_TYPE, FALSE) &&
            !dest->details->drop_occurred)
    {
        dest->details->drag_type = PEONY_ICON_DND_XDNDDIRECTSAVE;
...
static void
free_drag_data (PeonyTreeViewDragDest *dest)
{
...
static char *
get_root_uri (PeonyTreeViewDragDest *dest)
{
    char *uri;
...
static PeonyFile *
file_for_path (PeonyTreeViewDragDest *dest, GtkTreePath *path)
{
    PeonyFile *file;
    char *uri;
...
static GtkTreePath *
get_drop_path (PeonyTreeViewDragDest *dest,
               GtkTreePath *path)
{
    PeonyFile *file;
    GtkTreePath *ret;
    ...
    while (file == NULL /* dummy row */ ||
            !peony_drag_can_accept_info (file,
...
static char *
get_drop_target_uri_for_path (PeonyTreeViewDragDest *dest,
                              GtkTreePath *path)
{
    PeonyFile *file;
    char *target;
    ...
    target = peony_file_get_drop_target_uri (file);
    peony_file_unref (file);
...
static guint
get_drop_action (PeonyTreeViewDragDest *dest,
                 GdkDragContext *context,
                 GtkTreePath *path)
{
    char *drop_target;
    ...
    if (!dest->details->have_drag_data ||
            (dest->details->drag_type == PEONY_ICON_DND_UKUI_ICON_LIST &&
             dest->details->drag_list == NULL))
    {
        return 0;
    }
    ...
    case PEONY_ICON_DND_UKUI_ICON_LIST :
        ...
        peony_drag_default_drop_action_for_icons
        (context,
         drop_target,
         dest->details->drag_list,
         &action);
                      ...
                      gpointer data)
{
    ...
    dest = PEONY_TREE_VIEW_DRAG_DEST (data);
...
static char *
get_drop_target_uri_at_pos (PeonyTreeViewDragDest *dest, int x, int y)
{
    char *drop_target;
    GtkTreePath *path;
    GtkTreePath *drop_path;
...
static void
receive_uris (PeonyTreeViewDragDest *dest,
              GdkDragContext *context,
              GList *source_uris,
              int x, int y)
{
    ...
    {
        if (peony_drag_selection_includes_special_link (dest->details->drag_list))
        {
        ...
        real_action = peony_drag_drop_action_ask
                      (GTK_WIDGET (dest->details->tree_view), action);
    }

    /* We only want to copy external uris */
    if (dest->details->drag_type == PEONY_ICON_DND_URI_LIST)
    {
    ...
    if (real_action > 0)
    {
        if (!peony_drag_uris_local (drop_target, source_uris)
...
static void
receive_dropped_icons (PeonyTreeViewDragDest *dest,
                       GdkDragContext *context,
                       int x, int y)
{
    GList *source_uris;
    ...
    for (l = dest->details->drag_list; l != NULL; l = l->next)
    {
        source_uris = g_list_prepend (source_uris,
                                      ((PeonyDragSelectionItem *)l->data)->uri);
    }
...
static void
receive_dropped_uri_list (PeonyTreeViewDragDest *dest,
                          GdkDragContext *context,
                          int x, int y)
{
    char *drop_target;
...
static void
receive_dropped_text (PeonyTreeViewDragDest *dest,
                      GdkDragContext *context,
                      int x, int y)
{
    char *drop_target;
...
static void
receive_dropped_raw (PeonyTreeViewDragDest *dest,
...
static void
receive_dropped_netscape_url (PeonyTreeViewDragDest *dest,
                              GdkDragContext *context,
                              int x, int y)
{
    char *drop_target;
...
static void
receive_dropped_keyword (PeonyTreeViewDragDest *dest,
                         GdkDragContext *context,
                         int x, int y)
{
    char *drop_target_uri;
    PeonyFile *drop_target_file;
    ...
    drop_target_file = peony_file_get_by_uri (drop_target_uri);

    if (drop_target_file != NULL)
    {
        peony_drag_file_receive_dropped_keyword (drop_target_file,
                                                (char *) gtk_selection_data_get_data (dest->details->drag_data));
        peony_file_unref (drop_target_file);
    }
...
static gboolean
receive_xds (PeonyTreeViewDragDest *dest,
             GtkWidget *widget,
             guint32 time,
             GdkDragContext *context,
             int x, int y)
                             ...
                             gpointer data)
{
    ...
    if (!dest->details->have_drag_data)
    {
        ...
        {
            dest->details->drag_list =
                peony_drag_build_selection_list (selection_data);
        }
    }
...
static gboolean
set_direct_save_uri (PeonyTreeViewDragDest *dest,
                     GdkDragContext *context,
                     int x, int y)
{
    GFile *base, *child;
                       ...
                       GObject *object)
{
    ...
    dest = PEONY_TREE_VIEW_DRAG_DEST (user_data);
...
static void
peony_tree_view_drag_dest_dispose (GObject *object)
{
    PeonyTreeViewDragDest *dest;

    dest = PEONY_TREE_VIEW_DRAG_DEST (object);
...
static void
peony_tree_view_drag_dest_finalize (GObject *object)
{
    PeonyTreeViewDragDest *dest;
...
static void
peony_tree_view_drag_dest_init (PeonyTreeViewDragDest *dest)
{
    dest->details = g_new0 (PeonyTreeViewDragDestDetails, 1);
}

static void
peony_tree_view_drag_dest_class_init (PeonyTreeViewDragDestClass *class)
{
    GObjectClass *gobject_class;

    gobject_class = G_OBJECT_CLASS (class);

    gobject_class->dispose = peony_tree_view_drag_dest_dispose;
    gobject_class->finalize = peony_tree_view_drag_dest_finalize;

    signals[GET_ROOT_URI] =
        g_signal_new ("get_root_uri",
                      G_TYPE_FROM_CLASS (class),
                      G_SIGNAL_RUN_LAST,
                      G_STRUCT_OFFSET (PeonyTreeViewDragDestClass,
                                       get_root_uri),
                      NULL, NULL,
    ...
    signals[MOVE_COPY_ITEMS] =
        g_signal_new ("move_copy_items",
                      ...
                      peony_marshal_VOID__POINTER_STRING_ENUM_INT_INT,
                      G_TYPE_NONE, 5,
                      G_TYPE_POINTER,
                      G_TYPE_STRING,
...
peony_tree_view_drag_dest_new (GtkTreeView *tree_view)
{
    PeonyTreeViewDragDest *dest;
    GtkTargetList *targets;

    dest = g_object_new (PEONY_TYPE_TREE_VIEW_DRAG_DEST, NULL);
