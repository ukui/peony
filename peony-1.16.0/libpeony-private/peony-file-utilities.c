...
/* peony-file-utilities.c - implementation of file manipulation routines.
...
#define DEFAULT_PEONY_DIRECTORY_MODE (0755)
...
char *
peony_compute_title_for_location (GFile *location)
{
    PeonyFile *file;
    char *title;
    ...
    if (location)
    {
        ...
        title = peony_file_get_description (file);
        if (title == NULL)
        {
            title = peony_file_get_display_name (file);
        }
...
/**
 * peony_get_user_directory:
 *
 * Get the path for the directory containing peony settings.
 *
 * Return value: the directory path.
 **/
char* peony_get_user_directory(void)
{
	/* FIXME bugzilla.gnome.org 41286:
	 * How should we handle the case where this mkdir fails?
	 * Note that peony_application_startup will refuse to launch if this
	 * directory doesn't get created, so that case is OK. But the directory
	 * could be deleted after Peony was launched, and perhaps
	 ...
	 */
	char* user_directory = g_build_filename(g_get_user_config_dir(), "peony", NULL);
	/* Se necesita que esta dirección sea una carpeta, con los permisos
	 * DEFAULT_PEONY_DIRECTORY_MODE. Pero si es un archivo, el programa intentará
	 * eliminar el archivo silenciosamente. */
	if (g_file_test(user_directory, G_FILE_TEST_IS_DIR) == FALSE ||
		g_access(user_directory, DEFAULT_PEONY_DIRECTORY_MODE) == -1)
	{
		...
		if (g_file_test(user_directory, G_FILE_TEST_IS_SYMLINK) == TRUE)
		{
			...
			if (link)
			{
				...
				if (g_file_test(link, G_FILE_TEST_IS_DIR) != TRUE ||
					g_chmod(link, DEFAULT_PEONY_DIRECTORY_MODE) != 0)
				{
					/* podemos borrar el enlace y crear la carpeta */
					g_unlink(user_directory);
					g_mkdir(user_directory, DEFAULT_PEONY_DIRECTORY_MODE);
				}
		...
		{
			g_chmod(user_directory, DEFAULT_PEONY_DIRECTORY_MODE);
		}
		...
		else
		{
			...
			g_mkdir_with_parents(user_directory, DEFAULT_PEONY_DIRECTORY_MODE);
		}

		/* Faltan permisos */
		if (g_chmod(user_directory, DEFAULT_PEONY_DIRECTORY_MODE) != 0)
		{
			GtkWidget* dialog = gtk_message_dialog_new(
				...
				GTK_MESSAGE_ERROR,
				GTK_BUTTONS_CLOSE,
				"The path for the directory containing peony settings need read and write permissions: %s",
				user_directory);
...
/**
 * peony_get_accel_map_file:
 *
 * Get the path for the filename containing peony accelerator map.
 * The filename need not exist.
 *
 * Return value: the filename path, or NULL if the home directory could not be found
 **/
char* peony_get_accel_map_file(void)
{
	const gchar* override;

	override = g_getenv ("UKUI22_USER_DIR");

	if (override)
	{
		return g_build_filename(override, "accels", "peony", NULL);
	}
	else
	{
		return g_build_filename(g_get_user_config_dir(), "peony", "accels", NULL);
	}
}
...
static void
xdg_dir_changed (PeonyFile *file,
                 XdgDirEntry *dir)
{
...
free_xdg_dir_cache (void)
{
    ...
    if (cached_xdg_dirs != NULL)
    {
        for (i = 0; cached_xdg_dirs[i].type != NULL; i++)
        {
            ...
            {
                peony_file_monitor_remove (cached_xdg_dirs[i].file,
                                          &cached_xdg_dirs[i]);
                g_signal_handlers_disconnect_by_func (cached_xdg_dirs[i].file,
                                                      G_CALLBACK (xdg_dir_changed),
                                                      &cached_xdg_dirs[i]);
                peony_file_unref (cached_xdg_dirs[i].file);
            }
...
update_xdg_dir_cache (void)
{
    ...
    for (i = 0 ; cached_xdg_dirs[i].type != NULL; i++)
    {
        ...
        if (strcmp (cached_xdg_dirs[i].path, g_get_home_dir ()) != 0)
        {
            ...
            cached_xdg_dirs[i].file = peony_file_get_by_uri (uri);
            peony_file_monitor_add (cached_xdg_dirs[i].file,
                                   &cached_xdg_dirs[i],
...
char *
peony_get_xdg_dir (const char *type)
{
    int i;
...
{
    if (g_settings_get_boolean (peony_preferences, PEONY_PREFERENCES_DESKTOP_IS_HOME_DIR))
    {
        return g_strdup (g_get_home_dir());
    }
    else
    {
        return peony_get_xdg_dir ("DESKTOP");
    }
}

/**
 * peony_get_desktop_directory:
 *
 ...
 **/
char *
peony_get_desktop_directory (void)
{
...
GFile *
peony_get_desktop_location (void)
{
    char *desktop_directory;
    GFile *res;
...
/**
 * peony_get_desktop_directory_uri:
 *
 ...
 **/
char *
peony_get_desktop_directory_uri (void)
{
    char *desktop_path;
    char *desktop_uri;
...
char *
peony_get_home_directory_uri (void)
{
...
gboolean
peony_should_use_templates_directory (void)
{
    char *dir;
    gboolean res;

    dir = peony_get_xdg_dir ("TEMPLATES");
...
char *
peony_get_templates_directory (void)
{
    return peony_get_xdg_dir ("TEMPLATES");
}

void
peony_create_templates_directory (void)
{
    char *dir;

    dir = peony_get_templates_directory ();
    if (!g_file_test (dir, G_FILE_TEST_EXISTS))
    {
        g_mkdir (dir, DEFAULT_PEONY_DIRECTORY_MODE);
    }
    g_free (dir);
}

char *
peony_get_templates_directory_uri (void)
{
    char *directory, *uri;

    directory = peony_get_templates_directory ();
...
gboolean
peony_is_home_directory_file (GFile *dir,
                             const char *filename)
{
    char *dirname;
...
gboolean
peony_is_home_directory (GFile *dir)
{
...
gboolean
peony_is_root_directory (GFile *dir)
{
...
gboolean
peony_is_desktop_directory_file (GFile *dir,
                                const char *file)
{
...
gboolean
peony_is_desktop_directory (GFile *dir)
{
...
/**
 * peony_get_pixmap_directory
 *
 * Get the path for the directory containing Peony pixmaps.
 *
 * Return value: the directory path.
 **/
char *
peony_get_pixmap_directory (void)
{
    return g_strdup (DATADIR "/pixmaps/peony");
}
 ...
 */
char *
peony_pixmap_file (const char *partial_path)
{
    char *path;

    path = g_build_filename (DATADIR "/pixmaps/peony", partial_path, NULL);
    ...
    {
        char *tmp;
        tmp = peony_get_pixmap_directory ();
        g_debug ("Failed to locate \"%s\" in Peony pixmap path \"%s\". Incomplete installation?", partial_path, tmp);
        g_free (tmp);
    }
    g_free (path);
    return NULL;
...
char *
peony_get_data_file_path (const char *partial_path)
{
    char *path;
    char *user_directory;
    ...
    path = g_build_filename (PEONY_DATADIR, partial_path, NULL);
    if (g_file_test (path, G_FILE_TEST_EXISTS))
    {
        return path;
    }
...
char *
peony_ensure_unique_file_name (const char *directory_uri,
...
GFile *
peony_find_existing_uri_in_hierarchy (GFile *location)
{
    GFileInfo *info;
    GFile *tmp;
...
gboolean
peony_is_engrampa_installed (void)
{
 ...
 * (e.g. whenever Peony is doing file operations).
 *
 * Returns: an integer cookie, which must be passed to
 *    peony_uninhibit_power_manager() to resume
 *    normal power management.
 */
int
peony_inhibit_power_manager (const char *message)
{
...
/**
 * peony_uninhibit_power_manager:
 * @cookie: the cookie value returned by peony_inhibit_power_manager()
 *
 ...
 */
void
peony_uninhibit_power_manager (gint cookie)
{
    GDBusConnection *connection;
    GVariant *result;
    GError *error = NULL;
...
/* Returns TRUE if the file is in XDG_DATA_DIRS or
   in "~/.ukui2/". This is used for deciding
   if a desktop file is "trusted" based on the path */
gboolean
peony_is_in_system_dir (GFile *file)
{
    ...
    if (!res)
    {
        ...
        ukui2 = g_build_filename(g_get_home_dir(), ".ukui2", NULL);

        if (g_str_has_prefix (path, ukui2))
        {
            res = TRUE;
        }
...
GHashTable *
peony_trashed_files_get_original_directories (GList *files,
        GList **unhandled_files)
{
    GHashTable *directories;
    PeonyFile *file, *original_file, *original_dir;
    GList *l, *m;
    ...
    for (l = files; l != NULL; l = l->next)
    {
        ...
        original_file = peony_file_get_trash_original_file (file);

        original_dir = NULL;
        if (original_file != NULL)
        {
            original_dir = peony_file_get_parent (original_file);
        }
        ...
        if (original_dir != NULL)
        {
            if (directories == NULL)
            {
                directories = g_hash_table_new_full (g_direct_hash, g_direct_equal,
                                                     ...
                                                     (GDestroyNotify) peony_file_list_unref);
            }
            peony_file_ref (original_dir);
            ...
            m = g_list_append (m, peony_file_ref (file));
            g_hash_table_insert (directories, original_dir, m);
        }
        else if (unhandled_files != NULL)
        {
            *unhandled_files = g_list_append (*unhandled_files, peony_file_ref (file));
        }
...
locations_from_file_list (GList *file_list)
{
    ...
    {
        file = PEONY_FILE (l->data);
        ret = g_list_prepend (ret, peony_file_get_location (file));
    }
...
void
peony_restore_files_from_trash (GList *files,
                               GtkWindow *parent_window)
{
    PeonyFile *file, *original_dir;
    ...
    original_dirs_hash = peony_trashed_files_get_original_directories (files, &unhandled_files);
    ...
    if (original_dirs_hash != NULL)
    {
        ...
        {
            original_dir = PEONY_FILE (l->data);
            original_dir_location = peony_file_get_location (original_dir);

            files = g_hash_table_lookup (original_dirs_hash, original_dir);
            locations = locations_from_file_list (files);

            peony_file_operations_move
            (locations, NULL,
    ...
    peony_file_list_unref (unhandled_files);
}

#if !defined (PEONY_OMIT_SELF_CHECK)

void
peony_self_check_file_utilities (void)
{
}
