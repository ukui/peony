...
static int  peony_notebook_insert_page	 (GtkNotebook *notebook,
        ...
        int position);
static void peony_notebook_remove	 (GtkContainer *container,
...
G_DEFINE_TYPE (PeonyNotebook, peony_notebook, GTK_TYPE_NOTEBOOK);

static void
peony_notebook_class_init (PeonyNotebookClass *klass)
{
    ...
    signals[TAB_CLOSE_REQUEST] =
        g_signal_new ("tab-close-request",
                      ...
                      G_SIGNAL_RUN_LAST,
                      G_STRUCT_OFFSET (PeonyNotebookClass, tab_close_request),
                      NULL, NULL,
                      g_cclosure_marshal_VOID__OBJECT,
                      G_TYPE_NONE,
                      1,
                      PEONY_TYPE_WINDOW_SLOT);
}

static gint
find_tab_num_at_pos (PeonyNotebook *notebook, gint abs_x, gint abs_y)
{
...
static gboolean
button_press_cb (PeonyNotebook *notebook,
...
void
peony_notebook_sync_loading (PeonyNotebook *notebook,
                            PeonyWindowSlot *slot)
{
    GtkWidget *tab_label, *spinner, *icon;
    gboolean active;

    g_return_if_fail (PEONY_IS_NOTEBOOK (notebook));
...
void
peony_notebook_sync_tab_label (PeonyNotebook *notebook,
                              PeonyWindowSlot *slot)
{
                         ...
                         PeonyWindowSlot *slot)
{
    ...
    notebook = gtk_widget_get_ancestor (slot->content_box, PEONY_TYPE_NOTEBOOK);
    if (notebook != NULL)
    {
...
static GtkWidget *
build_tab_label (PeonyNotebook *nb, PeonyWindowSlot *slot)
{
    ...
    gtk_widget_set_name (close_button, "peony-tab-close-button");
...
static int
peony_notebook_insert_page (GtkNotebook *gnotebook,
                           GtkWidget *tab_widget,
                           GtkWidget *tab_label,
                           GtkWidget *menu_label,
                           int position)
{
    ...
    position = GTK_NOTEBOOK_CLASS (peony_notebook_parent_class)->insert_page (gnotebook,
               tab_widget,
               tab_label,
               menu_label,
               position);
...
int
peony_notebook_add_tab (PeonyNotebook *notebook,
                       PeonyWindowSlot *slot,
                       int position,
                       gboolean jump_to)
{
    ...
    g_return_val_if_fail (PEONY_IS_NOTEBOOK (notebook), -1);
    g_return_val_if_fail (PEONY_IS_WINDOW_SLOT (slot), -1);
...
static void
peony_notebook_remove (GtkContainer *container,
                      GtkWidget *tab_widget)
{
    GtkNotebook *gnotebook = GTK_NOTEBOOK (container);
    GTK_CONTAINER_CLASS (peony_notebook_parent_class)->remove (container, tab_widget);
...
void
peony_notebook_reorder_current_child_relative (PeonyNotebook *notebook,
        int offset)
{
    ...
    if (!peony_notebook_can_reorder_current_child_relative (notebook, offset))
    {
        return;
    }
...
void
peony_notebook_set_current_page_relative (PeonyNotebook *notebook,
        int offset)
{
    GtkNotebook *gnotebook;
    int page;
    ...
    if (!peony_notebook_can_set_current_page_relative (notebook, offset))
    {
        return;
    }
...
static gboolean
peony_notebook_is_valid_relative_position (PeonyNotebook *notebook,
        int offset)
{
    GtkNotebook *gnotebook;
    int page;
...
gboolean
peony_notebook_can_reorder_current_child_relative (PeonyNotebook *notebook,
        int offset)
{
    g_return_val_if_fail (PEONY_IS_NOTEBOOK (notebook), FALSE);

    return peony_notebook_is_valid_relative_position (notebook, offset);
}

gboolean
peony_notebook_can_set_current_page_relative (PeonyNotebook *notebook,
        int offset)
{
    g_return_val_if_fail (PEONY_IS_NOTEBOOK (notebook), FALSE);
