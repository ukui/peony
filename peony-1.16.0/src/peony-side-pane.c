...
static void peony_side_pane_class_init (PeonySidePaneClass *klass);
static void peony_side_pane_init       (GObject *object);
static void peony_side_pane_dispose    (GObject *object);
static void peony_side_pane_finalize   (GObject *object);
...
#if GTK_CHECK_VERSION (3, 0, 0)
EEL_CLASS_BOILERPLATE (PeonySidePane, peony_side_pane, GTK_TYPE_BOX)
#else
EEL_CLASS_BOILERPLATE (PeonySidePane, peony_side_pane, GTK_TYPE_VBOX)
#endif

static SidePanel *
panel_for_widget (PeonySidePane *side_pane, GtkWidget *widget)
{
    GList *l;
    SidePanel *panel;
...
switch_page_callback (GtkWidget *notebook,
                      ...
                      gpointer user_data)
{
    PeonySidePane *side_pane;
    SidePanel *panel;

    side_pane = PEONY_SIDE_PANE (user_data);
...
static void
select_panel (PeonySidePane *side_pane, SidePanel *panel)
{
    int page_num;
...
static void
peony_side_pane_class_init (PeonySidePaneClass *klass)
{
    ...
    gobject_class->finalize = peony_side_pane_finalize;
    gobject_class->dispose = peony_side_pane_dispose;

    signals[CLOSE_REQUESTED] = g_signal_new
                               ("close_requested",
                                G_TYPE_FROM_CLASS (klass),
                                G_SIGNAL_RUN_LAST,
                                G_STRUCT_OFFSET (PeonySidePaneClass,
                                        close_requested),
                                NULL, NULL,
    ...
    g_type_class_add_private (gobject_class, sizeof (PeonySidePaneDetails));
}
                      ...
                      GtkMenu *menu)
{
    ...
    side_pane = PEONY_SIDE_PANE (widget);

    side_pane->details->menu = NULL;
}

static void
peony_side_pane_init (GObject *object)
{
    PeonySidePane *side_pane;
    GtkWidget *hbox;
    ...
    side_pane = PEONY_SIDE_PANE (object);

    side_pane->details = G_TYPE_INSTANCE_GET_PRIVATE (object, PEONY_TYPE_SIDE_PANE, PeonySidePaneDetails);

#if GTK_CHECK_VERSION(3, 0, 0)
    GtkStyleContext *context;

    context = gtk_widget_get_style_context (GTK_WIDGET (object));
    gtk_style_context_add_class (context, "peony-side-pane");
#endif
...
static void
peony_side_pane_dispose (GObject *object)
{
    PeonySidePane *side_pane;
...
static void
peony_side_pane_finalize (GObject *object)
{
    PeonySidePane *side_pane;
    GList *l;
...
PeonySidePane *
peony_side_pane_new (void)
{
    return PEONY_SIDE_PANE (gtk_widget_new (peony_side_pane_get_type (), NULL));
}

void
peony_side_pane_add_panel (PeonySidePane *side_pane,
                          GtkWidget *widget,
                          const char *title,
                          const char *tooltip)
{
    ...
    g_return_if_fail (PEONY_IS_SIDE_PANE (side_pane));
...
void
peony_side_pane_remove_panel (PeonySidePane *side_pane,
                             GtkWidget *widget)
{
    SidePanel *panel;
    int page_num;
...
void
peony_side_pane_show_panel (PeonySidePane *side_pane,
                           GtkWidget        *widget)
{
    SidePanel *panel;
    int page_num;
                           ...
                           gpointer user_data)
{
    ...
    peony_side_pane_show_panel (side_pane, page);
}

static GtkWidget *
create_shortcut (PeonySidePane *side_pane,
                 SidePanel *panel,
                 GdkPixbuf *pixbuf)
{
    GtkWidget *button;
...
void
peony_side_pane_set_panel_image (PeonySidePane *side_pane,
                                GtkWidget *widget,
                                GdkPixbuf *pixbuf)
{
    SidePanel *panel;
...
GtkWidget *
peony_side_pane_get_current_panel (PeonySidePane *side_pane)
{
    int index;
...
GtkWidget *
peony_side_pane_get_title (PeonySidePane *side_pane)
{
