...
#define PEONY_X_CONTENT_BAR_GET_PRIVATE(o) (G_TYPE_INSTANCE_GET_PRIVATE ((o), PEONY_TYPE_X_CONTENT_BAR, PeonyXContentBarPrivate))

struct PeonyXContentBarPrivate
{
    GtkWidget *label;
    GtkWidget *button;
...
G_DEFINE_TYPE (PeonyXContentBar, peony_x_content_bar, GTK_TYPE_BOX)
#else
G_DEFINE_TYPE (PeonyXContentBar, peony_x_content_bar, GTK_TYPE_HBOX)
#endif

void
peony_x_content_bar_set_x_content_type (PeonyXContentBar *bar, const char *x_content_type)
{
    char *message;
    char *description;
    ...
    if (default_app != NULL)
    {
        ...
        if (icon != NULL)
        {
            ...
            icon_size = peony_get_icon_size_for_stock_size (GTK_ICON_SIZE_BUTTON);
...
const char *
peony_x_content_bar_get_x_content_type (PeonyXContentBar *bar)
{
    return bar->priv->x_content_type;
}

GMount *
peony_x_content_bar_get_mount (PeonyXContentBar *bar)
{
    return bar->priv->mount != NULL ? g_object_ref (bar->priv->mount) : NULL;
}

void
peony_x_content_bar_set_mount (PeonyXContentBar *bar, GMount *mount)
{
...
static void
peony_x_content_bar_set_property (GObject      *object,
                                 guint         prop_id,
                                 const GValue *value,
                                 GParamSpec   *pspec)
{
    PeonyXContentBar *bar;

    bar = PEONY_X_CONTENT_BAR (object);

    switch (prop_id)
    {
    case PROP_MOUNT:
        peony_x_content_bar_set_mount (bar, G_MOUNT (g_value_get_object (value)));
        break;
    case PROP_X_CONTENT_TYPE:
        peony_x_content_bar_set_x_content_type (bar, g_value_get_string (value));
        break;
    default:
...
static void
peony_x_content_bar_get_property (GObject    *object,
                                 guint       prop_id,
                                 GValue     *value,
                                 GParamSpec *pspec)
{
...
static void
peony_x_content_bar_finalize (GObject *object)
{
    PeonyXContentBar *bar = PEONY_X_CONTENT_BAR (object);

    g_free (bar->priv->x_content_type);
    if (bar->priv->mount != NULL)
        g_object_unref (bar->priv->mount);

    G_OBJECT_CLASS (peony_x_content_bar_parent_class)->finalize (object);
}

static void
peony_x_content_bar_class_init (PeonyXContentBarClass *klass)
{
    GObjectClass *object_class;

    object_class = G_OBJECT_CLASS (klass);
    object_class->get_property = peony_x_content_bar_get_property;
    object_class->set_property = peony_x_content_bar_set_property;
    object_class->finalize = peony_x_content_bar_finalize;

    g_type_class_add_private (klass, sizeof (PeonyXContentBarPrivate));
...
static void
button_clicked_callback (GtkWidget *button, PeonyXContentBar *bar)
{
    GAppInfo *default_app;
    ...
    if (default_app != NULL)
    {
        peony_autorun_launch_for_mount (bar->priv->mount, default_app);
        g_object_unref (default_app);
    }
}

static void
peony_x_content_bar_init (PeonyXContentBar *bar)
{
    GtkWidget *hbox;

    bar->priv = PEONY_X_CONTENT_BAR_GET_PRIVATE (bar);
...
GtkWidget *
peony_x_content_bar_new (GMount *mount,
                        const char *x_content_type)
{
    GObject *bar;

    bar = g_object_new (PEONY_TYPE_X_CONTENT_BAR,
                        "mount", mount,
