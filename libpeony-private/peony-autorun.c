...
static void peony_autorun_rebuild_combo_box (GtkWidget *combo_box);

void
peony_autorun_get_preferences (const char *x_content_type,
                              ...
                              gboolean *pref_open_folder)
{
    ...
    *pref_open_folder = FALSE;
    x_content_start_app = g_settings_get_strv (peony_media_preferences, PEONY_PREFERENCES_MEDIA_AUTORUN_X_CONTENT_START_APP);
...
void
peony_autorun_set_preferences (const char *x_content_type,
                              gboolean pref_start_app,
                              gboolean pref_ignore,
                              gboolean pref_open_folder)
{
    ...
    g_settings_set_strv (peony_media_preferences, PEONY_PREFERENCES_MEDIA_AUTORUN_X_CONTENT_START_APP, (const gchar * const*) x_content_start_app);
    ...
    g_settings_set_strv (peony_media_preferences, PEONY_PREFERENCES_MEDIA_AUTORUN_X_CONTENT_IGNORE, (const gchar * const*) x_content_ignore);
    ...
    g_settings_set_strv (peony_media_preferences, PEONY_PREFERENCES_MEDIA_AUTORUN_X_CONTENT_OPEN_FOLDER, (const gchar * const*) x_content_open_folder);
...
static void
peony_autorun_combobox_data_destroy (PeonyAutorunComboBoxData *data)
{
...
static void
other_application_selected (PeonyOpenWithDialog *dialog,
                            GAppInfo *app_info,
                            PeonyAutorunComboBoxData *data)
{
    ...
    {
        peony_autorun_set_preferences (data->x_content_type, TRUE, FALSE, FALSE);
    ...
    peony_autorun_rebuild_combo_box (data->combo_box);
}

static void
handle_dialog_closure (PeonyAutorunComboBoxData *data)
{
                   ...
                   PeonyAutorunComboBoxData *data)
{
    ...
    case AUTORUN_ASK:
        ...
        if (data->update_settings)
        {
            peony_autorun_set_preferences (x_content_type, FALSE, FALSE, FALSE);
        }
        break;
    case AUTORUN_IGNORE:
        if (data->changed_cb != NULL)
        ...
        if (data->update_settings)
        {
            peony_autorun_set_preferences (x_content_type, FALSE, TRUE, FALSE);
        }
        break;
    case AUTORUN_OPEN_FOLDER:
        ...
        if (data->update_settings)
        {
            peony_autorun_set_preferences (x_content_type, FALSE, FALSE, TRUE);
        }
        break;
    ...
    case AUTORUN_APP:
        ...
        if (data->update_settings)
        {
            peony_autorun_set_preferences (x_content_type, TRUE, FALSE, FALSE);
            g_app_info_set_as_default_for_type (app_info,
                                                x_content_type,
                                                NULL);
        }
    ...
    case AUTORUN_OTHER_APP:
    {
        ...
        dialog = peony_add_application_dialog_new (NULL, x_content_type);
...
static void
peony_autorun_rebuild_combo_box (GtkWidget *combo_box)
{
    PeonyAutorunComboBoxData *data;
    char *x_content_type;

    data = g_object_get_data (G_OBJECT (combo_box), "peony_autorun_combobox_data");
    if (data == NULL)
    {
        g_warning ("no 'peony_autorun_combobox_data' data!");
        return;
    }

    x_content_type = g_strdup (data->x_content_type);
    peony_autorun_prepare_combo_box (combo_box,
                                    x_content_type,
                                    data->include_ask,
...
void
peony_autorun_prepare_combo_box (GtkWidget *combo_box,
                                ...
                                gpointer user_data)
{
    ...
    peony_autorun_get_preferences (x_content_type, &pref_start_app, &pref_ignore, &pref_open_folder);
    ...
    if (num_apps == 0)
    {
        ...
        /* TODO: integrate with PackageKit-ukui to find applications */
    ...
    else
    {
        ...
        {
            data = g_new0 (PeonyAutorunComboBoxData, 1);
        }
    ...
    if (new_data)
    {
        g_object_set_data_full (G_OBJECT (combo_box),
                                "peony_autorun_combobox_data",
                                data,
                                (GDestroyNotify) peony_autorun_combobox_data_destroy);
    }
}
...
typedef struct
{
    ...
    PeonyAutorunOpenWindow open_window_func;
    gpointer user_data;
} AutorunDialogData;


void
peony_autorun_launch_for_mount (GMount *mount, GAppInfo *app_info)
{
    GFile *root;
    PeonyFile *file;
    GList *files;
    ...
    peony_launch_application (app_info,
                             files,
...
autorun_dialog_response (GtkDialog *dialog, gint response, AutorunDialogData *data)
{
    ...
    case AUTORUN_DIALOG_RESPONSE_EJECT:
        peony_file_operations_unmount_mount (GTK_WINDOW (dialog),
                                            data->mount,
                                            data->should_eject,
                                            FALSE);
        break;
    ...
    case GTK_RESPONSE_OK:
        ...
        if (data->remember)
        {
            ...
            peony_autorun_set_preferences (data->x_content_type, TRUE, data->selected_ignore, data->selected_open_folder);
        ...
        else
        {
            ...
            peony_autorun_set_preferences (data->x_content_type, FALSE, FALSE, FALSE);
        }

        if (!data->selected_ignore && !data->selected_open_folder && data->selected_app != NULL)
        {
            peony_autorun_launch_for_mount (data->mount, data->selected_app);
        }
...
static gboolean
do_autorun_for_content_type (GMount *mount, const char *x_content_type, PeonyAutorunOpenWindow open_window_func, gpointer user_data)
{
    ...
    if (!pref_ask && !pref_ignore && !pref_open_folder)
    {
        ...
        if (app_info != NULL)
        {
            peony_autorun_launch_for_mount (mount, app_info);
        }
        goto out;
    }
...
show_dialog:
    ...
    icon_size = peony_get_icon_size_for_stock_size (GTK_ICON_SIZE_DIALOG);
    icon_info = peony_icon_info_lookup (icon, icon_size);
    pixbuf = peony_icon_info_get_pixbuf_at_size (icon_info, icon_size);
    g_object_unref (icon_info);
    g_object_unref (icon);
    ...
    peony_autorun_prepare_combo_box (combo_box, x_content_type, FALSE, TRUE, FALSE, autorun_combo_changed, data);
    ...
    if (g_mount_can_eject (mount))
    {
        ...
        pixbuf = gtk_icon_theme_load_icon (gtk_icon_theme_get_default (),
                                           "media-eject",
                                           peony_get_icon_size_for_stock_size (GTK_ICON_SIZE_BUTTON),
                                           0,
                                           NULL);
                                       ...
                                       gpointer user_data)
{
    ...
    else
    {
        ...
        {
            if (g_settings_get_boolean (peony_media_preferences, PEONY_PREFERENCES_MEDIA_AUTOMOUNT_OPEN))
                open_folder = TRUE;
        }
    }
...
void
peony_autorun (GMount *mount, PeonyAutorunOpenWindow open_window_func, gpointer user_data)
{
    AutorunData *data;
...
void
peony_autorun_get_x_content_types_for_mount_async (GMount *mount,
        PeonyAutorunGetContent callback,
        ...
        gpointer user_data)
{
    ...
    cached = g_object_get_data (G_OBJECT (mount), "peony-content-type-cache");
    if (cached != NULL)
    {
        if (callback)
        {
...
char **
peony_autorun_get_cached_x_content_types_for_mount (GMount      *mount)
{
    char **cached;
...
remove_allow_volume (gpointer data)
{
    ...
    g_object_set_data (G_OBJECT (volume), "peony-allow-autorun", NULL);
    return FALSE;
}

void
peony_allow_autorun_for_volume (GVolume *volume)
{
    g_object_set_data (G_OBJECT (volume), "peony-allow-autorun", GINT_TO_POINTER (1));
}

#define INHIBIT_AUTORUN_SECONDS 10

void
peony_allow_autorun_for_volume_finish (GVolume *volume)
{
    if (g_object_get_data (G_OBJECT (volume), "peony-allow-autorun") != NULL)
    {
...
should_autorun_mount (GMount *mount)
{
    ...
    {
        if (g_object_get_data (G_OBJECT (enclosing_volume), "peony-allow-autorun") != NULL)
        {
            ignore_autorun = FALSE;
            g_object_set_data (G_OBJECT (enclosing_volume), "peony-allow-autorun", NULL);
        }
    }
