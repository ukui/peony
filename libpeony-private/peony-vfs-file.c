/* -*- Mode: C; indent-tabs-mode: t; c-basic-offset: 8; tab-width: 8 -*-
   ...
   peony-vfs-file.c: Subclass of PeonyFile to help implement the
...
static void peony_vfs_file_init       (gpointer   object,
                                      gpointer   klass);
static void peony_vfs_file_class_init (gpointer   klass);

EEL_CLASS_BOILERPLATE (PeonyVFSFile,
                       peony_vfs_file,
                       PEONY_TYPE_FILE)

static void
vfs_file_monitor_add (PeonyFile *file,
...
static void
vfs_file_monitor_remove (PeonyFile *file,
                         gconstpointer client)
{
...
static void
vfs_file_call_when_ready (PeonyFile *file,
...
static void
vfs_file_cancel_call_when_ready (PeonyFile *file,
                                 PeonyFileCallback callback,
                                 gpointer callback_data)
{
...
static gboolean
vfs_file_check_if_ready (PeonyFile *file,
                         PeonyFileAttributes file_attributes)
{
                                ...
                                gpointer callback_data)
{
    ...
    if (new_info != NULL)
    {
        if (peony_file_update_info (file, new_info))
        {
...
static void
vfs_file_set_metadata (PeonyFile           *file,
                       const char             *key,
                       const char             *value)
{
    GFileInfo *info;
...
static void
vfs_file_set_metadata_as_list (PeonyFile           *file,
                               const char             *key,
                               char                  **value)
{
    GFile *location;
...
vfs_file_get_deep_counts (PeonyFile *file,
                          ...
                          goffset *total_size)
{
    ...
    if (file->details->deep_counts_status != PEONY_REQUEST_NOT_STARTED)
    {
        if (directory_count != NULL)
        {
    ...
    type = peony_file_get_file_type (file);
    if (type == G_FILE_TYPE_UNKNOWN
            || type == G_FILE_TYPE_DIRECTORY)
    {
        return PEONY_REQUEST_NOT_STARTED;
    }
...
static gboolean
vfs_file_get_date (PeonyFile *file,
                   PeonyDateType date_type,
                   time_t *date)
{
    switch (date_type)
    {
    case PEONY_DATE_TYPE_CHANGED:
    ...
    case PEONY_DATE_TYPE_TRASHED:
        ...
        return TRUE;
    case PEONY_DATE_TYPE_PERMISSIONS_CHANGED:
        /* Before we have info on a file, the date is unknown. */
...
static char *
vfs_file_get_where_string (PeonyFile *file)
{
    return peony_file_get_parent_uri_for_display (file);
}
                         ...
                         gpointer callback_data)
{
    ...
    mounted_on = g_file_mount_mountable_finish (G_FILE (source_object),
                 res, &error);
    peony_file_operation_complete (op, mounted_on, error);
    if (mounted_on)
    {
                           ...
                           gpointer callback_data)
{
    ...
    peony_file_operation_complete (op, G_FILE (source_object), error);
    if (error)
    {
        g_error_free (error);
    }
...
static void
vfs_file_unmount (PeonyFile                   *file,
                  GMountOperation                *mount_op,
                  GCancellable                   *cancellable,
                  PeonyFileOperationCallback   callback,
                  gpointer                        callback_data)
...
static void
vfs_file_poll_for_media (PeonyFile *file)
{
    PeonyFileOperation *op;
    GFile *location;

    op = peony_file_operation_new (file, NULL, NULL);
...
static void
peony_vfs_file_init (gpointer object, gpointer klass)
{
}

static void
peony_vfs_file_class_init (gpointer klass)
{
