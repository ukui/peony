...
/* PeonyUndoStackManager - Manages undo of file operations (implementation)
 *
...
#include "peony-undostack-manager.h"
...
struct _PeonyUndoStackActionData
{
  /* Common stuff */
  PeonyUndoStackActionType type;
  gboolean isValid;
  ...
  PeonyUndoStackManager *manager;    /* Pointer to the manager */
...
struct _PeonyUndoStackManagerPrivate
{
  /* Private fields */
  GQueue *stack;
  guint undo_levels;
...
#define PEONY_UNDOSTACK_MANAGER_GET_PRIVATE(o)  \
   (G_TYPE_INSTANCE_GET_PRIVATE ((o), TYPE_PEONY_UNDOSTACK_MANAGER, PeonyUndoStackManagerPrivate))
...
static void peony_undostack_manager_set_property (GObject * object,
    guint prop_id, const GValue * value, GParamSpec * pspec);

static void peony_undostack_manager_get_property (GObject * object,
    guint prop_id, GValue * value, GParamSpec * pspec);

/* *****************************************************************
 Destructors prototypes
 ***************************************************************** */
static void peony_undostack_manager_finalize (GObject * object);

static void peony_undostack_manager_dispose (GObject * object);

/* *****************************************************************
 Type definition
 ***************************************************************** */
G_DEFINE_TYPE (PeonyUndoStackManager, peony_undostack_manager,
    G_TYPE_OBJECT);
...
static void stack_fix_size (PeonyUndoStackManagerPrivate * priv);

static gboolean can_undo (PeonyUndoStackManagerPrivate * priv);

static gboolean can_redo (PeonyUndoStackManagerPrivate * priv);

static void stack_push_action (PeonyUndoStackManagerPrivate * priv,
    PeonyUndoStackActionData * action);

static PeonyUndoStackActionData
    * stack_scroll_left (PeonyUndoStackManagerPrivate * priv);

static PeonyUndoStackActionData
    * stack_scroll_right (PeonyUndoStackManagerPrivate * priv);

static PeonyUndoStackActionData
    * get_next_redo_action (PeonyUndoStackManagerPrivate * priv);

static PeonyUndoStackActionData
    * get_next_undo_action (PeonyUndoStackManagerPrivate * priv);

static gchar *get_undo_label (PeonyUndoStackActionData * action);

static gchar *get_undo_description (PeonyUndoStackActionData * action);

static gchar *get_redo_label (PeonyUndoStackActionData * action);

static gchar *get_redo_description (PeonyUndoStackActionData * action);

static void do_menu_update (PeonyUndoStackManager * manager);
...
static void undo_redo_done_rename_callback (PeonyFile * file,
...
static void clear_redo_actions (PeonyUndoStackManagerPrivate * priv);

static gchar *get_first_target_short_name (PeonyUndoStackActionData *
    action);
 ...
 ***************************************************************** */
static void
peony_undostack_manager_class_init (PeonyUndoStackManagerClass * klass)
{
  ...
  g_type_class_add_private (klass, sizeof (PeonyUndoStackManagerPrivate));
  ...
  g_object_class->set_property = peony_undostack_manager_set_property;
  g_object_class->get_property = peony_undostack_manager_get_property;
  ...
  /* Hook deconstructors */
  g_object_class->dispose = peony_undostack_manager_dispose;
  g_object_class->finalize = peony_undostack_manager_finalize;
}

static void
peony_undostack_manager_init (PeonyUndoStackManager * self)
{
  PeonyUndoStackManagerPrivate *priv;

  priv = PEONY_UNDOSTACK_MANAGER_GET_PRIVATE (self);
...
static void
peony_undostack_manager_dispose (GObject * object)
{
  PeonyUndoStackManager *self = PEONY_UNDOSTACK_MANAGER (object);
  PeonyUndoStackManagerPrivate *priv = self->priv;
  ...
  G_OBJECT_CLASS (peony_undostack_manager_parent_class)->dispose (object);
}

static void
peony_undostack_manager_finalize (GObject * object)
{
  G_OBJECT_CLASS (peony_undostack_manager_parent_class)->finalize (object);
}
 ...
 ***************************************************************** */
static void
peony_undostack_manager_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec)
{
  g_return_if_fail (IS_PEONY_UNDOSTACK_MANAGER (object));

  PeonyUndoStackManager *manager = PEONY_UNDOSTACK_MANAGER (object);
  PeonyUndoStackManagerPrivate *priv = manager->priv;
...
static void
peony_undostack_manager_get_property (GObject * object, guint prop_id,
...
{
  static PeonyUndoStackManager *manager = NULL;

  if (manager == NULL) {
    manager =
        g_object_new (TYPE_PEONY_UNDOSTACK_MANAGER, "undo-levels", 32, NULL);
  }
...
void
peony_undostack_manager_request_menu_update (PeonyUndoStackManager *
    manager)
{
  do_menu_update (manager);
}
    ...
    GtkWidget * parent_view, PeonyUndostackFinishCallback cb)
{
  ...
  PeonyUndoStackActionData *action = stack_scroll_left (priv);
  ...
  if (action != NULL) {
    ...
    switch (action->type) {
      ...
      case PEONY_UNDOSTACK_CREATEFILEFROMTEMPLATE:
        ...
        g_free (new_name);
        break;
      case PEONY_UNDOSTACK_DUPLICATE:
        uris = construct_gfile_list (action->sources, action->src_dir);
        peony_file_operations_duplicate (uris, NULL, NULL,
            undo_redo_done_transfer_callback, action);
    	g_list_free_full (uris, g_object_unref);
        break;
      case PEONY_UNDOSTACK_RESTOREFROMTRASH:
      case PEONY_UNDOSTACK_MOVE:
        uris = construct_gfile_list (action->sources, action->src_dir);
        peony_file_operations_move (uris, NULL,
            action->dest_dir, NULL, undo_redo_done_transfer_callback, action);
    	g_list_free_full (uris, g_object_unref);
        break;
      case PEONY_UNDOSTACK_RENAME:
        new_name = get_uri_basename (action->new_uri);
        file = peony_file_get_by_uri (action->old_uri);
        peony_file_rename (file, new_name,
            undo_redo_done_rename_callback, action);
        g_object_unref (file);
        g_free (new_name);
        break;
      case PEONY_UNDOSTACK_CREATEEMPTYFILE:
        puri = get_uri_parent (action->target_uri);
        new_name = get_uri_basename (action->target_uri);
        peony_file_operations_new_file (NULL, NULL, puri,
            new_name,
            action->template,
        ...
        g_free (new_name);
        break;
      case PEONY_UNDOSTACK_CREATEFOLDER:
        puri = get_uri_parent (action->target_uri);
        peony_file_operations_new_folder (NULL, NULL, puri,
            undo_redo_done_create_callback, action);
        g_free (puri);
        break;
      case PEONY_UNDOSTACK_MOVETOTRASH:
        if (g_hash_table_size (action->trashed) > 0) {
          ...
          priv->undo_redo_flag = TRUE;
          peony_file_operations_trash_or_delete
      ...
      case PEONY_UNDOSTACK_SETPERMISSIONS:
        file = peony_file_get_by_uri (action->target_uri);
        peony_file_set_permissions (file,
            action->new_permissions, undo_redo_done_rename_callback, action);
        g_object_unref (file);
        break;
      case PEONY_UNDOSTACK_RECURSIVESETPERMISSIONS:
        puri = g_file_get_uri (action->dest_dir);
        peony_file_set_permissions_recursive (puri,
        ...
        g_free (puri);
        break;
      case PEONY_UNDOSTACK_CHANGEGROUP:
        file = peony_file_get_by_uri (action->target_uri);
        peony_file_set_group (file,
            action->new_group_name_or_id,
            undo_redo_done_rename_callback, action);
        g_object_unref (file);
        break;
      case PEONY_UNDOSTACK_CHANGEOWNER:
        file = peony_file_get_by_uri (action->target_uri);
        peony_file_set_owner (file,
            action->new_user_name_or_id,
            undo_redo_done_rename_callback, action);
        g_object_unref (file);
        break;
      case PEONY_UNDOSTACK_DELETE:
      default:
        priv->undo_redo_flag = FALSE;
    ...
    GtkWidget * parent_view, PeonyUndostackFinishCallback cb)
{
  ...
  PeonyUndoStackActionData *action = stack_scroll_right (priv);
  ...
  if (action != NULL) {
    ...
    switch (action->type) {
      ...
      case PEONY_UNDOSTACK_RESTOREFROMTRASH:
        uris = construct_gfile_list (action->destinations, action->dest_dir);
        peony_file_operations_trash_or_delete (uris, NULL,
            undo_redo_done_delete_callback, action);
    	g_list_free_full (uris, g_object_unref);
        break;
      case PEONY_UNDOSTACK_MOVETOTRASH:
      ...
      case PEONY_UNDOSTACK_RENAME:
        ...
        file = peony_file_get_by_uri (action->new_uri);
        peony_file_rename (file, new_name,
            undo_redo_done_rename_callback, action);
        g_object_unref (file);
        g_free (new_name);
...
{
  PeonyUndoStackActionData *action = NULL;
  GList *trash = NULL;
  GList *l;
  GQueue *tmp_stack = g_queue_copy(stack);

  while ((action = (PeonyUndoStackActionData *) g_queue_pop_tail (tmp_stack)) != NULL)
    if (action->trashed)
 ...
 ** ****************************************************************/
void
peony_undostack_manager_trash_has_emptied (PeonyUndoStackManager *
    manager)
{
  PeonyUndoStackManagerPrivate *priv = manager->priv;
  ...
  while ((action = (PeonyUndoStackActionData *) g_queue_pop_tail (tmp_stack)) != NULL)
  {
    ...
    if (action->type == PEONY_UNDOSTACK_MOVETOTRASH) {
        g_queue_remove (priv->stack, action);
    }
  }
...
peony_undostack_manager_data_new (PeonyUndoStackActionType type,
    gint items_count)
{
  PeonyUndoStackActionData *data =
      g_slice_new0 (PeonyUndoStackActionData);
  data->type = type;
  data->count = items_count;

  if (type == PEONY_UNDOSTACK_MOVETOTRASH) {
    data->trashed =
        g_hash_table_new_full (g_str_hash, g_str_equal, g_free, g_free);
  } else if (type == PEONY_UNDOSTACK_RECURSIVESETPERMISSIONS) {
 ...
 ** ****************************************************************/
void
peony_undostack_manager_data_set_src_dir (PeonyUndoStackActionData *
    data, GFile * src)
{
  if (!data)
    return;
 ...
 ** ****************************************************************/
void
peony_undostack_manager_data_set_dest_dir (PeonyUndoStackActionData *
    data, GFile * dest)
{
  if (!data)
    return;
 ...
 ** ****************************************************************/
void peony_undostack_manager_data_add_origin_target_pair
 ...
 ** ****************************************************************/
void
peony_undostack_manager_data_add_trashed_file (PeonyUndoStackActionData
 ...
 ** ****************************************************************/
void peony_undostack_manager_data_add_file_permissions
 ...
 ** ****************************************************************/
void peony_undostack_manager_data_set_file_permissions
 ...
 ** ****************************************************************/
void peony_undostack_manager_data_set_owner_change_information
    (PeonyUndoStackActionData * data, char *uri,
 ...
 ** ****************************************************************/
void peony_undostack_manager_data_set_group_change_information
    (PeonyUndoStackActionData * data, char *uri,
 ...
 ** ****************************************************************/
void peony_undostack_manager_data_set_recursive_permissions
    (PeonyUndoStackActionData * data, guint32 file_permissions,
 ...
 ** ****************************************************************/
void
peony_undostack_manager_data_set_create_data (PeonyUndoStackActionData *
...
stack_clear_n_oldest (GQueue * stack, guint n)
{
  ...
  for (i = 0; i < n; i++) {
    if ((action = (PeonyUndoStackActionData *) g_queue_pop_tail (stack)) == NULL)
        break;
    if (action->locked) {
      action->freed = TRUE;
    } else {
...
/** ---------------------------------------------------------------- */
static void
stack_fix_size (PeonyUndoStackManagerPrivate * priv)
{
...
/** ---------------------------------------------------------------- */
static void
clear_redo_actions (PeonyUndoStackManagerPrivate * priv)
{
  while (priv->index > 0) {
    PeonyUndoStackActionData *head = (PeonyUndoStackActionData *)
...
/** ---------------------------------------------------------------- */
static void
stack_push_action (PeonyUndoStackManagerPrivate * priv,
    PeonyUndoStackActionData * action)
{
  guint length;
...
/** ---------------------------------------------------------------- */
static gchar *
get_first_target_short_name (PeonyUndoStackActionData * action)
{
  GList *targets_first;
  gchar *file_name;
...
/** ---------------------------------------------------------------- */
static gchar *
get_undo_description (PeonyUndoStackActionData * action)
{
...
/** ---------------------------------------------------------------- */
static gchar *
get_redo_description (PeonyUndoStackActionData * action)
{
  gchar *description = NULL;
...
/** ---------------------------------------------------------------- */
static gchar *
get_undo_label (PeonyUndoStackActionData * action)
{
  gchar *label = NULL;
  guint count;
...
/** ---------------------------------------------------------------- */
static gchar *
get_redo_label (PeonyUndoStackActionData * action)
{
  gchar *label = NULL;
  guint count;
...
undo_redo_done_transfer_callback (GHashTable * debuting_uris, gpointer data)
{
  ...
  action = (PeonyUndoStackActionData *) data;
  ...
  PeonyUndoStackManager *manager = action->manager;
...
/** ---------------------------------------------------------------- */
static void
undo_redo_done_rename_callback (PeonyFile * file,
    GFile * result_location, GError * error, gpointer callback_data)
{
  undo_redo_done_transfer_callback (NULL, callback_data);
}
...
{
  PeonyUndoStackActionData *action = (PeonyUndoStackActionData *) data;
  ...
  if (action)
    g_slice_free (PeonyUndoStackActionData, action);
}
...
/** ---------------------------------------------------------------- */
static gboolean
can_undo (PeonyUndoStackManagerPrivate * priv)
{
...
/** ---------------------------------------------------------------- */
static gboolean
can_redo (PeonyUndoStackManagerPrivate * priv)
{
...
get_next_redo_action (PeonyUndoStackManagerPrivate * priv)
{
  ...
  PeonyUndoStackActionData *action = g_queue_peek_nth (priv->stack,
      priv->index - 1);
...
/** ---------------------------------------------------------------- */
static void
do_menu_update (PeonyUndoStackManager * manager)
{
  ...
  PeonyUndoStackActionData *action;
  PeonyUndoStackManagerPrivate *priv = manager->priv;
  PeonyUndoStackMenuData *data = g_slice_new0 (PeonyUndoStackMenuData);
  ...
  g_slice_free (PeonyUndoStackMenuData, data);
}
