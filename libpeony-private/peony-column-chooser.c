...
/* peony-column-chooser.h - A column chooser widget
...
#include <config.h>
#include "peony-column-chooser.h"
...
struct _PeonyColumnChooserDetails
{
    GtkTreeView *view;
    GtkListStore *store;
...
#if GTK_CHECK_VERSION (3, 0, 0)
G_DEFINE_TYPE(PeonyColumnChooser, peony_column_chooser, GTK_TYPE_BOX);
#else
G_DEFINE_TYPE(PeonyColumnChooser, peony_column_chooser, GTK_TYPE_HBOX);
#endif

static void peony_column_chooser_constructed (GObject *object);

static void
peony_column_chooser_set_property (GObject *object,
                                  guint param_id,
                                  const GValue *value,
                                  GParamSpec *pspec)
{
    PeonyColumnChooser *chooser;

    chooser = PEONY_COLUMN_CHOOSER (object);
...
static void
peony_column_chooser_class_init (PeonyColumnChooserClass *chooser_class)
{
    GObjectClass *oclass;

    oclass = G_OBJECT_CLASS (chooser_class);

    oclass->set_property = peony_column_chooser_set_property;
    oclass->constructed = peony_column_chooser_constructed;

    signals[CHANGED] = g_signal_new
                       ("changed",
                        G_TYPE_FROM_CLASS (chooser_class),
                        G_SIGNAL_RUN_LAST,
                        G_STRUCT_OFFSET (PeonyColumnChooserClass,
                                         changed),
                        NULL, NULL,
    ...
    g_type_class_add_private (chooser_class, sizeof (PeonyColumnChooserDetails));
}

static void
update_buttons (PeonyColumnChooser *chooser)
{
...
static void
list_changed (PeonyColumnChooser *chooser)
{
                          ...
                          gpointer user_data)
{
    ...
    chooser = PEONY_COLUMN_CHOOSER (user_data);
...
{
    update_buttons (PEONY_COLUMN_CHOOSER (user_data));
}
...
row_deleted_callback (GtkTreeModel *model,
                      ...
                      gpointer user_data)
{
    list_changed (PEONY_COLUMN_CHOOSER (user_data));
}

static void
add_tree_view (PeonyColumnChooser *chooser)
{
    GtkWidget *scrolled;
    GtkWidget *view;
    GtkListStore *store;
...
{
    g_signal_emit (PEONY_COLUMN_CHOOSER (user_data),
...
static void
add_buttons (PeonyColumnChooser *chooser)
{
    GtkWidget *box;
    GtkWidget *separator;
...
static void
populate_tree (PeonyColumnChooser *chooser)
{
    GList *columns;
    GList *l;

    columns = peony_get_columns_for_file (chooser->details->file);
...
static void
peony_column_chooser_constructed (GObject *object)
{
    PeonyColumnChooser *chooser;
...
static void
peony_column_chooser_init (PeonyColumnChooser *chooser)
{
    chooser->details = G_TYPE_INSTANCE_GET_PRIVATE ((chooser), PEONY_TYPE_COLUMN_CHOOSER, PeonyColumnChooserDetails);
...
static void
set_visible_columns (PeonyColumnChooser *chooser,
...
static char **
get_column_names (PeonyColumnChooser *chooser, gboolean only_visible)
{
    GPtrArray *ret;
    GtkTreeIter iter;
...
static gboolean
get_column_iter (PeonyColumnChooser *chooser,
                 PeonyColumn *column,
                 GtkTreeIter *iter)
{
    char *column_name;

    g_object_get (PEONY_COLUMN (column), "name", &column_name, NULL);
...
static void
set_column_order (PeonyColumnChooser *chooser,
                  char **column_order)

{
    ...
    columns = peony_get_columns_for_file (chooser->details->file);
    columns = peony_sort_columns (columns, column_order);
    ...
    for (l = columns; l != NULL; l = l->next)
    {
        ...
        if (get_column_iter (chooser, PEONY_COLUMN (l->data), &iter))
        {
            GtkTreeIter before;
            if (path)
            {
...
void
peony_column_chooser_set_settings (PeonyColumnChooser *chooser,
                                  char **visible_columns,
                                  char **column_order)
{
    g_return_if_fail (PEONY_IS_COLUMN_CHOOSER (chooser));
...
void
peony_column_chooser_get_settings (PeonyColumnChooser *chooser,
                                  char ***visible_columns,
                                  char ***column_order)
{
    g_return_if_fail (PEONY_IS_COLUMN_CHOOSER (chooser));
...
GtkWidget *
peony_column_chooser_new (PeonyFile *file)
{
    return g_object_new (PEONY_TYPE_COLUMN_CHOOSER, "file", file, NULL);
}
