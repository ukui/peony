...
/* peony-dnd.c - Common Drag & drop handling code shared by the icon container
   and the list view.
...
/* FIXME: This should really be back in Peony, not here in Eel. */
...
#include "peony-window-slot-info.h"
...
void
peony_drag_init (PeonyDragInfo     *drag_info,
...
void
peony_drag_finalize (PeonyDragInfo *drag_info)
{
    gtk_target_list_unref (drag_info->target_list);
    peony_drag_destroy_selection_list (drag_info->selection_list);

    g_free (drag_info);
}


/* Functions to deal with PeonyDragSelectionItems.  */

PeonyDragSelectionItem *
peony_drag_selection_item_new (void)
{
    return g_new0 (PeonyDragSelectionItem, 1);
}

static void
drag_selection_item_destroy (PeonyDragSelectionItem *item)
{
    g_free (item->uri);
    g_free (item);
}
...
void
peony_drag_destroy_selection_list (GList *list)
{
    GList *p;
...
GList *
peony_drag_uri_list_from_selection_list (const GList *selection_list)
{
    PeonyDragSelectionItem *selection_item;
    GList *uri_list;
    const GList *l;
    ...
    {
        selection_item = (PeonyDragSelectionItem *) l->data;
...
GList *
peony_drag_uri_list_from_array (const char **uris)
{
    GList *uri_list;
    int i;
...
GList *
peony_drag_build_selection_list (GtkSelectionData *data)
{
    GList *result;
    const guchar *p, *oldp;
    int size;
    ...
    while (size > 0)
    {
        ...
        if (*p == '\n' || *p == '\0')
        {
            ...
            if (p == 0)
            {
                g_warning ("Invalid x-special/ukui-icon-list data received: "
...
static gboolean
peony_drag_file_local_internal (const char *target_uri_string,
...
gboolean
peony_drag_uris_local (const char *target_uri,
                      const GList *source_uri_list)
{
    /* must have at least one item */
    g_assert (source_uri_list);

    return peony_drag_file_local_internal (target_uri, source_uri_list->data);
}

gboolean
peony_drag_items_local (const char *target_uri_string,
                       const GList *selection_list)
{
    /* must have at least one item */
    g_assert (selection_list);

    return peony_drag_file_local_internal (target_uri_string,
                                          ((PeonyDragSelectionItem *)selection_list->data)->uri);
}

gboolean
peony_drag_items_on_desktop (const GList *selection_list)
{
    char *uri;
     ...
     * and currently dropped items can only be from the same container
     */
    uri = ((PeonyDragSelectionItem *)selection_list->data)->uri;
    if (eel_uri_is_desktop (uri))
    {
        return TRUE;
    }

    desktop = peony_get_desktop_location ();
...
GdkDragAction
peony_drag_default_drop_action_for_netscape_url (GdkDragContext *context)
{
               ...
               PeonyFile *file2)
{
    ...
    {
        id1 = peony_file_get_filesystem_id (file1);
        id2 = peony_file_get_filesystem_id (file2);
...
source_is_deletable (GFile *file)
{
    ...
    /* if there's no a cached PeonyFile, it returns NULL */
    naut_file = peony_file_get_existing (file);
    if (naut_file == NULL)
    {
        return FALSE;
    }

    ret = peony_file_can_delete (naut_file);
    peony_file_unref (naut_file);

    return ret;
}

void
peony_drag_default_drop_action_for_icons (GdkDragContext *context,
        ...
        int *action)
{
    ...
    PeonyFile *dropped_file, *target_file;
    ...
    dropped_uri = ((PeonyDragSelectionItem *)items->data)->uri;
    dropped_file = peony_file_get_existing_by_uri (dropped_uri);
    target_file = peony_file_get_existing_by_uri (target_uri_string);

    /*
     * Check for trash URI.  We do a find_directory for any Trash directory.
     * Passing 0 permissions as ukui-vfs would override the permissions
    ...
    if (eel_uri_is_trash (target_uri_string))
    {
        ...
        peony_file_unref (dropped_file);
        peony_file_unref (target_file);
        return;

    }
    else if (dropped_file != NULL && peony_file_is_launcher (dropped_file))
    {
        if (actions & GDK_ACTION_MOVE)
        {
            *action = GDK_ACTION_MOVE;
    ...
    {
        target = peony_get_desktop_location ();

        peony_file_unref (target_file);
        target_file = peony_file_get (target);
    ...
    else if (target_file != NULL && peony_file_is_archive (target_file))
    {
...
GdkDragAction
peony_drag_default_drop_action_for_uri_list (GdkDragContext *context,
...
static gboolean
is_path_that_ukui_uri_list_extract_filenames_can_parse (const char *path)
{
 ...
 * trying to make the old ukui_uri_list_extract_filenames function
...
add_one_compatible_uri (const char *uri, int x, int y, int w, int h, gpointer data)
{
     ...
     * ukui-libs function or nothing will be able to understand
     * it.
     */
    ...
    else
    {
         ...
         * ukui_uri_list_extract_filenames implementation, and just leave
         * out any paths with those in them.
         */
        if (is_path_that_ukui_uri_list_extract_filenames_can_parse (local_path))
        {
...
gboolean
peony_drag_drag_data_get (GtkWidget *widget,
                         ...
                         PeonyDragEachSelectedItemIterator each_selected_item_iterator)
{
    ...
    case PEONY_ICON_DND_UKUI_ICON_LIST:
        ...
        (* each_selected_item_iterator) (add_one_ukui_icon, container_context, result);
        break;
...
GdkDragAction
peony_drag_drop_action_ask (GtkWidget *widget,
                           GdkDragAction actions)
{
    GtkWidget *menu;
    GtkWidget *menu_item;
    ...
    append_drop_action_menu_item (menu, _("Set as _Background"),
                                  ...
                                  (actions & PEONY_DND_ACTION_SET_AS_BACKGROUND) != 0,
                                  &damd);
...
GdkDragAction
peony_drag_drop_background_ask (GtkWidget *widget,
                               GdkDragAction actions)
{
    GtkWidget *menu;
    GtkWidget *menu_item;
    ...
    append_drop_action_menu_item (menu, _("Set as background for _all folders"),
                                  PEONY_DND_ACTION_SET_AS_GLOBAL_BACKGROUND,
                                  (actions & PEONY_DND_ACTION_SET_AS_GLOBAL_BACKGROUND) != 0,
                                  &damd);

    append_drop_action_menu_item (menu, _("Set as background for _this folder"),
                                  PEONY_DND_ACTION_SET_AS_FOLDER_BACKGROUND,
                                  (actions & PEONY_DND_ACTION_SET_AS_FOLDER_BACKGROUND) != 0,
                                  &damd);
...
gboolean
peony_drag_autoscroll_in_scroll_region (GtkWidget *widget)
{
...
void
peony_drag_autoscroll_calculate_delta (GtkWidget *widget, float *x_scroll_delta, float *y_scroll_delta)
{
...
void
peony_drag_autoscroll_start (PeonyDragInfo *drag_info,
                            GtkWidget        *widget,
                            GSourceFunc       callback,
                            gpointer          user_data)
{
    if (peony_drag_autoscroll_in_scroll_region (widget))
    {
...
void
peony_drag_autoscroll_stop (PeonyDragInfo *drag_info)
{
    if (drag_info->auto_scroll_timeout_id != 0)
    {
        g_source_remove (drag_info->auto_scroll_timeout_id);
...
gboolean
peony_drag_selection_includes_special_link (GList *selection_list)
{
    GList *node;
    char *uri;
    ...
    {
        uri = ((PeonyDragSelectionItem *) node->data)->uri;
                        ...
                        gpointer            user_data)
{
    ...
    g_assert (PEONY_IS_WINDOW_INFO (window));
    ...
    else
    {
        ...
        {
            target_slot = peony_window_info_get_active_slot (PEONY_WINDOW_INFO (window));
        }

        if (target_slot != NULL)
        {
            target_uri = peony_window_slot_info_get_current_location (target_slot);
        }
    }
    ...
    {
        if (drag_info->info == PEONY_ICON_DND_UKUI_ICON_LIST)
        {
            peony_drag_default_drop_action_for_icons (context, target_uri,
                    drag_info->data.selection_list,
                    &action);
        }
        else if (drag_info->info == PEONY_ICON_DND_URI_LIST)
        {
            action = peony_drag_default_drop_action_for_uri_list (context, target_uri);
        }
        else if (drag_info->info == PEONY_ICON_DND_NETSCAPE_URL)
        {
...
static void
drag_info_clear (PeonyDragSlotProxyInfo *drag_info)
{
    if (!drag_info->have_data)
    {
        goto out;
    ...
    if (drag_info->info == PEONY_ICON_DND_UKUI_ICON_LIST)
    {
        peony_drag_destroy_selection_list (drag_info->data.selection_list);
    }
    else if (drag_info->info == PEONY_ICON_DND_URI_LIST)
    {
                        ...
                        PeonyDragSlotProxyInfo *drag_info)
{
    ...
    if (target_slot != NULL)
    {
        target_view = peony_window_slot_info_get_current_view (target_slot);
    }
    ...
    if (target_slot != NULL && target_view != NULL)
    {
        if (drag_info->info == PEONY_ICON_DND_UKUI_ICON_LIST)
        {
            uri_list = peony_drag_uri_list_from_selection_list (drag_info->data.selection_list);
            g_assert (uri_list != NULL);

            peony_view_drop_proxy_received_uris (target_view,
                                                uri_list,
                                                target_uri,
        ...
        if (drag_info->info == PEONY_ICON_DND_NETSCAPE_URL)
        {
            peony_view_drop_proxy_received_netscape_url (target_view,
                               ...
                               gpointer            user_data)
{
    ...
    {
        drag_info->data.selection_list = peony_drag_build_selection_list (data);
    ...
    else if (info == PEONY_ICON_DND_URI_LIST)
    {
        ...
        drag_info->data.uri_list = peony_drag_uri_list_from_array ((const char **) uris);
        g_strfreev (uris);

        drag_info->have_valid_data = drag_info->data.uri_list != NULL;
    }
    else if (info == PEONY_ICON_DND_NETSCAPE_URL)
    {
...
void
peony_drag_slot_proxy_init (GtkWidget *widget,
                           PeonyDragSlotProxyInfo *drag_info)
{
