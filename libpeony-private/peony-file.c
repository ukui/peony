...
#include <libpeony-extension/peony-extension-private.h>
...
#undef PEONY_FILE_DEBUG_REF
#undef PEONY_FILE_DEBUG_REF_VALGRIND

#ifdef PEONY_FILE_DEBUG_REF_VALGRIND
...
/* Name of Peony trash directories */
...
typedef void (* ModifyListFunction) (GList **list, PeonyFile *file);
...
static void     peony_file_info_iface_init                (PeonyFileInfoIface *iface);
static char *   peony_file_get_owner_as_string            (PeonyFile          *file,
							      gboolean               include_real_name);
static char *   peony_file_get_type_as_string             (PeonyFile          *file);
static gboolean update_info_and_name                         (PeonyFile          *file,
							      GFileInfo             *info);
...
G_DEFINE_TYPE_WITH_CODE (PeonyFile, peony_file, G_TYPE_OBJECT,
			 G_IMPLEMENT_INTERFACE (PEONY_TYPE_FILE_INFO,
						peony_file_info_iface_init));

static void
peony_file_init (PeonyFile *file)
{
	file->details = G_TYPE_INSTANCE_GET_PRIVATE ((file), PEONY_TYPE_FILE, PeonyFileDetails);

	peony_file_clear_info (file);
	peony_file_invalidate_extension_info_internal (file);
}

static GObject*
peony_file_constructor (GType                  type,
			   ...
			   GObjectConstructParam *construct_params)
{
  ...
  object = (* G_OBJECT_CLASS (peony_file_parent_class)->constructor) (type,
									 n_construct_properties,
									 construct_params);

  file = PEONY_FILE (object);

  /* Set to default type after full construction */
  if (PEONY_FILE_GET_CLASS (file)->default_file_type != G_FILE_TYPE_UNKNOWN) {
	  file->details->type = PEONY_FILE_GET_CLASS (file)->default_file_type;
  }
...
gboolean
peony_file_set_display_name (PeonyFile *file,
...
static void
peony_file_clear_display_name (PeonyFile *file)
{
	eel_ref_str_unref (file->details->display_name);
...
get_metadata_from_info (GFileInfo *info)
{
	...
	for (i = 0; attrs[i] != NULL; i++) {
		id = peony_metadata_get_id (attrs[i] + strlen ("metadata::"));
		if (id == 0) {
			continue;
		}
...
gboolean
peony_file_update_metadata_from_info (PeonyFile *file,
					 GFileInfo *info)
{
...
peony_file_clear_info (PeonyFile *file)
{
	...
	if (!file->details->got_custom_display_name) {
		peony_file_clear_display_name (file);
	}
...
static PeonyFile *
peony_file_new_from_filename (PeonyDirectory *directory,
				 const char *filename,
				 gboolean self_owned)
{
	PeonyFile *file;
	...
	if (PEONY_IS_DESKTOP_DIRECTORY (directory)) {
		if (self_owned) {
			file = PEONY_FILE (g_object_new (PEONY_TYPE_DESKTOP_DIRECTORY_FILE, NULL));
		} else {
			/* This doesn't normally happen, unless the user somehow types in a uri
			 * that references a file like this. (See #349840) */
			file = PEONY_FILE (g_object_new (PEONY_TYPE_VFS_FILE, NULL));
		}
	} else if (PEONY_IS_SEARCH_DIRECTORY (directory)) {
		if (self_owned) {
			file = PEONY_FILE (g_object_new (PEONY_TYPE_SEARCH_DIRECTORY_FILE, NULL));
		} else {
			/* This doesn't normally happen, unless the user somehow types in a uri
			 * that references a file like this. (See #349840) */
			file = PEONY_FILE (g_object_new (PEONY_TYPE_VFS_FILE, NULL));
		}
	} else if (g_str_has_suffix (filename, PEONY_SAVED_SEARCH_EXTENSION)) {
		file = PEONY_FILE (g_object_new (PEONY_TYPE_SAVED_SEARCH_FILE, NULL));
	} else {
		file = PEONY_FILE (g_object_new (PEONY_TYPE_VFS_FILE, NULL));
	}

	file->details->directory = peony_directory_ref (directory);

	file->details->name = eel_ref_str_new (filename);

#ifdef PEONY_FILE_DEBUG_REF
...
static void
modify_link_hash_table (PeonyFile *file,
...
static void
add_to_link_hash_table_list (GList **list, PeonyFile *file)
{
...
static void
add_to_link_hash_table (PeonyFile *file)
{
	modify_link_hash_table (file, add_to_link_hash_table_list);
}

static void
remove_from_link_hash_table_list (GList **list, PeonyFile *file)
{
	if (g_list_find (*list, file) != NULL) {
...
static void
remove_from_link_hash_table (PeonyFile *file)
{
	modify_link_hash_table (file, remove_from_link_hash_table_list);
}

PeonyFile *
peony_file_new_from_info (PeonyDirectory *directory,
			     GFileInfo *info)
{
	PeonyFile *file;
	const char *mime_type;

	g_return_val_if_fail (PEONY_IS_DIRECTORY (directory), NULL);
	g_return_val_if_fail (info != NULL, NULL);

	mime_type = g_file_info_get_content_type (info);
	if (mime_type &&
	    strcmp (mime_type, PEONY_SAVED_SEARCH_MIMETYPE) == 0) {
...
static PeonyFile *
peony_file_get_internal (GFile *location, gboolean create)
{
	gboolean self_owned;
	...
	directory = peony_directory_get_internal (parent, create);

	g_object_unref (parent);

	/* Get the name for the file. */
	if (self_owned && directory != NULL) {
		basename = peony_directory_get_name_for_self_as_new_file (directory);
	} else {
	...
	if (directory == NULL) {
		...
		file = peony_directory_find_file_by_name (directory, basename);
	}
	...
	if (file != NULL) {
		...
		file = peony_file_new_from_filename (directory, basename, self_owned);
		if (self_owned) {
			g_assert (directory->details->as_file == NULL);
			directory->details->as_file = file;
		} else {
			peony_directory_add_file (directory, file);
		}
	}
...
PeonyFile *
peony_file_get (GFile *location)
{
	return peony_file_get_internal (location, TRUE);
}

PeonyFile *
peony_file_get_existing (GFile *location)
{
	return peony_file_get_internal (location, FALSE);
}

PeonyFile *
peony_file_get_existing_by_uri (const char *uri)
{
	GFile *location;
	PeonyFile *file;

	location = g_file_new_for_uri (uri);
	file = peony_file_get_internal (location, FALSE);
...
PeonyFile *
peony_file_get_by_uri (const char *uri)
{
	GFile *location;
	PeonyFile *file;

	location = g_file_new_for_uri (uri);
	file = peony_file_get_internal (location, TRUE);
	g_object_unref (location);
...
gboolean
peony_file_is_self_owned (PeonyFile *file)
{
...
finalize (GObject *object)
{
	...
	if (file->details->is_thumbnailing) {
		...
		peony_thumbnail_remove_from_queue (uri);
		g_free (uri);
	}

	peony_async_destroying_file (file);
	...
	if (peony_file_is_self_owned (file)) {
		...
		if (!file->details->is_gone) {
			peony_directory_remove_file (directory, file);
		}
	}
	...
	G_OBJECT_CLASS (peony_file_parent_class)->finalize (object);
}

PeonyFile *
peony_file_ref (PeonyFile *file)
{
	if (file == NULL) {
		return NULL;
	}
...
/**
 * peony_file_get_parent_uri_for_display:
 *
 ...
 */
char *
peony_file_get_parent_uri_for_display (PeonyFile *file)
{
	GFile *parent;
	char *result;
	...
	parent = peony_file_get_parent_location (file);
	if (parent) {
 ...
 * Use peony_file_get_parent_uri_for_display instead if the
 * result is to be displayed on-screen.
 * If the parent is NULL, returns the empty string.
 */
char *
peony_file_get_parent_uri (PeonyFile *file)
{
	g_assert (PEONY_IS_FILE (file));
	...
	return peony_directory_get_uri (file->details->directory);
}

GFile *
peony_file_get_parent_location (PeonyFile *file)
{
	g_assert (PEONY_IS_FILE (file));
	...
	return peony_directory_get_location (file->details->directory);
}

PeonyFile *
peony_file_get_parent (PeonyFile *file)
{
	g_assert (PEONY_IS_FILE (file));
	...
	return peony_directory_get_corresponding_file (file->details->directory);
}
...
{
	g_return_val_if_fail (PEONY_IS_FILE (file), FALSE);

	return file->details->can_read;
}

/**
 * peony_file_can_write:
 *
 ...
 */
gboolean
peony_file_can_write (PeonyFile *file)
{
	g_return_val_if_fail (PEONY_IS_FILE (file), FALSE);
 ...
 */
gboolean
peony_file_can_execute (PeonyFile *file)
{
	g_return_val_if_fail (PEONY_IS_FILE (file), FALSE);
...
gboolean
peony_file_can_mount (PeonyFile *file)
{
	g_return_val_if_fail (PEONY_IS_FILE (file), FALSE);
...
gboolean
peony_file_can_unmount (PeonyFile *file)
{
	g_return_val_if_fail (PEONY_IS_FILE (file), FALSE);
...
gboolean
peony_file_can_eject (PeonyFile *file)
{
	g_return_val_if_fail (PEONY_IS_FILE (file), FALSE);
...
gboolean
peony_file_can_start (PeonyFile *file)
{
	gboolean ret;
	GDrive *drive;
...
gboolean
peony_file_can_start_degraded (PeonyFile *file)
{
	gboolean ret;
	GDrive *drive;
...
gboolean
peony_file_can_poll_for_media (PeonyFile *file)
{
	gboolean ret;
	GDrive *drive;
...
gboolean
peony_file_is_media_check_automatic (PeonyFile *file)
{
	gboolean ret;
	GDrive *drive;
...
gboolean
peony_file_can_stop (PeonyFile *file)
{
	gboolean ret;
	GDrive *drive;
...
GDriveStartStopType
peony_file_get_start_stop_type (PeonyFile *file)
{
		     ...
		     gpointer                        callback_data)
{
	...
	if (PEONY_FILE_GET_CLASS (file)->mount == NULL) {
		if (callback) {
			error = NULL;
		...
		PEONY_FILE_GET_CLASS (file)->mount (file, mount_op, cancellable, callback, callback_data);
	}
}
...
unmount_done (void *callback_data)
{
	...
	peony_file_unref (data->file);
	g_free (data);
}

void
peony_file_unmount (PeonyFile                   *file,
		       GMountOperation                *mount_op,
		       GCancellable                   *cancellable,
		       PeonyFileOperationCallback   callback,
		       gpointer                        callback_data)
{
	...
	if (file->details->can_unmount) {
		if (PEONY_FILE_GET_CLASS (file)->unmount != NULL) {
			PEONY_FILE_GET_CLASS (file)->unmount (file, mount_op, cancellable, callback, callback_data);
		} else {
			if (callback) {
				error = NULL;
				g_set_error_literal (&error, G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED,
		...
		data->file = peony_file_ref (file);
		data->callback = callback;
		data->callback_data = callback_data;
		peony_file_operations_unmount_mount_full (NULL, file->details->mount, FALSE, TRUE, unmount_done, data);
	} else if (callback) {
...
void
peony_file_eject (PeonyFile                   *file,
		     GMountOperation                *mount_op,
		     GCancellable                   *cancellable,
		     PeonyFileOperationCallback   callback,
		     gpointer                        callback_data)
{
	...
	if (file->details->can_eject) {
		if (PEONY_FILE_GET_CLASS (file)->eject != NULL) {
			PEONY_FILE_GET_CLASS (file)->eject (file, mount_op, cancellable, callback, callback_data);
		} else {
			if (callback) {
				error = NULL;
				g_set_error_literal (&error, G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED,
		...
		data->callback = callback;
		data->callback_data = callback_data;
		peony_file_operations_unmount_mount_full (NULL, file->details->mount, TRUE, TRUE, unmount_done, data);
	} else if (callback) {
		callback (file, NULL, NULL, callback_data);
	}
}
...
void
peony_file_start (PeonyFile                   *file,
		     ...
		     gpointer                        callback_data)
{
	...
	if ((file->details->can_start || file->details->can_start_degraded) &&
	    PEONY_FILE_GET_CLASS (file)->start != NULL) {
		PEONY_FILE_GET_CLASS (file)->start (file, start_op, cancellable, callback, callback_data);
	} else {
		if (callback) {
			error = NULL;
			g_set_error_literal (&error, G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED,
		    ...
		    gpointer callback_data)
{
	...
	peony_file_operation_complete (op, NULL, error);
	if (error) {
		g_error_free (error);
	}
}
...
void
peony_file_stop (PeonyFile                   *file,
		    GMountOperation                *mount_op,
		    GCancellable                   *cancellable,
		    PeonyFileOperationCallback   callback,
		    gpointer                        callback_data)
{
	...
	if (PEONY_FILE_GET_CLASS (file)->stop != NULL) {
		if (file->details->can_stop) {
			PEONY_FILE_GET_CLASS (file)->stop (file, mount_op, cancellable, callback, callback_data);
		} else {
			if (callback) {
				error = NULL;
				g_set_error_literal (&error, G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED,
		...
		if (drive != NULL && g_drive_can_stop (drive)) {
			...
			op = peony_file_operation_new (file, callback, callback_data);
			if (cancellable) {
...
void
peony_file_poll_for_media (PeonyFile *file)
{
	if (file->details->can_poll_for_media) {
		if (PEONY_FILE_GET_CLASS (file)->stop != NULL) {
			PEONY_FILE_GET_CLASS (file)->poll_for_media (file);
		}
	} else if (file->details->mount != NULL) {
		GDrive *drive;
		drive = g_mount_get_drive (file->details->mount);
...
/**
 * peony_file_is_desktop_directory:
 *
 ...
 */
gboolean
peony_file_is_desktop_directory (PeonyFile *file)
{
	GFile *dir;
	...
	return peony_is_desktop_directory_file (dir, eel_ref_str_peek (file->details->name));
}

static gboolean
is_desktop_file (PeonyFile *file)
{
 ...
 */
gboolean
peony_file_can_rename (PeonyFile *file)
{
	gboolean can_rename;
	...
	if (PEONY_IS_DESKTOP_ICON_FILE (file)) {
		...
		if (link != NULL) {
			can_rename = peony_desktop_link_can_rename (link);
			g_object_unref (link);
		}
	}
...
gboolean
peony_file_can_delete (PeonyFile *file)
{
	g_return_val_if_fail (PEONY_IS_FILE (file), FALSE);
...
gboolean
peony_file_can_trash (PeonyFile *file)
{
	g_return_val_if_fail (PEONY_IS_FILE (file), FALSE);
...
GFile *
peony_file_get_location (PeonyFile *file)
{
	GFile *dir;
...
char *
peony_file_get_uri (PeonyFile *file)
{
	char *uri;
	GFile *loc;
	...
	loc = peony_file_get_location (file);
...
char *
peony_file_get_uri_scheme (PeonyFile *file)
{
	GFile *loc;
	char *scheme;
	...
	loc = peony_directory_get_location (file->details->directory);
...
PeonyFileOperation *
peony_file_operation_new (PeonyFile *file,
			     PeonyFileOperationCallback callback,
			     gpointer callback_data)
{
	PeonyFileOperation *op;

	op = g_new0 (PeonyFileOperation, 1);
	op->file = peony_file_ref (file);
...
static void
peony_file_operation_remove (PeonyFileOperation *op)
{
...
void
peony_file_operation_free (PeonyFileOperation *op)
{
	peony_file_operation_remove (op);
	peony_file_unref (op->file);
	g_object_unref (op->cancellable);
	if (op->free_data) {
		op->free_data (op->data);
	}
	// Start UNDO-REDO
	peony_undostack_manager_add_action (peony_undostack_manager_instance(),
										   op->undo_redo_data);
	// End UNDO-REDO
	g_free (op);
}
...
void
peony_file_operation_complete (PeonyFileOperation *op, GFile *result_file, GError *error)
{
	 ...
	 */
	peony_file_operation_remove (op);
	peony_file_changed (op->file);
	if (op->callback) {
		(* op->callback) (op->file, result_file, error, op->callback_data);
	}
	peony_file_operation_free (op);
}

void
peony_file_operation_cancel (PeonyFileOperation *op)
{
			  ...
			  gpointer callback_data)
{
	...
	if (new_info != NULL) {
		 ...
		 */
		existing_file = peony_directory_find_file_by_name (directory, new_name);
		if (existing_file != NULL) {
			peony_file_mark_gone (existing_file);
			peony_file_changed (existing_file);
		}

		old_uri = peony_file_get_uri (op->file);
		...
		new_uri = peony_file_get_uri (op->file);
		peony_directory_moved (old_uri, new_uri);
		g_free (new_uri);
		g_free (old_uri);
		...
		if (op->file->details->got_custom_display_name) {
			peony_file_invalidate_attributes (op->file,
							     PEONY_FILE_ATTRIBUTE_INFO |
		 ...
		 gpointer callback_data)
{
	...
	if (new_file != NULL) {
		// Start UNDO-REDO
		peony_undostack_manager_data_set_rename_information(op->undo_redo_data, G_FILE (source_object), new_file);
		// End UNDO-REDO
		g_file_query_info_async (new_file,
					 PEONY_FILE_DEFAULT_ATTRIBUTES,
					 0,
					 G_PRIORITY_DEFAULT,
					 op->cancellable,
...
static gboolean
name_is (PeonyFile *file, const char *new_name)
{
	const char *old_name;
...
void
peony_file_rename (PeonyFile *file,
		      const char *new_name,
		      PeonyFileOperationCallback callback,
		      gpointer callback_data)
{
	...
	if (PEONY_IS_DESKTOP_ICON_FILE (file)) {
		...
		if ((old_name != NULL && strcmp (new_name, old_name) == 0)) {
			...
			success = (link != NULL && peony_desktop_link_rename (link, new_name));
		}
	...
	if (is_renameable_desktop_file) {
		 ...
		 */
		uri = peony_file_get_uri (file);
		old_name = peony_link_local_get_text (uri);
		if (old_name != NULL && strcmp (new_name, old_name) == 0) {
			success = TRUE;
			name_changed = FALSE;
		} else {
			success = peony_link_local_set_text (uri, new_name);
			name_changed = TRUE;
		}
		g_free (old_name);
		g_free (uri);
	...
	// Start UNDO-REDO
	if (!peony_undostack_manager_is_undo_redo(peony_undostack_manager_instance())) {
		op->undo_redo_data = peony_undostack_manager_data_new (PEONY_UNDOSTACK_RENAME, 1);
	}
	// End UNDO-REDO
...
gboolean
peony_file_rename_in_progress (PeonyFile *file)
{
	GList *node;
	PeonyFileOperation *op;
...
void
peony_file_cancel (PeonyFile *file,
		      PeonyFileOperationCallback callback,
		      gpointer callback_data)
{
	GList *node, *next;
	...
	for (node = file->details->operations_in_progress; node != NULL; node = next) {
		...
		if (op->callback == callback && op->callback_data == callback_data) {
			peony_file_operation_cancel (op);
		}
	}
}

gboolean
peony_file_matches_uri (PeonyFile *file, const char *match_uri)
{
...
int
peony_file_compare_location (PeonyFile *file_1,
                                PeonyFile *file_2)
{
	GFile *loc_a, *loc_b;
	gboolean res;

	loc_a = peony_file_get_location (file_1);
	loc_b = peony_file_get_location (file_2);
...
gboolean
peony_file_is_local (PeonyFile *file)
{
	g_return_val_if_fail (PEONY_IS_FILE (file), FALSE);

	return peony_directory_is_local (file->details->directory);
}

static void
update_link (PeonyFile *link_file, PeonyFile *target_file)
{
	g_assert (PEONY_IS_FILE (link_file));
	g_assert (PEONY_IS_FILE (target_file));
...
static GList *
get_link_files (PeonyFile *target_file)
{
	char *uri;
	GList **link_files;
	...
	if (symbolic_links == NULL) {
		...
		uri = peony_file_get_uri (target_file);
		link_files = g_hash_table_lookup (symbolic_links, uri);
		g_free (uri);
	}
	if (link_files) {
		return peony_file_list_copy (*link_files);
	}
	return NULL;
}

static void
update_links_if_target (PeonyFile *target_file)
{
	GList *link_files, *p;

	link_files = get_link_files (target_file);
	for (p = link_files; p != NULL; p = p->next) {
		update_link (PEONY_FILE (p->data), target_file);
	}
	peony_file_list_free (link_files);
}
		      ...
		      gboolean update_name)
{
	...
	changed |= peony_file_set_display_name (file,
	...
	changed |=
		peony_file_update_metadata_from_info (file, info);
	...
	if (update_name) {
		...
		if (file->details->name == NULL ||
			...
			node = peony_directory_begin_file_name_change
			...
			if (!file->details->got_custom_display_name &&
				 ...
				 * peony_file_set_display_name() did
				 * not unset the display name.
				 */
				peony_file_clear_display_name (file);
			}

			peony_directory_end_file_name_change
...
gboolean
peony_file_update_info (PeonyFile *file,
			   GFileInfo *info)
{
...
gboolean
peony_file_update_name (PeonyFile *file, const char *name)
{
	gboolean ret;
...
gboolean
peony_file_update_name_and_directory (PeonyFile *file,
					 const char *name,
					 PeonyDirectory *new_directory)
{
	PeonyDirectory *old_directory;
	FileMonitors *monitors;

	g_return_val_if_fail (PEONY_IS_FILE (file), FALSE);
	g_return_val_if_fail (PEONY_IS_DIRECTORY (file->details->directory), FALSE);
	g_return_val_if_fail (!file->details->is_gone, FALSE);
	g_return_val_if_fail (!peony_file_is_self_owned (file), FALSE);
	g_return_val_if_fail (PEONY_IS_DIRECTORY (new_directory), FALSE);
	...
	monitors = peony_directory_remove_file_monitors (old_directory, file);
	peony_directory_remove_file (old_directory, file);

	file->details->directory = peony_directory_ref (new_directory);
	...
	peony_directory_add_file (new_directory, file);
	peony_directory_add_file_monitors (new_directory, file, monitors);
...
void
peony_file_set_directory (PeonyFile *file,
			     PeonyDirectory *new_directory)
{
	peony_file_update_name_and_directory (file, NULL, new_directory);
}

static Knowledge
get_item_count (PeonyFile *file,
		guint *count)
{
	gboolean known, unreadable;

	known = peony_file_get_directory_item_count
	  ...
	  goffset *size)
{
	 ...
	 * such thing as a size as far as ukui-vfs is concerned,
	 * so "unknowable".
	 */
	  ...
	  PeonyDateType type)
{
	...
	time = 0;
	switch (type) {
	case PEONY_DATE_TYPE_MODIFIED:
		time = file->details->mtime;
		break;
	case PEONY_DATE_TYPE_ACCESSED:
		time = file->details->atime;
		break;
	case PEONY_DATE_TYPE_TRASHED:
	...
	*time_out = time;
	 ...
	 * ukui-vfs is concerned, so "unknowable".
	 */
	if (time == 0) {
		return UNKNOWABLE;
	}
...
static int
compare_directories_by_count (PeonyFile *file_1, PeonyFile *file_2)
{
	/* Sort order:
...
static int
compare_files_by_size (PeonyFile *file_1, PeonyFile *file_2)
{
	/* Sort order:
...
static int
compare_by_size (PeonyFile *file_1, PeonyFile *file_2)
{
	/* Sort order:
	 *   Directories with n items
	 *   Directories with 0 items
	...
	is_directory_2 = peony_file_is_directory (file_2);
...
static int
compare_by_display_name (PeonyFile *file_1, PeonyFile *file_2)
{
	...
	name_1 = peony_file_peek_display_name (file_1);
	name_2 = peony_file_peek_display_name (file_2);
	...
	if (sort_last_1 && !sort_last_2) {
		...
		key_1 = peony_file_peek_display_name_collation_key (file_1);
		key_2 = peony_file_peek_display_name_collation_key (file_2);
...
static int
compare_by_directory_name (PeonyFile *file_1, PeonyFile *file_2)
{
	...
	directory_1 = peony_file_get_parent_uri_for_display (file_1);
	directory_2 = peony_file_get_parent_uri_for_display (file_2);
...
file_has_note (PeonyFile *file)
{
	...
	note = peony_file_get_metadata (file, PEONY_METADATA_KEY_ANNOTATION, NULL);
...
static GList *
prepend_automatic_keywords (PeonyFile *file,
			    GList *names)
{
	...
	if (peony_file_is_in_trash (file)) {
		names = g_list_prepend
			(names, g_strdup (PEONY_FILE_EMBLEM_NAME_TRASH));
	}
#endif
	if (file_has_note (file)) {
		names = g_list_prepend
			(names, g_strdup (PEONY_FILE_EMBLEM_NAME_NOTE));
	}

	/* Trash files are assumed to be read-only,
	 * so we want to ignore them here. */
	if (!peony_file_can_write (file) &&
	    !peony_file_is_in_trash (file) &&
	    (parent == NULL || peony_file_can_write (parent))) {
		names = g_list_prepend
			(names, g_strdup (PEONY_FILE_EMBLEM_NAME_CANT_WRITE));
	}
	if (!peony_file_can_read (file)) {
		names = g_list_prepend
			(names, g_strdup (PEONY_FILE_EMBLEM_NAME_CANT_READ));
	}
	if (peony_file_is_symbolic_link (file)) {
		names = g_list_prepend
			(names, g_strdup (PEONY_FILE_EMBLEM_NAME_SYMBOLIC_LINK));
	}
...
static void
fill_emblem_cache_if_needed (PeonyFile *file)
{
	GList *node, *keywords;
	char *scanner;
	size_t length;
	...
	file->details->compare_by_emblem_cache = g_malloc (sizeof(PeonyFileSortByEmblemCache) + length);
...
static int
compare_by_emblems (PeonyFile *file_1, PeonyFile *file_2)
{
...
static int
compare_by_type (PeonyFile *file_1, PeonyFile *file_2)
{
	...
	type_string_1 = peony_file_get_type_as_string (file_1);
	type_string_2 = peony_file_get_type_as_string (file_2);
...
static int
compare_by_time (PeonyFile *file_1, PeonyFile *file_2, PeonyDateType type)
{
	/* Sort order:
...
static int
compare_by_full_path (PeonyFile *file_1, PeonyFile *file_2)
{
	int compare;
...
static int
peony_file_compare_for_sort_internal (PeonyFile *file_1,
					 PeonyFile *file_2,
 ...
 **/
int
peony_file_compare_for_sort (PeonyFile *file_1,
				PeonyFile *file_2,
				...
				gboolean reversed)
{
	...
	result = peony_file_compare_for_sort_internal (file_1, file_2, directories_first, reversed);

	if (result == 0) {
		switch (sort_type) {
		case PEONY_FILE_SORT_BY_DISPLAY_NAME:
			result = compare_by_display_name (file_1, file_2);
			if (result == 0) {
				result = compare_by_directory_name (file_1, file_2);
			}
			break;
		case PEONY_FILE_SORT_BY_DIRECTORY:
			result = compare_by_full_path (file_1, file_2);
			break;
		case PEONY_FILE_SORT_BY_SIZE:
			/* Compare directory sizes ourselves, then if necessary
			 * use UkuiVFS to compare file sizes.
			 */
			...
			break;
		case PEONY_FILE_SORT_BY_TYPE:
			/* UkuiVFS doesn't know about our special text for certain
			...
			break;
		case PEONY_FILE_SORT_BY_MTIME:
			result = compare_by_time (file_1, file_2, PEONY_DATE_TYPE_MODIFIED);
			if (result == 0) {
				result = compare_by_full_path (file_1, file_2);
			}
			break;
		case PEONY_FILE_SORT_BY_ATIME:
			result = compare_by_time (file_1, file_2, PEONY_DATE_TYPE_ACCESSED);
			if (result == 0) {
				result = compare_by_full_path (file_1, file_2);
			}
			break;
		case PEONY_FILE_SORT_BY_TRASHED_TIME:
			result = compare_by_time (file_1, file_2, PEONY_DATE_TYPE_TRASHED);
			if (result == 0) {
				result = compare_by_full_path (file_1, file_2);
			}
			break;
		case PEONY_FILE_SORT_BY_EMBLEMS:
			/* UkuiVFS doesn't know squat about our emblems, so
...
int
peony_file_compare_for_sort_by_attribute_q   (PeonyFile                   *file_1,
						 PeonyFile                   *file_2,
						 GQuark                          attribute,
						 gboolean                        directories_first,
						 gboolean                        reversed)
{
	...
	/* Convert certain attributes into PeonyFileSortTypes and use
	 * peony_file_compare_for_sort()
	 */
	if (attribute == 0 || attribute == attribute_name_q) {
		return peony_file_compare_for_sort (file_1, file_2,
	...
	if (result == 0) {
		...
		value_1 = peony_file_get_string_attribute_q (file_1,
								attribute);
		value_2 = peony_file_get_string_attribute_q (file_2,
								attribute);
...
int
peony_file_compare_for_sort_by_attribute     (PeonyFile                   *file_1,
						 PeonyFile                   *file_2,
						 const char                     *attribute,
						 gboolean                        directories_first,
						 gboolean                        reversed)
{
	return peony_file_compare_for_sort_by_attribute_q (file_1, file_2,
 ...
 **/
int
peony_file_compare_display_name (PeonyFile *file,
				    const char *pattern)
{
	const char *name;
	int result;
	...
	name = peony_file_peek_display_name (file);
...
gboolean
peony_file_is_hidden_file (PeonyFile *file)
{
	return file->details->is_hidden;
}

static gboolean
is_file_hidden (PeonyFile *file)
{
			   ...
			   gboolean show_foreign)
{
	...
	if (peony_file_is_in_trash (file)) {
		...
		return (show_hidden || (!peony_file_is_hidden_file (file) && !is_file_hidden (file))) &&
			(show_foreign || !(peony_file_is_in_desktop (file) && peony_file_is_foreign_link (file)));
	}
}

gboolean
peony_file_is_home (PeonyFile *file)
{
	GFile *dir;
	...
	return peony_is_home_directory_file (dir,
						eel_ref_str_peek (file->details->name));
}

gboolean
peony_file_is_in_desktop (PeonyFile *file)
{
	if (file->details->directory->details->location) {
		return peony_is_desktop_directory (file->details->directory->details->location);
	}
	return FALSE;
...
GList *
peony_file_list_filter_hidden (GList    *files,
					     gboolean  show_hidden)
{
	...
	filtered_files = peony_file_list_copy (files);
	filtered_files = eel_g_list_partition (filtered_files,
					       ...
					       &removed_files);
	peony_file_list_free (removed_files);

	return filtered_files;
}

char *
peony_file_get_metadata (PeonyFile *file,
			    const char *key,
			    const char *default_metadata)
{
	...
	g_return_val_if_fail (PEONY_IS_FILE (file), g_strdup (default_metadata));

	id = peony_metadata_get_id (key);
...
GList *
peony_file_get_metadata_list (PeonyFile *file,
				 const char *key)
{
	GList *res;
	guint id;
...
void
peony_file_set_metadata (PeonyFile *file,
			    const char *key,
			    const char *default_metadata,
			    const char *metadata)
{
	...
	EEL_CALL_METHOD
		(PEONY_FILE_CLASS, file,
		 set_metadata, (file, key, val));
}

void
peony_file_set_metadata_list (PeonyFile *file,
				 const char *key,
				 GList *list)
{
	char **val;
...
gboolean
peony_file_get_boolean_metadata (PeonyFile *file,
				    const char   *key,
				    gboolean      default_metadata)
{
	...
	g_return_val_if_fail (PEONY_IS_FILE (file), default_metadata);

	result_as_string = peony_file_get_metadata
...
int
peony_file_get_integer_metadata (PeonyFile *file,
				    const char   *key,
				 ...
				 const char   *key)
{
	...
	time_string = peony_file_get_metadata (file, key, NULL);
...
void
peony_file_set_time_metadata (PeonyFile *file,
				 const char   *key,
				 time_t        time)
{
	char time_str[21];
	...
	peony_file_set_metadata (file, key, NULL, metadata);
}


void
peony_file_set_boolean_metadata (PeonyFile *file,
				    const char   *key,
				    gboolean      default_metadata,
				    gboolean      metadata)
{
	...
	peony_file_set_metadata (file, key,
				    default_metadata ? "true" : "false",
				    metadata ? "true" : "false");
}

void
peony_file_set_integer_metadata (PeonyFile *file,
				    const char   *key,
				    int           default_metadata,
				    int           metadata)
{
...
static const char *
peony_file_peek_display_name_collation_key (PeonyFile *file)
{
	const char *res;
...
static const char *
peony_file_peek_display_name (PeonyFile *file)
{
	const char *name;
	char *escaped_name;
	...
	*/
	...
	if (file->details->display_name == NULL) {
		...
		if (g_utf8_validate (name, -1, NULL)) {
			peony_file_set_display_name (file,
							name,
							NULL,
							FALSE);
		} else {
...
char *
peony_file_get_display_name (PeonyFile *file)
{
	return g_strdup (peony_file_peek_display_name (file));
}

char *
peony_file_get_edit_name (PeonyFile *file)
{
	const char *res;
...
char *
peony_file_get_name (PeonyFile *file)
{
	return g_strdup (eel_ref_str_peek (file->details->name));
}

/**
 * peony_file_get_description:
 * @file: a #PeonyFile.
 *
 ...
 */
char *
peony_file_get_description (PeonyFile *file)
{
...
void
peony_file_monitor_remove (PeonyFile *file,
			      gconstpointer client)
{
	g_return_if_fail (PEONY_IS_FILE (file));
	g_return_if_fail (client != NULL);
...
gboolean
peony_file_is_launcher (PeonyFile *file)
{
	return file->details->is_launcher;
}

gboolean
peony_file_is_foreign_link (PeonyFile *file)
{
	return file->details->is_foreign_link;
}

gboolean
peony_file_is_trusted_link (PeonyFile *file)
{
	return file->details->is_trusted_link;
}

gboolean
peony_file_has_activation_uri (PeonyFile *file)
{
 ...
 * the actual uri if the file is an desktop file or a peony
 * xml link file.
 */
char *
peony_file_get_activation_uri (PeonyFile *file)
{
	g_return_val_if_fail (PEONY_IS_FILE (file), NULL);
...
GFile *
peony_file_get_activation_location (PeonyFile *file)
{
	g_return_val_if_fail (PEONY_IS_FILE (file), NULL);
	...
	return peony_file_get_location (file);
}


char *
peony_file_get_drop_target_uri (PeonyFile *file)
{
	char *uri, *target_uri;
	GFile *location;
	PeonyDesktopLink *link;
...
static char *
get_custom_icon_metadata_uri (PeonyFile *file)
{
	char *custom_icon_uri;
	char *uri;
	char *dir_uri;

	uri = peony_file_get_metadata (file, PEONY_METADATA_KEY_CUSTOM_ICON, NULL);
	if (uri != NULL &&
	    peony_file_is_directory (file) &&
	    is_uri_relative (uri)) {
		dir_uri = peony_file_get_uri (file);
...
static GIcon *
get_custom_icon (PeonyFile *file)
{
	char *custom_icon_uri;
	GFile *icon_file;
	GIcon *icon;
...
peony_file_get_filesystem_use_preview (PeonyFile *file)
{
...
gboolean
peony_file_should_show_thumbnail (PeonyFile *file)
{
	const char *mime_type;
	GFilesystemPreviewType use_preview;
	 ...
	 */
	if (peony_thumbnail_is_mimetype_limited_by_size (mime_type) &&
	    file->details->thumbnail_path == NULL &&
	    peony_file_get_size (file) > cached_thumbnail_limit) {
		return FALSE;
	}

	if (show_image_thumbs == PEONY_SPEED_TRADEOFF_ALWAYS) {
		if (use_preview == G_FILESYSTEM_PREVIEW_TYPE_NEVER) {
			return FALSE;
		} else {
			return TRUE;
		}
	} else if (show_image_thumbs == PEONY_SPEED_TRADEOFF_NEVER) {
		return FALSE;
	} else {
		if (use_preview == G_FILESYSTEM_PREVIEW_TYPE_NEVER) {
...
GIcon *
peony_file_get_gicon (PeonyFile *file,
...
//				mount_icon = g_mount_get_icon (mount);
		...
		if (((flags & PEONY_FILE_ICON_FLAGS_EMBEDDING_TEXT) ||
		     (flags & PEONY_FILE_ICON_FLAGS_FOR_DRAG_ACCEPT) ||
		     (flags & PEONY_FILE_ICON_FLAGS_FOR_OPEN_FOLDER) ||
		     (flags & PEONY_FILE_ICON_FLAGS_USE_MOUNT_ICON) ||
		     (flags & PEONY_FILE_ICON_FLAGS_USE_MOUNT_ICON_AS_EMBLEM) ||
		     ((flags & PEONY_FILE_ICON_FLAGS_IGNORE_VISITING) == 0 &&
		      peony_file_has_open_window (file))) &&
			...
			if (is_folder && (flags & PEONY_FILE_ICON_FLAGS_FOR_OPEN_FOLDER)) {
		...
		if ((flags & PEONY_FILE_ICON_FLAGS_USE_MOUNT_ICON) &&
			...
			g_object_unref (icon);
			icon = mount_icon;
		} else if ((flags & PEONY_FILE_ICON_FLAGS_USE_MOUNT_ICON_AS_EMBLEM) &&
...
static GIcon *
get_default_file_icon (PeonyFileIconFlags flags)
{
...
PeonyIconInfo *
peony_file_get_icon (PeonyFile *file,
			int size,
			PeonyFileIconFlags flags)
{
	PeonyIconInfo *icon;
	...
	if (gicon) {
		...
		icon = peony_icon_info_lookup (gicon, size);
		g_object_unref (gicon);

		pixbuf = peony_icon_info_get_pixbuf (icon);
		if (pixbuf != NULL) {
			if (!file->details->is_launcher && !gdk_pixbuf_get_has_alpha (pixbuf)) {
				peony_ui_frame_image (&pixbuf);
			}
			g_object_unref (icon);

			icon = peony_icon_info_new_for_pixbuf (pixbuf);
	...
	if (flags & PEONY_FILE_ICON_FLAGS_FORCE_THUMBNAIL_SIZE) {
		modified_size = size;
	} else {
		modified_size = size * cached_thumbnail_size / PEONY_ICON_SIZE_STANDARD;
	}
	if (flags & PEONY_FILE_ICON_FLAGS_USE_THUMBNAILS &&
	    peony_file_should_show_thumbnail (file)) {
		if (file->details->thumbnail) {
			int w, h, s;
			double scale;
			...
			if (s <= cached_thumbnail_size) {
				scale = (double)size / PEONY_ICON_SIZE_STANDARD;
			}
			else {
				scale = (double)modified_size / s;
			}
			/* Make sure that icons don't get smaller than PEONY_ICON_SIZE_SMALLEST */
			if (s*scale <= PEONY_ICON_SIZE_SMALLEST) {
				scale = (double) PEONY_ICON_SIZE_SMALLEST / s;
			}
...
//								 MAX (size - 6, 1),
								 (MAX (w * scale, 1)*0.8),
								 (MAX ((h * scale)>48?(h*scale):(h*scale), 1)*0.8),
								 GDK_INTERP_BILINEAR);
			...
			gboolean is_image = file->details->mime_type &&
				...
				if (!is_image ||
					...
					peony_ui_frame_image (&scaled_pixbuf);
				}
			...
			if (modified_size > 128*1.25 &&
			    ...
			    peony_can_thumbnail_internally (file)) {
				/* Invalidate if we resize upward */
				file->details->thumbnail_wants_original = TRUE;
				peony_file_invalidate_attributes (file, PEONY_FILE_ATTRIBUTE_THUMBNAIL);
			}

			icon = peony_icon_info_new_for_pixbuf (scaled_pixbuf);
			...
			if (peony_can_thumbnail (file)) {
				peony_create_thumbnail (file);
			}
		}
	}
	...
	if (gicon) {
		icon = peony_icon_info_lookup (gicon, size);
		if (peony_icon_info_is_fallback (icon)) {
			g_object_unref (icon);
			icon = peony_icon_info_lookup (get_default_file_icon (flags), size);
		}
		g_object_unref (gicon);
		return icon;
	} else {
		return peony_icon_info_lookup (get_default_file_icon (flags), size);
	}
}

GdkPixbuf *
peony_file_get_icon_pixbuf (PeonyFile *file,
			       int size,
			       gboolean force_size,
			       PeonyFileIconFlags flags)
{
	...
	info = peony_file_get_icon (file, size, flags);
	if (force_size) {
		pixbuf =  peony_icon_info_get_pixbuf_at_size (info, size);
	} else {
		pixbuf = peony_icon_info_get_pixbuf (info);
	}
	g_object_unref (info);
...
char *
peony_file_get_custom_icon (PeonyFile *file)
{
	char *custom_icon;
...
gboolean
peony_file_get_date (PeonyFile *file,
			...
			time_t *date)
{
	...
	g_return_val_if_fail (date_type == PEONY_DATE_TYPE_CHANGED
			      || date_type == PEONY_DATE_TYPE_ACCESSED
			      || date_type == PEONY_DATE_TYPE_MODIFIED
			      || date_type == PEONY_DATE_TYPE_TRASHED
			      || date_type == PEONY_DATE_TYPE_PERMISSIONS_CHANGED, FALSE);
...
static char *
peony_file_get_where_string (PeonyFile *file)
{
	if (file == NULL) {
		return NULL;
	}
...
static const char *TODAY_TIME_FORMATS [] = {
	 ...
	 * in the Peony version of strftime that can be
...
static char *
peony_file_fit_date_as_string (PeonyFile *file,
				  PeonyDateType date_type,
				  int width,
				  ...
				  void *measure_context)
{
	...
	if (!peony_file_get_date (file, date_type, &file_time_raw)) {
		return NULL;
	}

	file_time = localtime (&file_time_raw);

	if (date_format_pref == PEONY_DATE_FORMAT_LOCALE) {
		return eel_strdup_strftime ("%c", file_time);
	} else if (date_format_pref == PEONY_DATE_FORMAT_ISO) {
...
/**
 * peony_file_fit_modified_date_as_string:
 *
 ...
 * @file: PeonyFile representing the file in question.
 ...
 **/
char *
peony_file_fit_modified_date_as_string (PeonyFile *file,
					   int width,
					   PeonyWidthMeasureCallback measure_callback,
					   PeonyTruncateCallback truncate_callback,
					   void *measure_context)
{
	return peony_file_fit_date_as_string (file, PEONY_DATE_TYPE_MODIFIED,
		width, measure_callback, truncate_callback, measure_context);
}

static char *
peony_file_get_trash_original_file_parent_as_string (PeonyFile *file)
{
	PeonyFile *orig_file, *parent;
	GFile *location;
	char *filename;

	if (file->details->trash_orig_path != NULL) {
		orig_file = peony_file_get_trash_original_file (file);
		parent = peony_file_get_parent (orig_file);
		location = peony_file_get_location (parent);

		filename = g_file_get_parse_name (location);

		g_object_unref (location);
		peony_file_unref (parent);
		peony_file_unref (orig_file);
 ...
 **/
static char *
peony_file_get_date_as_string (PeonyFile *file, PeonyDateType date_type)
{
	return peony_file_fit_date_as_string (file, date_type,
		0, NULL, NULL, NULL);
}

static PeonySpeedTradeoffValue show_directory_item_count;
static PeonySpeedTradeoffValue show_text_in_icons;

static void
show_text_in_icons_changed_callback (gpointer callback_data)
{
	show_text_in_icons = g_settings_get_enum (peony_preferences, PEONY_PREFERENCES_SHOW_TEXT_IN_ICONS);
}

static void
show_directory_item_count_changed_callback (gpointer callback_data)
{
	show_directory_item_count = g_settings_get_enum (peony_preferences, PEONY_PREFERENCES_SHOW_DIRECTORY_ITEM_COUNTS);
}

static gboolean
get_speed_tradeoff_preference_for_file (PeonyFile *file, PeonySpeedTradeoffValue value)
{
	GFilesystemPreviewType use_preview;
	...
	if (value == PEONY_SPEED_TRADEOFF_ALWAYS) {
		if (use_preview == G_FILESYSTEM_PREVIEW_TYPE_NEVER) {
			return FALSE;
		} else {
			return TRUE;
	...
	if (value == PEONY_SPEED_TRADEOFF_NEVER) {
		return FALSE;
	}

	g_assert (value == PEONY_SPEED_TRADEOFF_LOCAL_ONLY);
...
gboolean
peony_file_should_show_directory_item_count (PeonyFile *file)
{
...
gboolean
peony_file_should_show_type (PeonyFile *file)
{
	char *uri;
	gboolean ret;
...
gboolean
peony_file_should_get_top_left_text (PeonyFile *file)
{
	...
	if (!show_text_in_icons_callback_added) {
		g_signal_connect_swapped (peony_preferences,
								  "changed::" PEONY_PREFERENCES_SHOW_TEXT_IN_ICONS,
	...
	if (show_text_in_icons == PEONY_SPEED_TRADEOFF_ALWAYS) {
		return TRUE;
	}

	if (show_text_in_icons == PEONY_SPEED_TRADEOFF_NEVER) {
		return FALSE;
	}
 ...
 * @file: PeonyFile representing a directory.
 ...
 **/
gboolean
peony_file_get_directory_item_count (PeonyFile *file,
					guint *count,
					gboolean *count_unreadable)
{
	...
	if (!peony_file_should_show_directory_item_count (file)) {
		return FALSE;
	}
...
/**
 * peony_file_get_deep_counts
 *
 * Get the statistics about items inside a directory.
 * @file: PeonyFile representing a directory or file.
 ...
 * peony_file_should_show_directory_item_count returns FALSE
 * for this file.
 *
 ...
 **/
PeonyRequestStatus
peony_file_get_deep_counts (PeonyFile *file,
			       ...
			       gboolean force)
{
	...
	g_return_val_if_fail (PEONY_IS_FILE (file), PEONY_REQUEST_DONE);

	if (!force && !peony_file_should_show_directory_item_count (file)) {
		/* Set field so an existing value isn't treated as up-to-date
		 * when preference changes later.
		 */
		file->details->deep_counts_status = PEONY_REQUEST_NOT_STARTED;
...
void
peony_file_recompute_deep_counts (PeonyFile *file)
{
	if (file->details->deep_counts_status != PEONY_REQUEST_IN_PROGRESS) {
		file->details->deep_counts_status = PEONY_REQUEST_NOT_STARTED;
		if (file->details->directory != NULL) {
			peony_directory_add_file_to_work_queue (file->details->directory, file);
			peony_directory_async_state_changed (file->details->directory);
		}
	}
}

gboolean
peony_file_can_get_size (PeonyFile *file)
{
 ...
 **/
goffset
peony_file_get_size (PeonyFile *file)
{
...
time_t
peony_file_get_mtime (PeonyFile *file)
{
				  ...
				  gpointer callback_data)
{
	...
	new_info = g_file_query_info_finish (G_FILE (source_object), res, &error);
	if (new_info != NULL) {
		if (peony_file_update_info (op->file, new_info)) {
			peony_file_changed (op->file);
		}
		g_object_unref (new_info);
	}
...
void
peony_file_set_attributes (PeonyFile *file,
...
/**
 * peony_file_can_get_permissions:
 *
 ...
 */
gboolean
peony_file_can_get_permissions (PeonyFile *file)
{
	return file->details->has_permissions;
}

/**
 * peony_file_can_set_permissions:
 *
 ...
 */
gboolean
peony_file_can_set_permissions (PeonyFile *file)
{
	uid_t user_id;

	if (file->details->uid != -1 &&
	    peony_file_is_local (file)) {
		/* Check the user. */
		user_id = geteuid();
...
guint
peony_file_get_permissions (PeonyFile *file)
{
	g_return_val_if_fail (peony_file_can_get_permissions (file), 0);
 ...
 * peony_file_can_set_permissions returned TRUE.
 *
 * @file: PeonyFile representing the file in question.
 ...
 **/
void
peony_file_set_permissions (PeonyFile *file,
			       ...
			       gpointer callback_data)
{
	...
	// Start UNDO-REDO
	if (!peony_undostack_manager_is_undo_redo(peony_undostack_manager_instance())) {
		PeonyUndoStackActionData* undo_redo_data = peony_undostack_manager_data_new (PEONY_UNDOSTACK_SETPERMISSIONS, 1);
		peony_undostack_manager_data_set_file_permissions(undo_redo_data, peony_file_get_uri(file), file->details->permissions, new_permissions);
		peony_undostack_manager_add_action (peony_undostack_manager_instance(),
										   undo_redo_data);
	}
	// End UNDO-REDO
	...
	peony_file_set_attributes (file, info, callback, callback_data);
	g_object_unref (info);
}

/**
 * peony_file_can_get_selinux_context:
 *
 ...
 */
gboolean
peony_file_can_get_selinux_context (PeonyFile *file)
{
...
/**
 * peony_file_get_selinux_context:
 *
 ...
 **/
char *
peony_file_get_selinux_context (PeonyFile *file)
{
	char *translated;
	char *raw;
	...
	if (!peony_file_can_get_selinux_context (file)) {
		return NULL;
	}
 ...
 */
gboolean
peony_file_can_get_owner (PeonyFile *file)
{
 ...
 */
char *
peony_file_get_owner_name (PeonyFile *file)
{
	return peony_file_get_owner_as_string (file, FALSE);
}

/**
 * peony_file_can_set_owner:
 *
 * Check whether the current user is allowed to change
 * the owner of a file.
 *
 ...
 * peony_file_can_set_owner returns TRUE.
 *
 * @file: The file in question.
 ...
 */
void
peony_file_set_owner (PeonyFile *file,
			 ...
			 gpointer callback_data)
{
	...
	if (!peony_file_can_set_owner (file)) {
    ...
    // Start UNDO-REDO
	if (!peony_undostack_manager_is_undo_redo(peony_undostack_manager_instance())) {
		char* current_owner = peony_file_get_owner_as_string (file, FALSE);
		PeonyUndoStackActionData* undo_redo_data = peony_undostack_manager_data_new (PEONY_UNDOSTACK_CHANGEOWNER, 1);
		peony_undostack_manager_data_set_owner_change_information(undo_redo_data, peony_file_get_uri(file), current_owner, user_name_or_id);
		peony_undostack_manager_add_action (peony_undostack_manager_instance(),
										   undo_redo_data);
 ...
 */
gboolean
peony_file_can_get_group (PeonyFile *file)
{
 ...
 **/
char *
peony_file_get_group_name (PeonyFile *file)
{
	return g_strdup (eel_ref_str_peek (file->details->group));
}

/**
 * peony_file_can_set_group:
 *
 * Check whether the current user is allowed to change
 * the group of a file.
 *
 ...
 */
gboolean
peony_file_can_set_group (PeonyFile *file)
{
	uid_t user_id;
	 ...
	 * systems.
	 */
	if (!peony_file_can_get_group (file)) {
		return FALSE;
	}
 ...
 */
static GList *
peony_get_group_names_for_user (void)
{
	GList *list;
	struct group *group;
	int count, i;
...
/**
 * peony_get_group_names:
 *
 * Get a list of all group names.
 */
GList *
peony_get_all_group_names (void)
{
	GList *list;
	struct group *group;
...
/**
 * peony_file_get_settable_group_names:
 *
 * Get a list of all group names that the current user
 * can set the group of a specific file to.
 *
 * @file: The PeonyFile in question.
 */
GList *
peony_file_get_settable_group_names (PeonyFile *file)
{
	uid_t user_id;
	GList *result;

	if (!peony_file_can_set_group (file)) {
		return NULL;
	}
	...
	if (user_id == 0) {
		...
		result = peony_get_all_group_names ();
	} else if (user_id == (uid_t) file->details->uid) {
		/* Owner is allowed to set group to any that owner is member of. */
		result = peony_get_group_names_for_user ();
	} else {
		g_warning ("unhandled case in peony_get_settable_group_names");
		result = NULL;
	}
 ...
 * peony_file_can_set_group returns TRUE.
 *
 * @file: The file in question.
 ...
 */
void
peony_file_set_group (PeonyFile *file,
			 ...
			 gpointer callback_data)
{
	...
	// Start UNDO-REDO
	if (!peony_undostack_manager_is_undo_redo(peony_undostack_manager_instance())) {
		char* current_group = peony_file_get_group_name (file);
		PeonyUndoStackActionData* undo_redo_data = peony_undostack_manager_data_new (PEONY_UNDOSTACK_CHANGEGROUP, 1);
		peony_undostack_manager_data_set_group_change_information(undo_redo_data, peony_file_get_uri(file), current_group, group_name_or_id);
		peony_undostack_manager_add_action (peony_undostack_manager_instance(),
											undo_redo_data);
...
/**
 * peony_file_get_octal_permissions_as_string:
 *
 ...
 **/
static char *
peony_file_get_octal_permissions_as_string (PeonyFile *file)
{
	guint32 permissions;
...
/**
 * peony_file_get_permissions_as_string:
 *
 ...
 **/
static char *
peony_file_get_permissions_as_string (PeonyFile *file)
{
	guint32 permissions;
	...
	permissions = file->details->permissions;
	is_directory = peony_file_is_directory (file);
	is_link = peony_file_is_symbolic_link (file);
...
/**
 * peony_file_get_owner_as_string:
 *
 ...
 **/
static char *
peony_file_get_owner_as_string (PeonyFile *file, gboolean include_real_name)
{
	char *user_name;
 ...
 **/
static char *
peony_file_get_size_as_string (PeonyFile *file)
{
	guint item_count;
	gboolean count_unreadable;
	...
	if (peony_file_is_directory (file)) {
		if (!peony_file_get_directory_item_count (file, &item_count, &count_unreadable)) {
			return NULL;
		}
...
/**
 * peony_file_get_size_as_string_with_real_size:
 *
 * Get a user-displayable string representing a file size. The caller
 * is responsible for g_free-ing this string. The string is an item
 * count for directories.
 ...
 **/
static char *
peony_file_get_size_as_string_with_real_size (PeonyFile *file)
{
	guint item_count;
	gboolean count_unreadable;
	char * formatted;
...
static char *
peony_file_get_deep_count_as_string_internal (PeonyFile *file,
						 ...
						 gboolean report_file_count)
{
	...
	g_assert (PEONY_IS_FILE (file));
	g_assert (peony_file_is_directory (file));
	...
	if (status == PEONY_REQUEST_NOT_STARTED) {
		return NULL;
	}
	...
	if (total_count == 0) {
		switch (status) {
		case PEONY_REQUEST_IN_PROGRESS:
	 ...
	 * Callers can distinguish them using peony_file_get_deep_counts
	 * directly if desired.
	 */
	if (report_size)
	{
...
/**
 * peony_file_get_deep_size_as_string:
 *
 ...
 **/
static char *
peony_file_get_deep_size_as_string (PeonyFile *file)
{
	return peony_file_get_deep_count_as_string_internal (file, TRUE, FALSE, FALSE);
}

/**
 * peony_file_get_deep_total_count_as_string:
 *
 ...
 **/
static char *
peony_file_get_deep_total_count_as_string (PeonyFile *file)
{
	return peony_file_get_deep_count_as_string_internal (file, FALSE, TRUE, TRUE);
}

/**
 * peony_file_get_deep_file_count_as_string:
 *
 * Get a user-displayable string representing the count of all contained
 ...
 **/
static char *
peony_file_get_deep_file_count_as_string (PeonyFile *file)
{
	return peony_file_get_deep_count_as_string_internal (file, FALSE, FALSE, TRUE);
}

/**
 * peony_file_get_deep_directory_count_as_string:
 *
 * Get a user-displayable string representing the count of all contained
 ...
 **/
static char *
peony_file_get_deep_directory_count_as_string (PeonyFile *file)
{
	return peony_file_get_deep_count_as_string_internal (file, FALSE, TRUE, FALSE);
}

/**
 * peony_file_get_string_attribute:
 *
 ...
 * peony_file_get_string_attribute_with_default if you want a non-NULL
 * default.
 *
 * @file: PeonyFile representing the file in question.
...
peony_file_get_string_attribute_q (PeonyFile *file, GQuark attribute_q)
{
	...
	if (attribute_q == attribute_type_q) {
		return peony_file_get_type_as_string (file);
	}
	if (attribute_q == attribute_mime_type_q) {
		return peony_file_get_mime_type (file);
	}
	if (attribute_q == attribute_size_q) {
		return peony_file_get_size_as_string (file);
	}
	if (attribute_q == attribute_size_detail_q) {
		return peony_file_get_size_as_string_with_real_size (file);
	}
	if (attribute_q == attribute_deep_size_q) {
		return peony_file_get_deep_size_as_string (file);
	}
	if (attribute_q == attribute_deep_file_count_q) {
		return peony_file_get_deep_file_count_as_string (file);
	}
	if (attribute_q == attribute_deep_directory_count_q) {
		return peony_file_get_deep_directory_count_as_string (file);
	}
	if (attribute_q == attribute_deep_total_count_q) {
		return peony_file_get_deep_total_count_as_string (file);
	}
	if (attribute_q == attribute_trash_orig_path_q) {
		return peony_file_get_trash_original_file_parent_as_string (file);
	}
	if (attribute_q == attribute_date_modified_q) {
		return peony_file_get_date_as_string (file,
	...
	if (attribute_q == attribute_permissions_q) {
		return peony_file_get_permissions_as_string (file);
	}
	if (attribute_q == attribute_selinux_context_q) {
		return peony_file_get_selinux_context (file);
	}
	if (attribute_q == attribute_octal_permissions_q) {
		return peony_file_get_octal_permissions_as_string (file);
	}
	if (attribute_q == attribute_owner_q) {
		return peony_file_get_owner_as_string (file, TRUE);
	}
	if (attribute_q == attribute_group_q) {
		return peony_file_get_group_name (file);
	}
	if (attribute_q == attribute_uri_q) {
		return peony_file_get_uri (file);
	}
	if (attribute_q == attribute_where_q) {
		return peony_file_get_where_string (file);
	}
	if (attribute_q == attribute_link_target_q) {
		return peony_file_get_symbolic_link_target_path (file);
	}
	if (attribute_q == attribute_volume_q) {
		return peony_file_get_volume_name (file);
	}
	if (attribute_q == attribute_free_space_q) {
		return peony_file_get_volume_free_space (file);
	}
...
char *
peony_file_get_string_attribute (PeonyFile *file, const char *attribute_name)
{
	return peony_file_get_string_attribute_q (file, g_quark_from_string (attribute_name));
}


/**
 * peony_file_get_string_attribute_with_default:
 *
 * Get a user-displayable string from a named attribute. Use g_free to
 ...
 * peony_file_get_string_attribute if you want NULL instead of a default
 * result.
 *
 * @file: PeonyFile representing the file in question.
 * @attribute_name: The name of the desired attribute. See the description of
 * peony_file_get_string for the set of available attributes.
 *
 ...
 **/
char *
peony_file_get_string_attribute_with_default_q (PeonyFile *file, GQuark attribute_q)
{
	char *result;
	guint item_count;
	gboolean count_unreadable;
	...
	result = peony_file_get_string_attribute_q (file, attribute_q);
	if (result != NULL) {
		return result;
	}
	...
	if (attribute_q == attribute_size_q) {
		...
		count_unreadable = FALSE;
		if (peony_file_is_directory (file)) {
			peony_file_get_directory_item_count (file, &item_count, &count_unreadable);
		}
		return g_strdup (count_unreadable ? _("? items") : "...");
	}
	if (attribute_q == attribute_deep_size_q) {
		status = peony_file_get_deep_counts (file, NULL, NULL, NULL, NULL, FALSE);
...
char *
peony_file_get_string_attribute_with_default (PeonyFile *file, const char *attribute_name)
{
	return peony_file_get_string_attribute_with_default_q (file, g_quark_from_string (attribute_name));
}

gboolean
peony_file_is_date_sort_attribute_q (GQuark attribute_q)
{
 ...
 **/
static char *
get_description (PeonyFile *file)
{
	const char *mime_type;
	char *description;
...
static char *
update_description_for_link (PeonyFile *file, char *string)
{
	char *res;

	if (peony_file_is_symbolic_link (file)) {
		g_assert (!peony_file_is_broken_symbolic_link (file));
		if (string == NULL) {
...
static char *
peony_file_get_type_as_string (PeonyFile *file)
{
	if (file == NULL) {
		return NULL;
	}

	if (peony_file_is_broken_symbolic_link (file)) {
 ...
 **/
char *
peony_file_get_mime_type (PeonyFile *file)
{
	if (file != NULL) {
		g_return_val_if_fail (PEONY_IS_FILE (file), NULL);
 ...
 **/
gboolean
peony_file_is_mime_type (PeonyFile *file, const char *mime_type)
{
	g_return_val_if_fail (PEONY_IS_FILE (file), FALSE);
...
gboolean
peony_file_is_launchable (PeonyFile *file)
{
 ...
 **/
GList *
peony_file_get_emblem_icons (PeonyFile *file,
				char **exclude)
{
	GList *keywords, *l;
	GList *icons;
...
#ifdef TRASH_IS_FAST_ENOUGH
		if (strcmp (keyword, PEONY_FILE_EMBLEM_NAME_TRASH) == 0) {
			char *uri;
...
GList *
peony_file_get_emblem_pixbufs (PeonyFile *file,
				  int size,
				  gboolean force_size,
				  char **exclude)
{
	...
	icons = peony_file_get_emblem_icons (file, exclude);
	pixbufs = NULL;

	for (l = icons; l != NULL; l = l->next) {
		icon = l->data;

		icon_info = peony_icon_info_lookup (icon, size);
		if (force_size) {
			pixbuf = peony_icon_info_get_pixbuf_nodefault_at_size (icon_info, size);
		} else {
			pixbuf = peony_icon_info_get_pixbuf_nodefault (icon_info);
		}
 ...
 **/
GList *
peony_file_get_keywords (PeonyFile *file)
{
	GList *keywords;
	...
	keywords = peony_file_get_metadata_list
		(file, PEONY_METADATA_KEY_EMBLEMS);
 ...
 **/
void
peony_file_set_keywords (PeonyFile *file, GList *keywords)
{
	...
	peony_file_set_metadata_list
		(file, PEONY_METADATA_KEY_EMBLEMS, canonical_keywords);
	g_list_free (canonical_keywords);
}

/**
 * peony_file_is_symbolic_link
 *
 ...
 **/
gboolean
peony_file_is_symbolic_link (PeonyFile *file)
{
	return file->details->is_symlink;
}

gboolean
peony_file_is_mountpoint (PeonyFile *file)
{
	return file->details->is_mountpoint;
}

GMount *
peony_file_get_mount (PeonyFile *file)
{
	if (file->details->mount) {
		      ...
		      gpointer data)
{
	...
	peony_file_invalidate_attributes (file, PEONY_FILE_ATTRIBUTE_MOUNT);
}

void
peony_file_set_mount (PeonyFile *file,
			 GMount *mount)
{
	if (file->details->mount) {
		g_signal_handlers_disconnect_by_func (file->details->mount, file_mount_unmounted, file);
...
/**
 * peony_file_is_broken_symbolic_link
 *
 ...
 **/
gboolean
peony_file_is_broken_symbolic_link (PeonyFile *file)
{
	if (file == NULL) {
		return FALSE;
	}
	...
	return peony_file_get_file_type (file) == G_FILE_TYPE_SYMBOLIC_LINK;
}
		...
		gpointer user_data)
{
	...
	directory = PEONY_DIRECTORY (user_data);
...
/**
 * peony_file_get_volume_free_space
 ...
 */
char *
peony_file_get_volume_free_space (PeonyFile *file)
{
	PeonyDirectory *directory;
	GFile *location;
	char *res;
	...
	directory = peony_directory_get_for_file (file);
 ...
 */
char *
peony_file_get_volume_name (PeonyFile *file)
{
	GFile *location;
	char *res;
	GMount *mount;
 ...
 * @file: PeonyFile representing the symbolic link in question.
 *
 * Returns: newly-allocated copy of the file path of the target of the symbolic link.
 */
char *
peony_file_get_symbolic_link_target_path (PeonyFile *file)
{
	if (!peony_file_is_symbolic_link (file)) {
...
/**
 * peony_file_get_symbolic_link_target_uri
 *
 ...
 */
char *
peony_file_get_symbolic_link_target_uri (PeonyFile *file)
{
...
/**
 * peony_file_is_peony_link
 *
 * Check if this file is a "peony link", meaning a historical
 * peony xml link file or a desktop file.
 * @file: PeonyFile representing the file in question.
 *
 * Returns: True if the file is a peony link.
 *
 **/
gboolean
 ...
 **/
gboolean
peony_file_is_directory (PeonyFile *file)
{
	return peony_file_get_file_type (file) == G_FILE_TYPE_DIRECTORY;
}

/**
 * peony_file_is_user_special_directory
 *
 ...
 */
gboolean
peony_file_is_user_special_directory (PeonyFile *file,
...
gboolean
peony_file_is_archive (PeonyFile *file)
{
	char *mime_type;
	int i;
 ...
 **/
gboolean
peony_file_is_in_trash (PeonyFile *file)
{
	g_assert (PEONY_IS_FILE (file));

	return peony_directory_is_in_trash (file->details->directory);
}

GError *
peony_file_get_file_info_error (PeonyFile *file)
{
	if (!file->details->get_info_failed) {
		return NULL;
	}
 ...
 **/
gboolean
peony_file_contains_text (PeonyFile *file)
{
	if (file == NULL) {
		return FALSE;
	}
	...
	return peony_file_is_mime_type (file, "text/plain");
}

/**
 * peony_file_is_binary
 *
 ...
 **/
gboolean
peony_file_is_binary (PeonyFile *file)
{
	if (!peony_file_can_read(file))
	{
		return FALSE;
	...
	fp = g_fopen (g_file_get_path (peony_file_get_location (file)), "r");
	if (fp == NULL)
	{
		return FALSE;
	}
 ...
 **/
gboolean
peony_file_is_executable (PeonyFile *file)
{
 ...
 **/
char *
peony_file_peek_top_left_text (PeonyFile *file,
				  gboolean  need_large_text,
				  gboolean *needs_loading)
{
	g_return_val_if_fail (PEONY_IS_FILE (file), NULL);

	if (!peony_file_should_get_top_left_text (file)) {
		if (needs_loading) {
			*needs_loading = FALSE;
		}
		return NULL;
	...
	if (!file->details->got_top_left_text) {
		...
		if (peony_file_contains_text (file)) {
			return " ...";
		}
		return NULL;
	}
 ...
 **/
char *
peony_file_get_top_left_text (PeonyFile *file)
{
	return g_strdup (peony_file_peek_top_left_text (file, FALSE, NULL));
}

char *
peony_file_get_filesystem_id (PeonyFile *file)
{
	return g_strdup (eel_ref_str_peek (file->details->filesystem_id));
}

PeonyFile *
peony_file_get_trash_original_file (PeonyFile *file)
{
	GFile *location;
	...
	if (file->details->trash_orig_path != NULL) {
		...
		original_file = peony_file_get (location);
		g_object_unref (location);
	}
 ...
 **/
void
peony_file_changed (PeonyFile *file)
{
	GList fake_list;
	...
	if (peony_file_is_self_owned (file)) {
		...
		fake_list.next = NULL;
		fake_list.prev = NULL;
		peony_directory_emit_change_signals
			(file->details->directory, &fake_list);
	}
}

/**
 * peony_file_updated_deep_count_in_progress
 *
 * Notify clients that a newer deep count is available for
 * the directory in question.
 */
void
peony_file_updated_deep_count_in_progress (PeonyFile *file) {
	...
	for (node = link_files; node != NULL; node = node->next) {
		peony_file_updated_deep_count_in_progress (PEONY_FILE (node->data));
	}
	peony_file_list_free (link_files);
}

/**
 * peony_file_emit_changed
 *
 ...
 **/
void
peony_file_emit_changed (PeonyFile *file)
{
	GList *link_files, *p;
	...
	for (p = link_files; p != NULL; p = p->next) {
		if (p->data != file) {
			peony_file_changed (PEONY_FILE (p->data));
		}
	}
	peony_file_list_free (link_files);
}
 ...
 **/
gboolean
peony_file_is_gone (PeonyFile *file)
{
	g_return_val_if_fail (PEONY_IS_FILE (file), FALSE);
...
/**
 * peony_file_is_not_yet_confirmed
 *
 ...
 **/
gboolean
peony_file_is_not_yet_confirmed (PeonyFile *file)
{
	g_return_val_if_fail (PEONY_IS_FILE (file), FALSE);
 ...
 */
gboolean
peony_file_check_if_ready (PeonyFile *file,
...
void
peony_file_cancel_call_when_ready (PeonyFile *file,
				      PeonyFileCallback callback,
				      gpointer callback_data)
{
	g_return_if_fail (callback != NULL);
...
static void
invalidate_directory_count (PeonyFile *file)
{
	file->details->directory_count_is_up_to_date = FALSE;
}

static void
invalidate_deep_counts (PeonyFile *file)
{
	file->details->deep_counts_status = PEONY_REQUEST_NOT_STARTED;
}

static void
invalidate_mime_list (PeonyFile *file)
{
	file->details->mime_list_is_up_to_date = FALSE;
}

static void
invalidate_top_left_text (PeonyFile *file)
{
	file->details->top_left_text_is_up_to_date = FALSE;
}

static void
invalidate_file_info (PeonyFile *file)
{
	file->details->file_info_is_up_to_date = FALSE;
}

static void
invalidate_link_info (PeonyFile *file)
{
	file->details->link_info_is_up_to_date = FALSE;
}

static void
invalidate_thumbnail (PeonyFile *file)
{
	file->details->thumbnail_is_up_to_date = FALSE;
}

static void
invalidate_mount (PeonyFile *file)
{
...
peony_file_invalidate_extension_info_internal (PeonyFile *file)
{
	...
	file->details->pending_info_providers =
		peony_extensions_get_for_type (PEONY_TYPE_INFO_PROVIDER);
}

void
peony_file_invalidate_attributes_internal (PeonyFile *file,
					      PeonyFileAttributes file_attributes)
{
	Request request;
	...
	request = peony_directory_set_up_request (file_attributes);
...
gboolean
peony_file_has_open_window (PeonyFile *file)
{
	return file->details->has_open_window;
}

void
peony_file_set_has_open_window (PeonyFile *file,
...
gboolean
peony_file_is_thumbnailing (PeonyFile *file)
{
	g_return_val_if_fail (PEONY_IS_FILE (file), FALSE);
...
void
peony_file_set_is_thumbnailing (PeonyFile *file,
...
void
peony_file_invalidate_attributes (PeonyFile *file,
				     PeonyFileAttributes file_attributes)
{
	/* Cancel possible in-progress loads of any of these attributes */
	peony_directory_cancel_loading_file_attributes (file->details->directory,
							   file,
							   file_attributes);

	/* Actually invalidate the values */
	peony_file_invalidate_attributes_internal (file, file_attributes);
...
PeonyFileAttributes
peony_file_get_all_attributes (void)
{
	return  PEONY_FILE_ATTRIBUTE_INFO |
		PEONY_FILE_ATTRIBUTE_LINK_INFO |
		PEONY_FILE_ATTRIBUTE_DEEP_COUNTS |
		PEONY_FILE_ATTRIBUTE_DIRECTORY_ITEM_COUNT |
		PEONY_FILE_ATTRIBUTE_DIRECTORY_ITEM_MIME_TYPES |
		PEONY_FILE_ATTRIBUTE_TOP_LEFT_TEXT |
		PEONY_FILE_ATTRIBUTE_LARGE_TOP_LEFT_TEXT |
		PEONY_FILE_ATTRIBUTE_EXTENSION_INFO |
		PEONY_FILE_ATTRIBUTE_THUMBNAIL |
		PEONY_FILE_ATTRIBUTE_MOUNT;
}

void
peony_file_invalidate_all_attributes (PeonyFile *file)
{
	PeonyFileAttributes all_attributes;

	all_attributes = peony_file_get_all_attributes ();
	peony_file_invalidate_attributes (file, all_attributes);
}
 ...
 **/
void
peony_file_dump (PeonyFile *file)
{
 ...
 **/
GList *
peony_file_list_ref (GList *list)
{
	g_list_foreach (list, (GFunc) peony_file_ref, NULL);
	return list;
}

/**
 * peony_file_list_unref
 *
 * Unref all the files in a list.
 * @list: GList of files.
 **/
void
peony_file_list_unref (GList *list)
{
	g_list_foreach (list, (GFunc) peony_file_unref, NULL);
}
 ...
 **/
void
peony_file_list_free (GList *list)
{
	peony_file_list_unref (list);
	g_list_free (list);
}

/**
 * peony_file_list_copy
 *
 * Copy the list of files, making a new ref of each,
 * @list: GList of files.
 **/
GList *
peony_file_list_copy (GList *list)
{
	return g_list_copy (peony_file_list_ref (list));
}
				      ...
				      gboolean *is_trash)
{
	...
	*is_trash = FALSE;
	...
	if (file && peony_file_is_directory (file)) {
		is_download_dir =
			peony_file_is_user_special_directory (file, G_USER_DIRECTORY_DOWNLOAD);
		is_desktop_dir =
			peony_file_is_user_special_directory (file, G_USER_DIRECTORY_DESKTOP);
		is_trash_dir =
			peony_file_is_in_trash (file);
...
const gchar *
peony_file_get_default_sort_attribute (PeonyFile *file,
					  gboolean *reversed)
{
	const gchar *retval;
	gboolean is_download, is_trash, res;
...
{
	return compare_by_display_name (PEONY_FILE (a), PEONY_FILE (b));
}

/**
 * peony_file_list_sort_by_display_name
 *
 * Sort the list of files by file name.
 * @list: GList of files.
 **/
GList *
peony_file_list_sort_by_display_name (GList *list)
{
...
file_list_ready_data_free (FileListReadyData *data)
{
	...
	if (l != NULL) {
		...
		peony_file_list_free (data->file_list);
			  ...
			  gpointer callback_data)
{
	...
	data->file_list = peony_file_list_copy (file_list);
...
static void
file_list_file_ready_callback (PeonyFile *file,
			       gpointer user_data)
{
	FileListReadyData *data;
...
void
peony_file_list_call_when_ready (GList *file_list,
				    PeonyFileAttributes attributes,
				    ...
				    gpointer callback_data)
{
	...
	if (handle) {
		*handle = (PeonyFileListHandle *) data;
	}
...
void
peony_file_list_cancel_call_when_ready (PeonyFileListHandle *handle)
{
	GList *l;
	PeonyFile *file;
	FileListReadyData *data;
...
char *
peony_extract_top_left_text (const char *text,
				gboolean large,
				int length)
{
        GString* buffer;
	...
	if (large) {
		max_bytes = PEONY_FILE_LARGE_TOP_LEFT_TEXT_MAXIMUM_BYTES;
		max_lines = PEONY_FILE_LARGE_TOP_LEFT_TEXT_MAXIMUM_LINES;
		max_cols = PEONY_FILE_LARGE_TOP_LEFT_TEXT_MAXIMUM_CHARACTERS_PER_LINE;
	} else {
		max_bytes = PEONY_FILE_TOP_LEFT_TEXT_MAXIMUM_BYTES;
		max_lines = PEONY_FILE_TOP_LEFT_TEXT_MAXIMUM_LINES;
		max_cols = PEONY_FILE_TOP_LEFT_TEXT_MAXIMUM_CHARACTERS_PER_LINE;
	}
...
{
	g_settings_get (peony_preferences,
					PEONY_PREFERENCES_IMAGE_FILE_THUMBNAIL_LIMIT,
...
{
	cached_thumbnail_size = g_settings_get_int (peony_icon_view_preferences, PEONY_PREFERENCES_ICON_VIEW_THUMBNAIL_SIZE);
...
{
	show_image_thumbs = g_settings_get_enum (peony_preferences, PEONY_PREFERENCES_SHOW_IMAGE_FILE_THUMBNAILS);
...
static void
peony_file_class_init (PeonyFileClass *class)
{
	GtkIconTheme *icon_theme;

	peony_file_info_getter = peony_file_get_internal;
	...
	G_OBJECT_CLASS (class)->constructor = peony_file_constructor;

	signals[CHANGED] =
		g_signal_new ("changed",
		              G_TYPE_FROM_CLASS (class),
		              G_SIGNAL_RUN_LAST,
		              G_STRUCT_OFFSET (PeonyFileClass, changed),
		              NULL, NULL,
	...
	signals[UPDATED_DEEP_COUNT_IN_PROGRESS] =
		g_signal_new ("updated_deep_count_in_progress",
		              G_TYPE_FROM_CLASS (class),
		              G_SIGNAL_RUN_LAST,
		              G_STRUCT_OFFSET (PeonyFileClass, updated_deep_count_in_progress),
		              NULL, NULL,
			      g_cclosure_marshal_VOID__VOID,
		              G_TYPE_NONE, 0);

	g_type_class_add_private (class, sizeof (PeonyFileDetails));

	eel_g_settings_add_auto_enum (peony_preferences,
				                  PEONY_PREFERENCES_DATE_FORMAT,
				                  &date_format_pref);

	thumbnail_limit_changed_callback (NULL);
	g_signal_connect_swapped (peony_preferences,
							  "changed::" PEONY_PREFERENCES_IMAGE_FILE_THUMBNAIL_LIMIT,
	...
	g_signal_connect_swapped (peony_icon_view_preferences,
							  "changed::" PEONY_PREFERENCES_ICON_VIEW_THUMBNAIL_SIZE,
...
static void
peony_file_add_emblem (PeonyFile *file,
...
static void
peony_file_add_string_attribute (PeonyFile *file,
...
static void
peony_file_invalidate_extension_info (PeonyFile *file)
{
	peony_file_invalidate_attributes (file, PEONY_FILE_ATTRIBUTE_EXTENSION_INFO);
}

void
peony_file_info_providers_done (PeonyFile *file)
{
	g_list_free_full (file->details->extension_emblems, g_free);
...
static void
peony_file_info_iface_init (PeonyFileInfoIface *iface)
{
	iface->is_gone = peony_file_is_gone;
	iface->get_name = peony_file_get_name;
	iface->get_file_type = peony_file_get_file_type;
	iface->get_location = peony_file_get_location;
	iface->get_uri = peony_file_get_uri;
	iface->get_parent_location = peony_file_get_parent_location;
	iface->get_parent_uri = peony_file_get_parent_uri;
	iface->get_parent_info = peony_file_get_parent;
	iface->get_mount = peony_file_get_mount;
	iface->get_uri_scheme = peony_file_get_uri_scheme;
	iface->get_activation_uri = peony_file_get_activation_uri;
	iface->get_mime_type = peony_file_get_mime_type;
	iface->is_mime_type = peony_file_is_mime_type;
	iface->is_directory = peony_file_is_directory;
	iface->can_write = peony_file_can_write;
	iface->add_emblem = peony_file_add_emblem;
	iface->get_string_attribute = peony_file_get_string_attribute;
	iface->add_string_attribute = peony_file_add_string_attribute;
	iface->invalidate_extension_info = peony_file_invalidate_extension_info;
}

#if !defined (PEONY_OMIT_SELF_CHECK)

void
peony_self_check_file (void)
{
	PeonyFile *file_1;
	PeonyFile *file_2;
	GList *list;
        ...
        EEL_CHECK_INTEGER_RESULT (peony_directory_number_outstanding (), 0);

	file_1 = peony_file_get_by_uri ("file:///home/");
	...
	EEL_CHECK_INTEGER_RESULT (G_OBJECT (file_1->details->directory)->ref_count, 1);
        EEL_CHECK_INTEGER_RESULT (peony_directory_number_outstanding (), 1);

	peony_file_unref (file_1);

        EEL_CHECK_INTEGER_RESULT (peony_directory_number_outstanding (), 0);

	file_1 = peony_file_get_by_uri ("file:///etc");
	file_2 = peony_file_get_by_uri ("file:///usr");
	...
	EEL_CHECK_STRING_RESULT (peony_file_get_name (file_1), "home");

	EEL_CHECK_BOOLEAN_RESULT (peony_file_get_by_uri ("file:///home/") == file_1, TRUE);
	peony_file_unref (file_1);
	 ...
	 * in real life */
	file_1 = peony_file_get_by_uri (":");
	EEL_CHECK_STRING_RESULT (peony_file_get_name (file_1), ":");
	peony_file_unref (file_1);
#endif

	file_1 = peony_file_get_by_uri ("eazel:");
	EEL_CHECK_STRING_RESULT (peony_file_get_name (file_1), "eazel");
	peony_file_unref (file_1);
	...
	EEL_CHECK_BOOLEAN_RESULT (peony_file_compare_for_sort (file_1, file_2, PEONY_FILE_SORT_BY_DISPLAY_NAME, FALSE, FALSE) < 0, TRUE);
	EEL_CHECK_BOOLEAN_RESULT (peony_file_compare_for_sort (file_1, file_2, PEONY_FILE_SORT_BY_DISPLAY_NAME, FALSE, TRUE) > 0, TRUE);
	EEL_CHECK_BOOLEAN_RESULT (peony_file_compare_for_sort (file_1, file_1, PEONY_FILE_SORT_BY_DISPLAY_NAME, FALSE, FALSE) == 0, TRUE);
	EEL_CHECK_BOOLEAN_RESULT (peony_file_compare_for_sort (file_1, file_1, PEONY_FILE_SORT_BY_DISPLAY_NAME, TRUE, FALSE) == 0, TRUE);
	EEL_CHECK_BOOLEAN_RESULT (peony_file_compare_for_sort (file_1, file_1, PEONY_FILE_SORT_BY_DISPLAY_NAME, FALSE, TRUE) == 0, TRUE);
	EEL_CHECK_BOOLEAN_RESULT (peony_file_compare_for_sort (file_1, file_1, PEONY_FILE_SORT_BY_DISPLAY_NAME, TRUE, TRUE) == 0, TRUE);

	peony_file_unref (file_1);
	peony_file_unref (file_2);
}
