...
/* peony-customization-data.c - functions to collect and load customization
   names and images */

#include <config.h>
#include "peony-customization-data.h"
...
static char *            get_file_path_for_mode              (const PeonyCustomizationData *data,
        const char *file_name);
static char*             format_name_for_display             (PeonyCustomizationData *data, const char *name);
...
gboolean
peony_customization_data_get_next_element_for_display (PeonyCustomizationData *data,
        char **emblem_name,
        ...
        char **label_out)
{
    ...
    if (data->current_file_list == NULL)
    {
        if (data->reading_mode == READ_PUBLIC_CUSTOMIZATIONS)
        {
            ...
            data->reading_mode = READ_PRIVATE_CUSTOMIZATIONS;
            data->current_file_list = data->private_file_list;
            return peony_customization_data_get_next_element_for_display (data,
                    emblem_name,
                    pixbuf_out,
                    label_out);
        }
    ...
    *emblem_name = g_strdup (g_file_info_get_name (current_file_info));
    ...
    if (strcmp (data->customization_name, "patterns") == 0 &&
            data->pattern_frame != NULL)
    {
        pixbuf = peony_customization_make_pattern_chit (orig_pixbuf, data->pattern_frame, FALSE, is_reset_image);
    }
    else
    {
...
gboolean
peony_customization_data_private_data_was_displayed (PeonyCustomizationData *data)
{
    return data->private_data_was_displayed;
}

void
peony_customization_data_destroy (PeonyCustomizationData *data)
{
...
{
    return g_build_filename (PEONY_DATADIR,
                             customization_name,
                             NULL);
}
...
static char *
get_file_path_for_mode (const PeonyCustomizationData *data,
                        const char *file_name)
{
...
GdkPixbuf*
peony_customization_make_pattern_chit (GdkPixbuf *pattern_tile, GdkPixbuf *frame, gboolean dragging, gboolean is_reset)
{
...
static char*
format_name_for_display (PeonyCustomizationData *data, const char* name)
{
 ...
 */
static void
load_name_map_hash_table (PeonyCustomizationData *data)
{
    char *xml_path;
    ...
    xml_path = g_build_filename (PEONY_DATADIR, "browser.xml", NULL);
    if (xml_path)
    {
