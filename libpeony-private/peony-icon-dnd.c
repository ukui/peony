...
/* peony-icon-dnd.c - Drag & drop handling for the icon container widget.
...
static const GtkTargetEntry drop_types [] =
{
    ...
    { PEONY_ICON_DND_NETSCAPE_URL_TYPE, 0, PEONY_ICON_DND_NETSCAPE_URL },
    { PEONY_ICON_DND_URI_LIST_TYPE, 0, PEONY_ICON_DND_URI_LIST },
    { PEONY_ICON_DND_COLOR_TYPE, 0, PEONY_ICON_DND_COLOR },
    { PEONY_ICON_DND_BGIMAGE_TYPE, 0, PEONY_ICON_DND_BGIMAGE },
    { PEONY_ICON_DND_KEYWORD_TYPE, 0, PEONY_ICON_DND_KEYWORD },
    { PEONY_ICON_DND_RESET_BACKGROUND_TYPE,  0, PEONY_ICON_DND_RESET_BACKGROUND },
    { PEONY_ICON_DND_XDNDDIRECTSAVE_TYPE, 0, PEONY_ICON_DND_XDNDDIRECTSAVE }, /* XDS Protocol Type */
    { PEONY_ICON_DND_RAW_TYPE, 0, PEONY_ICON_DND_RAW },
    /* Must be last: */
    { PEONY_ICON_DND_ROOTWINDOW_DROP_TYPE,  0, PEONY_ICON_DND_ROOTWINDOW_DROP }
};
...
static char * peony_icon_container_find_drop_target (PeonyIconContainer *container,
        GdkDragContext *context,
        int x, int y, gboolean *icon_hit,
        gboolean rewrite_desktop);

static EelCanvasItem *
create_selection_shadow (PeonyIconContainer *container,
                         GList *list)
{
...
typedef struct
{
    ...
    PeonyDragEachSelectedItemDataGet iteratee;
...
static gboolean
icon_get_data_binder (PeonyIcon *icon, gpointer data)
{
    ...
    g_assert (PEONY_IS_ICON_CONTAINER (context->iterator_context));

    container = PEONY_ICON_CONTAINER (context->iterator_context);
    ...
    uri = peony_icon_container_get_icon_uri (container, icon);
    if (uri == NULL)
    {
...
/* Iterate over each selected icon in a PeonyIconContainer,
 * calling each_function on each.
 */
static void
peony_icon_container_each_selected_icon (PeonyIconContainer *container,
                                        gboolean (*each_function) (PeonyIcon *, gpointer), gpointer data)
{
    GList *p;
    PeonyIcon *icon;
...
/* Adaptor function used with peony_icon_container_each_selected_icon
 * to help iterate over all selected items, passing uris, x, y, w and h
 * values to the iteratee
 */
static void
each_icon_get_data_binder (PeonyDragEachSelectedItemDataGet iteratee,
                           gpointer iterator_context, gpointer data)
{
    IconGetDataBinderContext context;
    PeonyIconContainer *container;

    g_assert (PEONY_IS_ICON_CONTAINER (iterator_context));
    container = PEONY_ICON_CONTAINER (iterator_context);

    context.iterator_context = iterator_context;
    context.iteratee = iteratee;
    context.iteratee_data = data;
    peony_icon_container_each_selected_icon (container, icon_get_data_binder, &context);
}
                        ...
                        gpointer data)
{
    ...
    g_assert (PEONY_IS_ICON_CONTAINER (widget));
    g_return_if_fail (context != NULL);

    /* Call common function from peony-drag that set's up
     * the selection data in the right format. Pass it means to
     * iterate all the selected icons.
     */
    peony_drag_drag_data_get (widget, context, selection_data,
...
static void
peony_icon_container_position_shadow (PeonyIconContainer *container,
                                     int x, int y)
{
...
static void
peony_icon_container_dropped_icon_feedback (GtkWidget *widget,
        GtkSelectionData *data,
        int x, int y)
{
    PeonyIconContainer *container;
    PeonyIconDndInfo *dnd_info;

    container = PEONY_ICON_CONTAINER (widget);
    dnd_info = container->details->dnd_info;

    /* Delete old selection list. */
    peony_drag_destroy_selection_list (dnd_info->drag_info.selection_list);
    ...
    dnd_info->drag_info.selection_list = peony_drag_build_selection_list (data);
    dnd_info->shadow = create_selection_shadow (container, dnd_info->drag_info.selection_list);
    peony_icon_container_position_shadow (container, x, y);
}
...
get_direct_save_filename (GdkDragContext *context)
{
    ...
    if (!gdk_property_get (gdk_drag_context_get_source_window (context), gdk_atom_intern (PEONY_ICON_DND_XDNDDIRECTSAVE_TYPE, FALSE),
...
static void
set_direct_save_uri (GtkWidget *widget, GdkDragContext *context, PeonyDragInfo *drag_info, int x, int y)
{
    GFile *base, *child;
    ...
    drag_info->data_type = PEONY_ICON_DND_XDNDDIRECTSAVE;

    uri = NULL;

    filename = get_direct_save_filename (context);
    drop_target = peony_icon_container_find_drop_target (PEONY_ICON_CONTAINER (widget),
    ...
    if (filename != NULL && drop_target != NULL)
    {
        ...
        gdk_property_change (gdk_drag_context_get_source_window (context),
                             gdk_atom_intern (PEONY_ICON_DND_XDNDDIRECTSAVE_TYPE, FALSE),
...
get_data_on_first_target_we_support (GtkWidget *widget, GdkDragContext *context, guint32 time, int x, int y)
{
    ...
    if (drop_types_list == NULL)
    {
        ...
        gtk_target_list_add_text_targets (drop_types_list, PEONY_ICON_DND_TEXT);
    }
    if (drop_types_list_root == NULL)
    {
        drop_types_list_root = gtk_target_list_new (drop_types,
                               G_N_ELEMENTS (drop_types));
        gtk_target_list_add_text_targets (drop_types_list_root, PEONY_ICON_DND_TEXT);
    }

    if (peony_icon_container_get_is_desktop (PEONY_ICON_CONTAINER (widget)))
    {
    ...
    if (target != GDK_NONE)
    {
        ...
        drag_info = &(PEONY_ICON_CONTAINER (widget)->details->dnd_info->drag_info);

        found = gtk_target_list_find (list, target, &info);
        g_assert (found);

        /* Don't get_data for destructive ops */
        if ((info == PEONY_ICON_DND_ROOTWINDOW_DROP ||
                info == PEONY_ICON_DND_XDNDDIRECTSAVE) &&
        ...
        {
            if (info == PEONY_ICON_DND_XDNDDIRECTSAVE)
            {
...
static void
peony_icon_container_ensure_drag_data (PeonyIconContainer *container,
                                      GdkDragContext *context,
                                      guint32 time)
{
    PeonyIconDndInfo *dnd_info;
...
static PeonyIcon *
peony_icon_container_item_at (PeonyIconContainer *container,
                             int x, int y)
{
    GList *p;
    int size;
    ...
    for (p = container->details->icons; p != NULL; p = p->next)
    {
        ...
        eel_canvas_w2c (EEL_CANVAS (container),
                        ...
                        &canvas_point.x1,
                        &canvas_point.y1);
        if (peony_icon_canvas_item_hit_test_rectangle (icon->item, canvas_point))
        {
            return icon;
        }
    }
...
static char *
get_container_uri (PeonyIconContainer *container)
{
    char *uri;
...
static gboolean
peony_icon_container_selection_items_local (PeonyIconContainer *container,
        GList *items)
{
    ...
    {
        result = peony_drag_items_on_desktop (items);
    }
    else
    {
        result = peony_drag_items_local (container_uri_string, items);
    }
...
get_background_drag_action (PeonyIconContainer *container,
                            GdkDragAction action)
{
    /* FIXME: This function is very FMDirectoryView specific, and
     * should be moved out of peony-icon-dnd.c */
    GdkDragAction valid_actions;

    if (action == GDK_ACTION_ASK)
    {
        valid_actions = PEONY_DND_ACTION_SET_AS_FOLDER_BACKGROUND;
        if (!eel_background_is_desktop (eel_get_widget_background (GTK_WIDGET (container))))
        {
            valid_actions |= PEONY_DND_ACTION_SET_AS_GLOBAL_BACKGROUND;
        }

        action = peony_drag_drop_background_ask
...
static void
receive_dropped_color (PeonyIconContainer *container,
                       int x, int y,
                       GdkDragAction action,
                       GtkSelectionData *data)
{
...
static void
receive_dropped_tile_image (PeonyIconContainer *container, GdkDragAction action, GtkSelectionData *data)
{
    g_assert (data != NULL);
...
static void
receive_dropped_keyword (PeonyIconContainer *container, const char *keyword, int x, int y)
{
    char *uri;
    double world_x, world_y;

    PeonyIcon *drop_target_icon;
    PeonyFile *file;
    ...
    drop_target_icon = peony_icon_container_item_at (container, world_x, world_y);
     ...
     */
    uri = peony_icon_container_get_icon_uri (container, drop_target_icon);
    ...
    peony_drag_file_receive_dropped_keyword (file, keyword);

    peony_file_unref (file);
    peony_icon_container_update_icon (container, drop_target_icon);
}

/* handle dropped url */
static void
receive_dropped_netscape_url (PeonyIconContainer *container, const char *encoded_url, GdkDragContext *context, int x, int y)
{
    char *drop_target;
    ...
    drop_target = peony_icon_container_find_drop_target (container, context, x, y, NULL, TRUE);
...
static void
receive_dropped_uri_list (PeonyIconContainer *container, const char *uri_list, GdkDragContext *context, int x, int y)
{
    char *drop_target;
...
/* handle dropped text */
static void
receive_dropped_text (PeonyIconContainer *container, const char *text, GdkDragContext *context, int x, int y)
{
    char *drop_target;
...
static void
receive_dropped_raw (PeonyIconContainer *container, const char *raw_data, int length, const char *direct_save_uri, GdkDragContext *context, int x, int y)
{
    char *drop_target;
...
auto_scroll_timeout_callback (gpointer data)
{
    ...
    g_assert (PEONY_IS_ICON_CONTAINER (data));
    ...
    peony_drag_autoscroll_calculate_delta (widget, &x_scroll_delta, &y_scroll_delta);
    ...
    if (!peony_icon_container_scroll (container, (int)x_scroll_delta, (int)y_scroll_delta))
    {
...
static void
set_up_auto_scroll_if_needed (PeonyIconContainer *container)
{
    peony_drag_autoscroll_start (&container->details->dnd_info->drag_info,
                                GTK_WIDGET (container),
...
static void
stop_auto_scroll (PeonyIconContainer *container)
{
    peony_drag_autoscroll_stop (&container->details->dnd_info->drag_info);
}

static void
handle_local_move (PeonyIconContainer *container,
                   double world_x, double world_y)
{
    GList *moved_icons, *p;
    PeonyDragSelectionItem *item;
    ...
    for (p = container->details->dnd_info->drag_info.selection_list; p != NULL; p = p->next)
    {
        ...
        icon = peony_icon_container_get_icon_by_uri
               (container, item->uri);
        ...
        if (icon == NULL)
        {
            ...
            file = peony_file_get_by_uri (item->uri);
            ...
            peony_file_set_time_metadata (file,
                                         PEONY_METADATA_KEY_ICON_POSITION_TIMESTAMP, now);

            peony_icon_container_add (container, PEONY_ICON_CONTAINER_ICON_DATA (file));
...
static void
handle_nonlocal_move (PeonyIconContainer *container,
                      GdkDragAction action,
                      int x, int y,
                      ...
                      gboolean icon_hit)
{
    ...
    for (p = container->details->dnd_info->drag_info.selection_list; p != NULL; p = p->next)
    {
        ...
        source_uris = g_list_prepend (source_uris, ((PeonyDragSelectionItem *)p->data)->uri);
    }
    ...
    if (!icon_hit)
    {
        ...
        {
            item_x = ((PeonyDragSelectionItem *)p->data)->icon_x;
            if (is_rtl)
                item_x = -item_x - ((PeonyDragSelectionItem *)p->data)->icon_width;
            g_array_index (source_item_locations, GdkPoint, index).x = item_x;
            g_array_index (source_item_locations, GdkPoint, index).y =
                ((PeonyDragSelectionItem *)p->data)->icon_y;
        }
    }
    ...
    {
        target_uri = peony_get_desktop_directory_uri ();
...
static char *
peony_icon_container_find_drop_target (PeonyIconContainer *container,
                                      GdkDragContext *context,
                                      int x, int y,
                                      gboolean *icon_hit,
                                      gboolean rewrite_desktop)
{
    ...
    if (drop_target_icon != NULL)
    {
        icon_uri = peony_icon_container_get_icon_uri (container, drop_target_icon);
        if (icon_uri != NULL)
        {
            file = peony_file_get_by_uri (icon_uri);

            if (!peony_drag_can_accept_info (file,
    ...
    if (drop_target_icon == NULL)
    {
                ...
                eel_uri_is_desktop (container_uri))
        {
            ...
            container_uri = peony_get_desktop_directory_uri ();
        }
    ...
    return peony_icon_container_get_icon_drop_target_uri (container, drop_target_icon);
}
...
selection_is_image_file (GList *selection_list)
{
    ...
    PeonyDragSelectionItem *selected_item;
    gboolean result;
    GFile *location;
    GFileInfo *info;
...
static void
peony_icon_container_receive_dropped_icons (PeonyIconContainer *container,
        GdkDragContext *context,
        int x, int y)
{
    char *drop_target;
    ...
    if (real_action == GDK_ACTION_ASK)
    {
        ...
        if (peony_drag_selection_includes_special_link (container->details->dnd_info->drag_info.selection_list))
        {
        ...
        else
        {
            ...
            {
                action |= PEONY_DND_ACTION_SET_AS_BACKGROUND;
            }
        }
        real_action = peony_drag_drop_action_ask (GTK_WIDGET (container), action);
    }

    if (real_action == (GdkDragAction) PEONY_DND_ACTION_SET_AS_BACKGROUND)
    {
    ...
    if (real_action > 0)
    {
        ...
        drop_target = peony_icon_container_find_drop_target (container,
        ...
        if (!icon_hit && real_action == GDK_ACTION_MOVE)
        {
             ...
             */
            local_move_only = peony_icon_container_selection_items_local
    ...
    g_free (drop_target);
    peony_drag_destroy_selection_list (container->details->dnd_info->drag_info.selection_list);
    container->details->dnd_info->drag_info.selection_list = NULL;
}

static void
peony_icon_container_get_drop_action (PeonyIconContainer *container,
                                     GdkDragContext *context,
                                     int x, int y,
                                     int *action)
{
    ...
    icon = peony_icon_container_item_at (container, world_x, world_y);
    ...
    *action = 0;
    ...
    case PEONY_ICON_DND_UKUI_ICON_LIST:
        ...
        peony_drag_default_drop_action_for_icons (context, drop_target,
                container->details->dnd_info->drag_info.selection_list,
                action);
        g_free (drop_target);
        break;
    case PEONY_ICON_DND_URI_LIST:
        drop_target = peony_icon_container_find_drop_target (container,
                      context, x, y, &icon_hit, FALSE);
        *action = peony_drag_default_drop_action_for_uri_list (context, drop_target);
    ...
    case PEONY_ICON_DND_NETSCAPE_URL:
        *action = peony_drag_default_drop_action_for_netscape_url (context);
        break;
...
static void
set_drop_target (PeonyIconContainer *container,
                 PeonyIcon *icon)
{
    PeonyIcon *old_icon;
    ...
    peony_icon_container_update_icon (container, old_icon);
    peony_icon_container_update_icon (container, icon);
}

static void
peony_icon_dnd_update_drop_target (PeonyIconContainer *container,
                                  GdkDragContext *context,
                                  int x, int y)
{
    PeonyIcon *icon;
    ...
    if (icon != NULL && (container->details->dnd_info->drag_info.data_type != PEONY_ICON_DND_KEYWORD))
    {
        uri = peony_icon_container_get_icon_uri (container, icon);
        file = peony_file_get_by_uri (uri);
        g_free (uri);
...
static void
peony_icon_container_free_drag_data (PeonyIconContainer *container)
{
    PeonyIconDndInfo *dnd_info;
                     ...
                     gpointer data)
{
    ...
    dnd_info = PEONY_ICON_CONTAINER (widget)->details->dnd_info;
    ...
    set_drop_target (PEONY_ICON_CONTAINER (widget), NULL);
    stop_auto_scroll (PEONY_ICON_CONTAINER (widget));
    peony_icon_container_free_drag_data(PEONY_ICON_CONTAINER (widget));
}
...
#if GTK_CHECK_VERSION(3,0,0)
    surface = peony_icon_canvas_item_get_drag_surface (container->details->drag_icon->item);
#else
    pixmap = peony_icon_canvas_item_get_image (container->details->drag_icon->item, &mask, colormap);
#endif
...
void
peony_icon_dnd_begin_drag (PeonyIconContainer *container,
                      ...
                      guint32 time)
{
    ...
    peony_icon_container_ensure_drag_data (PEONY_ICON_CONTAINER (widget), context, time);
    peony_icon_container_position_shadow (PEONY_ICON_CONTAINER (widget), x, y);
    peony_icon_dnd_update_drop_target (PEONY_ICON_CONTAINER (widget), context, x, y);
    set_up_auto_scroll_if_needed (PEONY_ICON_CONTAINER (widget));
    /* Find out what the drop actions are based on our drag selection and
     * the drop target.
     */
    action = 0;
    peony_icon_container_get_drop_action (PEONY_ICON_CONTAINER (widget), context, x, y,
                                         &action);
    if (action != 0)
    {
...
void
peony_icon_dnd_end_drag (PeonyIconContainer *container)
{
    PeonyIconDndInfo *dnd_info;
                             ...
                             gpointer user_data)
{
    ...
    {
    case PEONY_ICON_DND_UKUI_ICON_LIST:
        peony_icon_container_dropped_icon_feedback (widget, data, x, y);
        break;
    case PEONY_ICON_DND_COLOR:
    case PEONY_ICON_DND_BGIMAGE:
    case PEONY_ICON_DND_KEYWORD:
    ...
    if (drag_info->drop_occured)
    {
        ...
        {
        case PEONY_ICON_DND_UKUI_ICON_LIST:
            peony_icon_container_receive_dropped_icons
            (PEONY_ICON_CONTAINER (widget),
             context, x, y);
            break;
        case PEONY_ICON_DND_COLOR:
            receive_dropped_color (PEONY_ICON_CONTAINER (widget),
                                   x, y,
                                   gdk_drag_context_get_selected_action (context),
                                   data);
            success = TRUE;
        ...
        case PEONY_ICON_DND_XDNDDIRECTSAVE:
        {
                    ...
                    selection_data[0] == 'F')
            {
                gtk_drag_get_data (widget, context,
                                   gdk_atom_intern (PEONY_ICON_DND_RAW_TYPE,
                                                    FALSE),
                                   time);
                return;
            }
                     ...
                     drag_info->direct_save_uri != NULL)
            {
                ...
                p.x = x;
                p.y = y;
                peony_file_changes_queue_schedule_position_set (
                    location,
                    p,
...
void
peony_icon_dnd_init (PeonyIconContainer *container)
{
    ...
    container->details->dnd_info = g_new0 (PeonyIconDndInfo, 1);
    peony_drag_init (&container->details->dnd_info->drag_info,
    ...
    gtk_target_list_add_text_targets (targets, PEONY_ICON_DND_TEXT);
...
void
peony_icon_dnd_fini (PeonyIconContainer *container)
{
    g_return_if_fail (PEONY_IS_ICON_CONTAINER (container));
    ...
    if (container->details->dnd_info != NULL)
    {
        ...
        peony_drag_finalize (&container->details->dnd_info->drag_info);
