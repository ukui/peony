...
/* peony-file-operations.c - Peony file operations.
...
#include "peony-autorun.h"
#include "peony-trash-monitor.h"
...
{
	peony_progress_info_finish (common->progress);

	if (common->inhibit_cookie != -1) {
		peony_uninhibit_power_manager (common->inhibit_cookie);
	}
...
can_delete_without_confirm (GFile *file)
{
	if (g_file_has_uri_scheme (file, "burn") ||
	    g_file_has_uri_scheme (file, "x-peony-desktop")) {
		return TRUE;
	}
		      ...
		      va_list varargs)
{
	...
	peony_progress_info_pause (job->progress);
	g_io_scheduler_job_send_to_mainloop (job->io_job,
					     do_run_simple_dialog,
					     data,
					     NULL);
	peony_progress_info_resume (job->progress);
	res = data->result;
...
{
	job->inhibit_cookie = peony_inhibit_power_manager (message);
}
...
should_confirm_trash (void)
{
	...
	confirm_trash = g_settings_get_boolean (prefs, PEONY_PREFERENCES_CONFIRM_TRASH);
			...
			TransferInfo *transfer_info)
{
	...
	peony_progress_info_take_status (job->progress,
					    f (_("Deleting files")));

	elapsed = g_timer_elapsed (job->time, NULL);
	if (elapsed < SECONDS_NEEDED_FOR_RELIABLE_TRANSFER_RATE) {

		peony_progress_info_set_details (job->progress, files_left_s);
	} else {
		...
		peony_progress_info_take_details (job->progress, details);
	...
	if (source_info->num_files != 0) {
		peony_progress_info_set_progress (job->progress, transfer_info->num_files, source_info->num_files);
	}
}
 ...
 retry:
	...
	if (!job_aborted (job) &&
		...
		skip:
			...
			peony_file_changes_queue_file_removed (dir);
	     ...
	     gboolean toplevel)
{
	...
	if (g_file_delete (file, job->cancellable, &error)) {
		peony_file_changes_queue_file_removed (file);
		transfer_info->num_files ++;
		report_delete_progress (job, source_info, transfer_info);
		return;
	}
		       ...
		       int total_files)
{
	...
	s = f (ngettext ("%'d file left to trash",
			 ...
			 files_left),
	       files_left);
	peony_progress_info_take_details (job->progress, s);

	if (total_files != 0) {
		peony_progress_info_set_progress (job->progress, files_trashed, total_files);
	}
}
...
trash_files (CommonJob *job, GList *files, int *files_skipped)
{
	...
	for (l = files;
		...
		mtime = peony_undostack_manager_get_file_modification_time (file);
		...
		skip:
			...
			// Start UNDO-REDO
			peony_undostack_manager_data_add_trashed_file (job->undo_redo_data, file, mtime);
			// End UNDO-REDO
	    ...
	    gpointer user_data)
{
	...
	peony_progress_info_start (job->common.progress);
			  ...
			  gpointer                done_callback_data)
{
	...
	if (try_trash && !peony_undostack_manager_is_undo_redo(peony_undostack_manager_instance())) {
		job->common.undo_redo_data = peony_undostack_manager_data_new (PEONY_UNDOSTACK_MOVETOTRASH, g_list_length(files));
		GFile* src_dir = g_file_get_parent (files->data);
		peony_undostack_manager_data_set_src_dir (job->common.undo_redo_data, src_dir);
	}
	// End UNDO-REDO
...
void
peony_file_operations_trash_or_delete (GList                  *files,
					  GtkWindow              *parent_window,
					  PeonyDeleteCallback  done_callback,
					  gpointer                done_callback_data)
{
...
void
peony_file_operations_delete (GList                  *files,
				 GtkWindow              *parent_window,
				 PeonyDeleteCallback  done_callback,
				 gpointer                done_callback_data)
{
...
void
peony_file_operations_unmount_mount_full (GtkWindow                      *parent_window,
					     GMount                         *mount,
					     gboolean                        eject,
...
void
peony_file_operations_unmount_mount (GtkWindow                      *parent_window,
					GMount                         *mount,
					gboolean                        eject,
					gboolean                        check_trash)
{
	peony_file_operations_unmount_mount_full (parent_window, mount, eject,
		 ...
		 gpointer user_data)
{
	...
	error = NULL;
	peony_allow_autorun_for_volume_finish (G_VOLUME (source_object));
	...
	mount_callback = (PeonyMountCallback)
...
void
peony_file_operations_mount_volume (GtkWindow *parent_window,
				       GVolume *volume,
				       gboolean allow_autorun)
{
	peony_file_operations_mount_volume_full (parent_window, volume,
						    allow_autorun, NULL, NULL);
}

void
peony_file_operations_mount_volume_full (GtkWindow *parent_window,
					    GVolume *volume,
					    gboolean allow_autorun,
					    PeonyMountCallback mount_callback,
					    GObject *mount_callback_data_object)
		       ...
		       SourceInfo *source_info)
{
	...
	peony_progress_info_take_details (job->progress, s);
	peony_progress_info_pulse_progress (job->progress);
}
		      ...
		      TransferInfo *transfer_info)
{
	...
	peony_progress_info_set_progress (job->progress, transfer_info->num_bytes, total_size);
}
		 ...
		 char **dest_fs_type)
{
 ...
 retry:
	...
	// Start UNDO-REDO
	peony_undostack_manager_data_add_origin_target_pair (job->undo_redo_data, src, *dest);
	// End UNDO-REDO
...
do_run_conflict_dialog (gpointer _data)
{
	...
	dialog = peony_file_conflict_dialog_new (data->parent,
						    data->src,
						    data->dest,
						    data->dest_dir);
	...
	if (response == CONFLICT_RESPONSE_RENAME) {
		data->resp_data->new_name =
			peony_file_conflict_dialog_get_new_name (PEONY_FILE_CONFLICT_DIALOG (dialog));
	} else if (response != GTK_RESPONSE_CANCEL ||
		   response != GTK_RESPONSE_NONE) {
		   data->resp_data->apply_to_all =
			   peony_file_conflict_dialog_get_apply_to_all
				(PEONY_FILE_CONFLICT_DIALOG (dialog));
	}
		...
		gboolean last_item)
{
 ...
 retry:
	...
	if (res) {
		...
		if (copy_job->is_move) {
			peony_file_changes_queue_file_moved (src, dest);
		} else {
			peony_file_changes_queue_file_added (dest);
		}
	...
	/* Needs to recurse */
		...
		if (overwrite && would_recurse) {
			...
			peony_file_changes_queue_file_removed (dest);
		}
			       ...
			       gpointer done_callback_data)
{
	...
	if (!peony_undostack_manager_is_undo_redo(peony_undostack_manager_instance())) {
		job->common.undo_redo_data = peony_undostack_manager_data_new (PEONY_UNDOSTACK_COPY, g_list_length(files));
		GFile* src_dir = g_file_get_parent (files->data);
		peony_undostack_manager_data_set_src_dir (job->common.undo_redo_data, src_dir);
		g_object_ref (target_dir);
		peony_undostack_manager_data_set_dest_dir (job->common.undo_redo_data, target_dir);
	}
	// End UNDO-REDO
...
report_move_progress (CopyMoveJob *move_job, int total, int left)
{
	...
	peony_progress_info_take_details (job->progress,
...
void
peony_file_operations_move (GList *files,
			       GArray *relative_item_points,
			       GFile *target_dir,
			       GtkWindow *parent_window,
			       PeonyCopyCallback  done_callback,
			       gpointer done_callback_data)
{
	...
	if (!peony_undostack_manager_is_undo_redo(peony_undostack_manager_instance())) {
		if (g_file_has_uri_scheme (g_list_first(files)->data, "trash")) {
			job->common.undo_redo_data = peony_undostack_manager_data_new (PEONY_UNDOSTACK_RESTOREFROMTRASH, g_list_length(files));
		} else {
			job->common.undo_redo_data = peony_undostack_manager_data_new (PEONY_UNDOSTACK_MOVE, g_list_length(files));
		}
		GFile* src_dir = g_file_get_parent (files->data);
		peony_undostack_manager_data_set_src_dir (job->common.undo_redo_data, src_dir);
		g_object_ref (target_dir);
...
report_link_progress (CopyMoveJob *link_job, int total, int left)
{
	...
	peony_progress_info_set_progress (job->progress, left, total);
}
 ...
 retry:
	...
	if (path == NULL) {
		not_local = TRUE;
					      ...
					      &error)) {
		// Start UNDO-REDO
		peony_undostack_manager_data_add_origin_target_pair (common->undo_redo_data, src, dest);
		// End UNDO-REDO
		g_free (path);
		if (debuting_files) {
			g_hash_table_replace (debuting_files, g_object_ref (dest), GINT_TO_POINTER (TRUE));
		...
		peony_file_changes_queue_file_added (dest);
		if (position) {
			peony_file_changes_queue_schedule_position_set (dest, *position, common->screen_num);
		} else {
			peony_file_changes_queue_schedule_position_remove (dest);
		}
...
void
peony_file_operations_link (GList *files,
			       GArray *relative_item_points,
			       GFile *target_dir,
			       GtkWindow *parent_window,
			       gboolean bSendToDesktop,
			       ...
			       gpointer done_callback_data)
{
	...
	// Start UNDO-REDO
	if (!peony_undostack_manager_is_undo_redo(peony_undostack_manager_instance())) {
		job->common.undo_redo_data = peony_undostack_manager_data_new (PEONY_UNDOSTACK_CREATELINK, g_list_length(files));
		GFile* src_dir = g_file_get_parent (files->data);
		peony_undostack_manager_data_set_src_dir (job->common.undo_redo_data, src_dir);
		g_object_ref (target_dir);
		peony_undostack_manager_data_set_dest_dir (job->common.undo_redo_data, target_dir);
...
void
peony_file_operations_duplicate (GList *files,
				    GArray *relative_item_points,
				    GtkWindow *parent_window,
				    PeonyCopyCallback  done_callback,
				    gpointer done_callback_data)
{
	...
	// Start UNDO-REDO
	if (!peony_undostack_manager_is_undo_redo(peony_undostack_manager_instance())) {
		job->common.undo_redo_data = peony_undostack_manager_data_new (PEONY_UNDOSTACK_DUPLICATE, g_list_length(files));
		GFile* src_dir = g_file_get_parent (files->data);
		peony_undostack_manager_data_set_src_dir (job->common.undo_redo_data, src_dir);
		g_object_ref (src_dir);
		peony_undostack_manager_data_set_dest_dir (job->common.undo_redo_data, src_dir);
	}
	// End UNDO-REDO
		      ...
		      GFileInfo *info)
{
	...
	if (!job_aborted (common) &&
		...
		// Start UNDO-REDO
		peony_undostack_manager_data_add_file_permissions(common->undo_redo_data, file, current);
		// End UNDO-REDO
		     ...
		     gpointer user_data)
{
	...
	peony_progress_info_set_status (common->progress,
...
void
peony_file_set_permissions_recursive (const char *directory,
					 guint32         file_permissions,
					 guint32         file_mask,
					 guint32         dir_permissions,
					 guint32         dir_mask,
					 ...
					 gpointer  callback_data)
{
	...
	// Start UNDO-REDO
	if (!peony_undostack_manager_is_undo_redo(peony_undostack_manager_instance())) {
		job->common.undo_redo_data = peony_undostack_manager_data_new (PEONY_UNDOSTACK_RECURSIVESETPERMISSIONS, 1);
		g_object_ref (job->file);
		peony_undostack_manager_data_set_dest_dir (job->common.undo_redo_data, job->file);
		peony_undostack_manager_data_set_recursive_permissions(job->common.undo_redo_data, file_permissions, file_mask, dir_permissions, dir_mask);
	}
	// End UNDO-REDO
...
typedef struct {
	PeonyCopyCallback real_callback;
	gpointer real_data;
} MoveTrashCBData;
...
void
peony_file_operations_copy_move (const GList *item_uris,
				    GArray *relative_item_points,
				    ...
				    gpointer done_callback_data)
{
	...
	if (copy_action == GDK_ACTION_COPY) {
		...
		if (target_dir == NULL ||
			...
			peony_file_operations_duplicate (locations,
							    relative_item_points,
							    parent_window,
							    done_callback, done_callback_data);
		} else {
			peony_file_operations_copy (locations,
						       relative_item_points,
						       dest,
						       parent_window,
						       done_callback, done_callback_data);
		...
		if (g_file_has_uri_scheme (dest, "trash")) {
			...
			peony_file_operations_trash_or_delete (locations,
								  parent_window,
								  (PeonyDeleteCallback) callback_for_move_to_trash,
								  cb_data);
		} else {
			peony_file_operations_move (locations,
						       relative_item_points,
						       dest,
						       parent_window,
						       done_callback, done_callback_data);
	    ...
	    gpointer user_data)
{
 ...
 retry:
	...
	if (job->make_dir) {
		...
		// Start UNDO-REDO
		if (res) {
			peony_undostack_manager_data_set_create_data(common->undo_redo_data,
					g_file_get_uri(dest),
					NULL);
		}
		// End UNDO-REDO
...
void
peony_file_operations_new_folder (GtkWidget *parent_view,
				     ...
				     gpointer done_callback_data)
{
	...
	// Start UNDO-REDO
	if (!peony_undostack_manager_is_undo_redo(peony_undostack_manager_instance())) {
		job->common.undo_redo_data = peony_undostack_manager_data_new (PEONY_UNDOSTACK_CREATEFOLDER, 1);
	}
	// End UNDO-REDO
...
void
peony_file_operations_new_file_from_template (GtkWidget *parent_view,
						 GdkPoint *target_point,
						 const char *parent_dir,
						 ...
						 gpointer done_callback_data)
{
	...
	// Start UNDO-REDO
	if (!peony_undostack_manager_is_undo_redo(peony_undostack_manager_instance())) {
		job->common.undo_redo_data = peony_undostack_manager_data_new (PEONY_UNDOSTACK_CREATEFILEFROMTEMPLATE, 1);
	}
	// End UNDO-REDO
...
void
peony_file_operations_new_file (GtkWidget *parent_view,
				   GdkPoint *target_point,
				   const char *parent_dir,
				   const char *target_filename,
				   ...
				   gpointer done_callback_data)
{
	...
	// Start UNDO-REDO
	if (!peony_undostack_manager_is_undo_redo(peony_undostack_manager_instance())) {
		job->common.undo_redo_data = peony_undostack_manager_data_new (PEONY_UNDOSTACK_CREATEEMPTYFILE, 1);
	}
	// End UNDO-REDO
...
empty_trash_job_done (gpointer user_data)
{
	...
	peony_undostack_manager_trash_has_emptied(peony_undostack_manager_instance());
...
void
peony_file_operations_empty_trash (GtkWidget *parent_view)
{
	EmptyTrashJob *job;
	GtkWindow *parent_window;
...
void
peony_file_mark_desktop_file_trusted (GFile *file,
					 GtkWindow *parent_window,
					 gboolean interactive,
					 PeonyOpCallback done_callback,
					 gpointer done_callback_data)
...
void
peony_self_check_file_operations (void)
{
