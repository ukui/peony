/* -*- Mode: C; indent-tabs-mode: t; c-basic-offset: 8; tab-width: 8 -*-
   ...
   peony-directory.c: Peony directory model.
...
static void               peony_directory_finalize         (GObject                *object);
static void               peony_directory_init             (gpointer                object,
        gpointer                klass);
static void               peony_directory_class_init (PeonyDirectoryClass *klass);
static PeonyDirectory *peony_directory_new              (GFile                  *location);
static char *             real_get_name_for_self_as_new_file  (PeonyDirectory      *directory);
static GList *            real_get_file_list                  (PeonyDirectory      *directory);
static gboolean		  real_is_editable                    (PeonyDirectory      *directory);
static void               set_directory_location              (PeonyDirectory      *directory,
        GFile                  *location);

EEL_CLASS_BOILERPLATE (PeonyDirectory,
                       peony_directory,
                       G_TYPE_OBJECT)

static void
peony_directory_class_init (PeonyDirectoryClass *klass)
{
    GObjectClass *object_class;

    object_class = G_OBJECT_CLASS (klass);

    object_class->finalize = peony_directory_finalize;

    signals[FILES_ADDED] =
        g_signal_new ("files_added",
                      G_TYPE_FROM_CLASS (object_class),
                      G_SIGNAL_RUN_LAST,
                      G_STRUCT_OFFSET (PeonyDirectoryClass, files_added),
                      NULL, NULL,
    ...
    signals[FILES_CHANGED] =
        g_signal_new ("files_changed",
                      G_TYPE_FROM_CLASS (object_class),
                      G_SIGNAL_RUN_LAST,
                      G_STRUCT_OFFSET (PeonyDirectoryClass, files_changed),
                      NULL, NULL,
                      g_cclosure_marshal_VOID__POINTER,
                      G_TYPE_NONE, 1, G_TYPE_POINTER);
    signals[DONE_LOADING] =
        g_signal_new ("done_loading",
                      G_TYPE_FROM_CLASS (object_class),
                      G_SIGNAL_RUN_LAST,
                      G_STRUCT_OFFSET (PeonyDirectoryClass, done_loading),
                      NULL, NULL,
    ...
    signals[LOAD_ERROR] =
        g_signal_new ("load_error",
                      G_TYPE_FROM_CLASS (object_class),
                      G_SIGNAL_RUN_LAST,
                      G_STRUCT_OFFSET (PeonyDirectoryClass, load_error),
                      NULL, NULL,
                      g_cclosure_marshal_VOID__POINTER,
                      G_TYPE_NONE, 1, G_TYPE_POINTER);
    ...
    g_type_class_add_private (klass, sizeof (PeonyDirectoryDetails));
}

static void
peony_directory_init (gpointer object, gpointer klass)
{
    PeonyDirectory *directory;

    directory = PEONY_DIRECTORY(object);

    directory->details = G_TYPE_INSTANCE_GET_PRIVATE ((directory), PEONY_TYPE_DIRECTORY, PeonyDirectoryDetails);
    directory->details->file_hash = g_hash_table_new (g_str_hash, g_str_equal);
    directory->details->high_priority_queue = peony_file_queue_new ();
    directory->details->low_priority_queue = peony_file_queue_new ();
    directory->details->extension_queue = peony_file_queue_new ();
    directory->details->free_space = (guint64)-1;
}

PeonyDirectory *
peony_directory_ref (PeonyDirectory *directory)
{
    if (directory == NULL)
    {
        return directory;
...
void
peony_directory_unref (PeonyDirectory *directory)
{
    if (directory == NULL)
    {
        return;
...
static void
peony_directory_finalize (GObject *object)
{
    PeonyDirectory *directory;
    ...
    peony_file_queue_destroy (directory->details->high_priority_queue);
    peony_file_queue_destroy (directory->details->low_priority_queue);
    peony_file_queue_destroy (directory->details->extension_queue);
...
invalidate_one_count (gpointer key, gpointer value, gpointer user_data)
{
    ...
    g_assert (key != NULL);
    g_assert (PEONY_IS_DIRECTORY (value));
    g_assert (user_data == NULL);

    directory = PEONY_DIRECTORY (value);
...
void
emit_change_signals_for_all_files (PeonyDirectory *directory)
{
    GList *files;
    ...
    peony_file_list_ref (files);
    peony_directory_emit_change_signals (directory, files);
...
collect_all_directories (gpointer key, gpointer value, gpointer callback_data)
{
    ...
    *dirs = g_list_prepend (*dirs, peony_directory_ref (directory));
}
...
emit_change_signals_for_all_files_in_all_directories (void)
{
    ...
    {
        directory = PEONY_DIRECTORY (l->data);
...
add_preferences_callbacks (void)
{
    ...
    g_signal_connect_swapped (peony_preferences,
                              ...
                              NULL);
    g_signal_connect_swapped (peony_preferences,
                              "changed::" PEONY_PREFERENCES_DATE_FORMAT,
                              G_CALLBACK(async_data_preference_changed_callback),
                              NULL);
}

/**
 * peony_directory_get_by_uri:
 ...
 */
PeonyDirectory *
peony_directory_get_internal (GFile *location, gboolean create)
{
    PeonyDirectory *directory;
    ...
    if (directories == NULL)
    {
        directories = eel_g_hash_table_new_free_at_exit
                      (g_file_hash, (GCompareFunc)g_file_equal, "peony-directory.c: directories");
    ...
    else if (create)
    {
        ...
        directory = peony_directory_new (location);
        if (directory == NULL)
        {
            return NULL;
        }
...
PeonyDirectory *
peony_directory_get (GFile *location)
{
    if (location == NULL)
    {
        return NULL;
    ...
    return peony_directory_get_internal (location, TRUE);
}

PeonyDirectory *
peony_directory_get_existing (GFile *location)
{
    if (location == NULL)
    {
        return NULL;
    ...
    return peony_directory_get_internal (location, FALSE);
}
...
peony_directory_get_by_uri (const char *uri)
{
    ...
    directory = peony_directory_get_internal (location, TRUE);
    g_object_unref (location);
    return directory;
}

PeonyDirectory *
peony_directory_get_for_file (PeonyFile *file)
{
    char *uri;
    PeonyDirectory *directory;
    ...
    directory = peony_directory_get_by_uri (uri);
    g_free (uri);
    return directory;
}

/* Returns a reffed PeonyFile object for this directory.
 */
PeonyFile *
peony_directory_get_corresponding_file (PeonyDirectory *directory)
{
    PeonyFile *file;
    char *uri;
...
/* Returns a reffed PeonyFile object for this directory, but only if the
 * PeonyFile object has already been created.
 */
PeonyFile *
peony_directory_get_existing_corresponding_file (PeonyDirectory *directory)
{
    PeonyFile *file;
    char *uri;
...
/* peony_directory_get_name_for_self_as_new_file:
 *
 * Get a name to display for the file representing this
 * directory. This is called only when there's no VFS
 * directory for this PeonyDirectory.
 */
char *
peony_directory_get_name_for_self_as_new_file (PeonyDirectory *directory)
{
    g_return_val_if_fail (PEONY_IS_DIRECTORY (directory), NULL);

    return EEL_CALL_METHOD_WITH_RETURN_VALUE
           (PEONY_DIRECTORY_CLASS, directory,
            get_name_for_self_as_new_file, (directory));
}

static char *
real_get_name_for_self_as_new_file (PeonyDirectory *directory)
{
    char *directory_uri;
    char *name, *colon;

    directory_uri = peony_directory_get_uri (directory);
...
char *
peony_directory_get_uri (PeonyDirectory *directory)
{
    g_return_val_if_fail (PEONY_IS_DIRECTORY (directory), NULL);
...
GFile *
peony_directory_get_location (PeonyDirectory  *directory)
{
    g_return_val_if_fail (PEONY_IS_DIRECTORY (directory), NULL);
...
static PeonyDirectory *
peony_directory_new (GFile *location)
{
    PeonyDirectory *directory;
    char *uri;
    ...
    {
        directory = PEONY_DIRECTORY (g_object_new (PEONY_TYPE_DESKTOP_DIRECTORY, NULL));
    }
    else if (eel_uri_is_search (uri))
    {
        directory = PEONY_DIRECTORY (g_object_new (PEONY_TYPE_SEARCH_DIRECTORY, NULL));
    }
    else if (g_str_has_suffix (uri, PEONY_SAVED_SEARCH_EXTENSION))
    {
        directory = PEONY_DIRECTORY (peony_search_directory_new_from_saved_search (uri));
    }
    else
    {
        directory = PEONY_DIRECTORY (g_object_new (PEONY_TYPE_VFS_DIRECTORY, NULL));
    }
...
gboolean
peony_directory_is_local (PeonyDirectory *directory)
{
    g_return_val_if_fail (PEONY_IS_DIRECTORY (directory), FALSE);
    ...
    return peony_directory_is_in_trash (directory) ||
           g_file_is_native (directory->details->location);
}

gboolean
peony_directory_is_in_trash (PeonyDirectory *directory)
{
...
gboolean
peony_directory_are_all_files_seen (PeonyDirectory *directory)
{
    g_return_val_if_fail (PEONY_IS_DIRECTORY (directory), FALSE);
...
static void
add_to_hash_table (PeonyDirectory *directory, PeonyFile *file, GList *node)
{
    const char *name;
...
static GList *
extract_from_hash_table (PeonyDirectory *directory, PeonyFile *file)
{
    const char *name;
    GList *node;
...
void
peony_directory_add_file (PeonyDirectory *directory, PeonyFile *file)
{
    GList *node;
    ...
    else if (peony_directory_has_active_request_for_file (directory, file))
    {
...
GList *
peony_directory_begin_file_name_change (PeonyDirectory *directory,
                                       PeonyFile *file)
{
    /* Find the list node in the hash table. */
...
void
peony_directory_end_file_name_change (PeonyDirectory *directory,
                                     PeonyFile *file,
                                     GList *node)
{
...
PeonyFile *
peony_directory_find_file_by_name (PeonyDirectory *directory,
                                  const char *name)
{
    GList *node;
    ...
    node = g_hash_table_lookup (directory->details->file_hash,
                                name);
    return node == NULL ? NULL : PEONY_FILE (node->data);
}

void
peony_directory_emit_files_added (PeonyDirectory *directory,
                                 GList *added_files)
{
    if (added_files != NULL)
    {
...
void
peony_directory_emit_files_changed (PeonyDirectory *directory,
                                   GList *changed_files)
{
...
void
peony_directory_emit_change_signals (PeonyDirectory *directory,
                                    GList *changed_files)
{
    GList *p;
    ...
    {
        peony_file_emit_changed (p->data);
    }
    peony_directory_emit_files_changed (directory, changed_files);
}

void
peony_directory_emit_done_loading (PeonyDirectory *directory)
{
    g_signal_emit (directory,
...
void
peony_directory_emit_load_error (PeonyDirectory *directory,
                                GError *error)
{
    g_signal_emit (directory,
...
get_parent_directory (GFile *location)
{
    ...
    if (parent)
    {
        directory = peony_directory_get_internal (parent, TRUE);
...
get_parent_directory_if_exists (GFile *location)
{
    ...
    if (parent)
    {
        directory = peony_directory_get_internal (parent, FALSE);
        g_object_unref (parent);
        return directory;
    }
    return NULL;
...
{
    g_assert (PEONY_IS_DIRECTORY (key));
    g_assert (value != NULL);
    g_assert (user_data == NULL);
...
static void
call_files_changed_common (PeonyDirectory *directory, GList *file_list)
{
    GList *node;
    PeonyFile *file;
    ...
    for (node = file_list; node != NULL; node = node->next)
    {
        ...
        {
            peony_directory_add_file_to_work_queue (directory,
                                                   file);
        }
    }
    peony_directory_async_state_changed (directory);
    peony_directory_emit_change_signals (directory, file_list);
}
...
call_files_changed_free_list (gpointer key, gpointer value, gpointer user_data)
{
    ...
    call_files_changed_common (PEONY_DIRECTORY (key), value);
    g_list_free (value);
}
...
call_get_file_info_free_list (gpointer key, gpointer value, gpointer user_data)
{
    ...
    peony_directory_get_info_for_new_files (directory, files);
...
invalidate_count_and_unref (gpointer key, gpointer value, gpointer user_data)
{
    ...
    peony_directory_invalidate_count_and_mime_list (key);
    peony_directory_unref (key);
}

static void
collect_parent_directories (GHashTable *hash_table, PeonyDirectory *directory)
{
...
void
peony_directory_notify_files_added (GList *files)
{
    ...
    for (p = files; p != NULL; p = p->next)
    {
        ...
        if (directory == NULL)
        {
            ...
            if (parent)
            {
                file = peony_file_get_existing (parent);
                g_object_unref (parent);
            }
        ...
        file = peony_file_get_existing (location);
        /* We check is_added here, because the file could have been added
         * to the directory by a peony_file_get() but not gotten
         * files_added emitted
         */
    ...
    /* Now get file info for the new files. This creates PeonyFile
...
void
peony_directory_notify_files_changed (GList *files)
{
...
void
peony_directory_notify_files_removed (GList *files)
{
    GHashTable *changed_lists;
    GList *p;
    PeonyDirectory *directory;
    ...
    for (p = files; p != NULL; p = p->next)
    {
        ...
        /* Find the file. */
        file = peony_file_get_existing (location);
        if (file != NULL && !peony_file_rename_in_progress (file))
        {
...
static void
set_directory_location (PeonyDirectory *directory,
                        GFile *location)
{
    if (directory->details->location)
    {
...
static void
change_directory_location (PeonyDirectory *directory,
                           GFile *new_location)
{
...
static GList *
peony_directory_moved_internal (GFile *old_location,
                               GFile *new_location)
{
    ...
    {
        directory = PEONY_DIRECTORY (node->data);
        ...
        if (new_directory_location)
        {
            ...
            if (directory->details->as_file != NULL)
            {
                affected_files = g_list_prepend
                                 (affected_files,
                                  peony_file_ref (directory->details->as_file));
            }
...
void
peony_directory_moved (const char *old_uri,
                      const char *new_uri)
{
    GList *list, *node;
    GHashTable *hash;
    ...
    list = peony_directory_moved_internal (old_location, new_location);
...
void
peony_directory_notify_files_moved (GList *file_pairs)
{
    GList *p, *affected_files, *node;
    GFilePair *pair;
    PeonyFile *file;
    PeonyDirectory *old_directory, *new_directory;
    GHashTable *parent_directories;
    GList *new_files_list, *unref_list;
    GHashTable *added_lists, *changed_lists;
    char *name;
    PeonyFileAttributes cancel_attributes;
    ...
    cancel_attributes = peony_file_get_all_attributes ();
    ...
    for (p = file_pairs; p != NULL; p = p->next)
    {
        ...
        file = peony_file_get_existing (to_location);
        if (file != NULL)
        {
            /* Mark it gone and prepare to send the changed signal. */
            peony_file_mark_gone (file);
        ...
        affected_files = peony_directory_moved_internal (from_location,
                         to_location);
        ...
        file = peony_file_get_existing (from_location);
        if (file == NULL)
        {
        ...
        else
        {
            ...
            peony_directory_cancel_loading_file_attributes
             ...
             * anyway.
             */
            peony_directory_unref (new_directory);

            /* Update the file's name and directory. */
            name = g_file_get_basename (to_location);
            peony_file_update_name_and_directory
            (file, name, new_directory);
            g_free (name);

            /* Update file attributes */
            peony_file_invalidate_attributes (file, PEONY_FILE_ATTRIBUTE_INFO);
            ...
            /* Unref each file once to balance out peony_file_get_by_uri. */
    ...
    peony_file_list_free (unref_list);
    ...
    peony_directory_notify_files_added (new_files_list);
    g_list_free (new_files_list);
}

void
peony_directory_schedule_position_set (GList *position_setting_list)
{
    GList *p;
    const PeonyFileChangesQueuePosition *item;
    PeonyFile *file;
    char str[64];
    time_t now;
    ...
    {
        item = (PeonyFileChangesQueuePosition *) p->data;

        file = peony_file_get (item->location);
...
gboolean
peony_directory_contains_file (PeonyDirectory *directory,
                              PeonyFile *file)
{
    g_return_val_if_fail (PEONY_IS_DIRECTORY (directory), FALSE);
...
void
peony_directory_call_when_ready (PeonyDirectory *directory,
...
void
peony_directory_cancel_callback (PeonyDirectory *directory,
                                PeonyDirectoryCallback callback,
                                gpointer callback_data)
{
    g_return_if_fail (PEONY_IS_DIRECTORY (directory));
...
void
peony_directory_file_monitor_add (PeonyDirectory *directory,
...
void
peony_directory_file_monitor_remove (PeonyDirectory *directory,
                                    gconstpointer client)
{
    g_return_if_fail (PEONY_IS_DIRECTORY (directory));
    g_return_if_fail (client != NULL);
...
void
peony_directory_force_reload (PeonyDirectory *directory)
{
    g_return_if_fail (PEONY_IS_DIRECTORY (directory));
...
gboolean
peony_directory_is_not_empty (PeonyDirectory *directory)
{
    g_return_val_if_fail (PEONY_IS_DIRECTORY (directory), FALSE);
...
GList *
peony_directory_get_file_list (PeonyDirectory *directory)
{
    return EEL_CALL_METHOD_WITH_RETURN_VALUE
           (PEONY_DIRECTORY_CLASS, directory,
...
static GList *
real_get_file_list (PeonyDirectory *directory)
{
    ...
    peony_file_list_ref (non_tentative_files);
    return non_tentative_files;
}

static gboolean
real_is_editable (PeonyDirectory *directory)
{
    return TRUE;
}

gboolean
peony_directory_is_editable (PeonyDirectory *directory)
{
    return EEL_CALL_METHOD_WITH_RETURN_VALUE
           (PEONY_DIRECTORY_CLASS, directory,
...
GList *
peony_directory_match_pattern (PeonyDirectory *directory, const char *pattern)
{
    GList *files, *l, *ret;
    GPatternSpec *spec;
    ...
    for (l = files; l; l = l->next)
    {
        ...
        {
            ret = g_list_prepend(ret, peony_file_ref (file));
        }
...
/**
 * peony_directory_list_ref
 *
 * Ref all the directories in a list.
 * @list: GList of directories.
 **/
GList *
peony_directory_list_ref (GList *list)
{
    g_list_foreach (list, (GFunc) peony_directory_ref, NULL);
    return list;
}

/**
 * peony_directory_list_unref
 *
 * Unref all the directories in a list.
 * @list: GList of directories.
 **/
void
peony_directory_list_unref (GList *list)
{
    g_list_foreach (list, (GFunc) peony_directory_unref, NULL);
}

/**
 * peony_directory_list_free
 *
 * Free a list of directories after unrefing them.
 * @list: GList of directories.
 **/
void
peony_directory_list_free (GList *list)
{
    peony_directory_list_unref (list);
    g_list_free (list);
}

/**
 * peony_directory_list_copy
 *
 * Copy the list of directories, making a new ref of each,
 * @list: GList of directories.
 **/
GList *
peony_directory_list_copy (GList *list)
{
    return g_list_copy (peony_directory_list_ref (list));
}

static int
compare_by_uri (PeonyDirectory *a, PeonyDirectory *b)
{
    char *uri_a, *uri_b;
    int res;
...
{
    return compare_by_uri (PEONY_DIRECTORY (a), PEONY_DIRECTORY (b));
}

/**
 * peony_directory_list_sort_by_uri
 *
 * Sort the list of directories by directory uri.
 * @list: GList of directories.
 **/
GList *
peony_directory_list_sort_by_uri (GList *list)
{
    return g_list_sort (list, compare_by_uri_cover);
}

gboolean
peony_directory_is_desktop_directory (PeonyDirectory   *directory)
{
    if (directory->details->location == NULL)
    {
        return FALSE;
    ...
    return peony_is_desktop_directory (directory->details->location);
}
...
static void
got_files_callback (PeonyDirectory *directory, GList *files, gpointer callback_data)
{
    g_assert (PEONY_IS_DIRECTORY (directory));
...
/* Return the number of extant PeonyDirectories */
int
peony_directory_number_outstanding (void)
{
    return directories ? g_hash_table_size (directories) : 0;
}

void
peony_self_check_directory (void)
{
    PeonyDirectory *directory;
    PeonyFile *file;

    directory = peony_directory_get_by_uri ("file:///etc");
    file = peony_file_get_by_uri ("file:///etc/passwd");
    ...
    peony_file_set_metadata (file, "test", "default", "value");
    EEL_CHECK_STRING_RESULT (peony_file_get_metadata (file, "test", "default"), "value");

    peony_file_set_boolean_metadata (file, "test_boolean", TRUE, TRUE);
    EEL_CHECK_BOOLEAN_RESULT (peony_file_get_boolean_metadata (file, "test_boolean", TRUE), TRUE);
    peony_file_set_boolean_metadata (file, "test_boolean", TRUE, FALSE);
    EEL_CHECK_BOOLEAN_RESULT (peony_file_get_boolean_metadata (file, "test_boolean", TRUE), FALSE);
    EEL_CHECK_BOOLEAN_RESULT (peony_file_get_boolean_metadata (NULL, "test_boolean", TRUE), TRUE);

    peony_file_set_integer_metadata (file, "test_integer", 0, 17);
    EEL_CHECK_INTEGER_RESULT (peony_file_get_integer_metadata (file, "test_integer", 0), 17);
    peony_file_set_integer_metadata (file, "test_integer", 0, -1);
    EEL_CHECK_INTEGER_RESULT (peony_file_get_integer_metadata (file, "test_integer", 0), -1);
    peony_file_set_integer_metadata (file, "test_integer", 42, 42);
    EEL_CHECK_INTEGER_RESULT (peony_file_get_integer_metadata (file, "test_integer", 42), 42);
    EEL_CHECK_INTEGER_RESULT (peony_file_get_integer_metadata (NULL, "test_integer", 42), 42);
    EEL_CHECK_INTEGER_RESULT (peony_file_get_integer_metadata (file, "nonexistent_key", 42), 42);
    */

    EEL_CHECK_BOOLEAN_RESULT (peony_directory_get_by_uri ("file:///etc") == directory, TRUE);
    peony_directory_unref (directory);
    ...
    peony_directory_file_monitor_remove (directory, &data_dummy);
    ...
    peony_directory_call_when_ready (directory,
