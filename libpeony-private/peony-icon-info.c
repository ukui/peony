...
#include "peony-default-file-icon.h"
#include <gtk/gtk.h>
#include <gio/gio.h>

struct _PeonyIconInfo
{
    GObject parent;
...
G_DEFINE_TYPE (PeonyIconInfo,
               peony_icon_info,
               G_TYPE_OBJECT);

static void
peony_icon_info_init (PeonyIconInfo *icon)
{
...
gboolean
peony_icon_info_is_fallback (PeonyIconInfo  *icon)
{
...
static void
peony_icon_info_finalize (GObject *object)
{
    PeonyIconInfo *icon;

    icon = PEONY_ICON_INFO (object);
    ...
    G_OBJECT_CLASS (peony_icon_info_parent_class)->finalize (object);
}

static void
peony_icon_info_class_init (PeonyIconInfoClass *icon_info_class)
{
    GObjectClass *gobject_class;

    gobject_class = (GObjectClass *) icon_info_class;

    gobject_class->finalize = peony_icon_info_finalize;

}

PeonyIconInfo *
peony_icon_info_new_for_pixbuf (GdkPixbuf *pixbuf)
{
    PeonyIconInfo *icon;

    icon = g_object_new (PEONY_TYPE_ICON_INFO, NULL);
...
peony_icon_info_new_for_icon_info (GtkIconInfo *icon_info)
{
    PeonyIconInfo *icon;
    GdkPoint *points;
    gint n_points;
...
void
peony_icon_info_clear_caches (void)
{
    if (loadable_icon_cache)
    {
...
PeonyIconInfo *
peony_icon_info_lookup (GIcon *icon,
                       int size)
{
    ...
    if (G_IS_LOADABLE_ICON (icon))
    {
        ...
        icon_info = peony_icon_info_new_for_pixbuf (pixbuf);
    ...
    else if (G_IS_THEMED_ICON (icon))
    {
        ...
        {
            return peony_icon_info_new_for_pixbuf (NULL);
        }
        ...
        icon_info = peony_icon_info_new_for_icon_info (gtkicon_info);
...
PeonyIconInfo *
peony_icon_info_lookup_from_name (const char *name,
                                 int size)
{
    GIcon *icon;
    PeonyIconInfo *info;
...
PeonyIconInfo *
peony_icon_info_lookup_from_path (const char *path,
                                 int size)
{
    GFile *icon_file;
    GIcon *icon;
...
GdkPixbuf *
peony_icon_info_get_pixbuf_nodefault (PeonyIconInfo  *icon)
{
    GdkPixbuf *res;
...
GdkPixbuf *
peony_icon_info_get_pixbuf (PeonyIconInfo *icon)
{
    GdkPixbuf *res;

    res = peony_icon_info_get_pixbuf_nodefault (icon);
    if (res == NULL)
    {
        res = gdk_pixbuf_new_from_data (peony_default_file_icon,
                                        GDK_COLORSPACE_RGB,
                                        TRUE,
                                        8,
                                        peony_default_file_icon_width,
                                        peony_default_file_icon_height,
                                        peony_default_file_icon_width * 4, /* stride */
...
GdkPixbuf *
peony_icon_info_get_pixbuf_nodefault_at_size (PeonyIconInfo  *icon,
        gsize              forced_size)
{
    ...
    pixbuf = peony_icon_info_get_pixbuf_nodefault (icon);
...
GdkPixbuf *
peony_icon_info_get_pixbuf_at_size (PeonyIconInfo  *icon,
                                   gsize              forced_size)
{
    GdkPixbuf *pixbuf, *scaled_pixbuf;
    int w, h, s;
...
gboolean
peony_icon_info_get_embedded_rect (PeonyIconInfo  *icon,
...
gboolean
peony_icon_info_get_attach_points (PeonyIconInfo  *icon,
                                  GdkPoint         **points,
                                  gint              *n_points)
{
 ...
 *
 * Return value: icon size between PEONY_ICON_SIZE_SMALLEST and
 * PEONY_ICON_SIZE_LARGEST, inclusive.
 */
guint
peony_get_icon_size_for_zoom_level (PeonyZoomLevel zoom_level)
{
    switch (zoom_level)
    {
    case PEONY_ZOOM_LEVEL_SMALLEST:
        return PEONY_ICON_SIZE_SMALLEST;
    case PEONY_ZOOM_LEVEL_SMALLER:
        return PEONY_ICON_SIZE_SMALLER;
    case PEONY_ZOOM_LEVEL_SMALL:
        return PEONY_ICON_SIZE_SMALL;
    ...
    case PEONY_ZOOM_LEVEL_LARGER:
        return PEONY_ICON_SIZE_LARGER;
    case PEONY_ZOOM_LEVEL_LARGEST:
        return PEONY_ICON_SIZE_LARGEST;
    }
    g_return_val_if_reached (PEONY_ICON_SIZE_STANDARD);
}

float
peony_get_relative_icon_size_for_zoom_level (PeonyZoomLevel zoom_level)
{
    return (float)peony_get_icon_size_for_zoom_level (zoom_level) / PEONY_ICON_SIZE_STANDARD;
}

guint
peony_icon_get_larger_icon_size (guint size)
{
    if (size < PEONY_ICON_SIZE_SMALLEST)
    {
        return PEONY_ICON_SIZE_SMALLEST;
    }
    if (size < PEONY_ICON_SIZE_SMALLER)
    {
        return PEONY_ICON_SIZE_SMALLER;
    }
    if (size < PEONY_ICON_SIZE_SMALL)
    {
        return PEONY_ICON_SIZE_SMALL;
    }
    ...
    if (size < PEONY_ICON_SIZE_LARGE)
    {
        return PEONY_ICON_SIZE_LARGE;
    }
    if (size < PEONY_ICON_SIZE_LARGER)
    {
        return PEONY_ICON_SIZE_LARGER;
    }
    return PEONY_ICON_SIZE_LARGEST;
...
guint
peony_icon_get_smaller_icon_size (guint size)
{
    if (size > PEONY_ICON_SIZE_LARGEST)
    {
        return PEONY_ICON_SIZE_LARGEST;
    }
    if (size > PEONY_ICON_SIZE_LARGER)
...
gint
peony_get_icon_size_for_stock_size (GtkIconSize size)
{
    gint w, h;
...
guint
peony_icon_get_emblem_size_for_icon_size (guint size)
{
    if (size >= 96)
        return 48;
    if (size >= 64)
...
gboolean
peony_icon_theme_can_render (GThemedIcon *icon)
{
	GtkIconTheme *icon_theme;
...
GIcon *
peony_user_special_directory_get_gicon (GUserDirectory directory)
{

	#define ICON_CASE(x) \
		case G_USER_DIRECTORY_ ## x:\
			return g_themed_icon_new (PEONY_ICON_FOLDER_ ## x);
...
float peony_get_desktop_icon_width_size_for_zoom_level (PeonyZoomLevel zoom_level)
{
	...
	*/	
    ...
    case PEONY_ZOOM_LEVEL_SMALLER:
    case PEONY_ZOOM_LEVEL_SMALL:
        return (PEONY_DESKTOP_ICON_GRID_WIDTH_SMALL);
    case PEONY_ZOOM_LEVEL_STANDARD:
        return (PEONY_DESKTOP_ICON_GRID_WIDTH_STANDARD);
    case PEONY_ZOOM_LEVEL_LARGE:
        return (PEONY_DESKTOP_ICON_GRID_WIDTH_LARGE);
    case PEONY_ZOOM_LEVEL_LARGER:
        return (PEONY_DESKTOP_ICON_GRID_WIDTH_LARGER);
    case PEONY_ZOOM_LEVEL_LARGEST:
        return (PEONY_DESKTOP_ICON_GRID_WIDTH_LARGEST);
    }
    g_return_val_if_reached ((PEONY_DESKTOP_ICON_GRID_WIDTH_STANDARD));
}
...
float peony_get_desktop_icon_height_size_for_zoom_level (PeonyZoomLevel zoom_level)
{
	...
	*/	
    ...
    case PEONY_ZOOM_LEVEL_SMALLER:
    case PEONY_ZOOM_LEVEL_SMALL:
        return (PEONY_DESKTOP_ICON_GRID_HEIGHT_SMALL);
    case PEONY_ZOOM_LEVEL_STANDARD:
        return (PEONY_DESKTOP_ICON_GRID_HEIGHT_STANDARD);
    case PEONY_ZOOM_LEVEL_LARGE:
        return (PEONY_DESKTOP_ICON_GRID_HEIGHT_LARGE);
    case PEONY_ZOOM_LEVEL_LARGER:
        return (PEONY_DESKTOP_ICON_GRID_HEIGHT_LARGER);
    case PEONY_ZOOM_LEVEL_LARGEST:
        return (PEONY_DESKTOP_ICON_GRID_HEIGHT_LARGEST);
    }
    g_return_val_if_reached ((PEONY_DESKTOP_ICON_GRID_HEIGHT_STANDARD));
}
