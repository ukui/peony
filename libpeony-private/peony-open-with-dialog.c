...
struct _PeonyOpenWithDialogDetails
{
...
G_DEFINE_TYPE (PeonyOpenWithDialog, peony_open_with_dialog, GTK_TYPE_DIALOG);

static void
peony_open_with_dialog_finalize (GObject *object)
{
    PeonyOpenWithDialog *dialog;

    dialog = PEONY_OPEN_WITH_DIALOG (object);
    ...
    G_OBJECT_CLASS (peony_open_with_dialog_parent_class)->finalize (object);
}
 ...
 */
static gboolean
check_application (PeonyOpenWithDialog *dialog)
{
    char *command;
    char *path = NULL;
    char **argv = NULL;
 ...
 */
static GAppInfo *
add_or_find_application (PeonyOpenWithDialog *dialog)
{
    GAppInfo *app;
    char *app_name;
...
static void
emit_application_selected (PeonyOpenWithDialog *dialog,
...
static void
response_cb (PeonyOpenWithDialog *dialog,
             int response_id,
             gpointer data)
{
    GAppInfo *application;
...
static void
peony_open_with_dialog_class_init (PeonyOpenWithDialogClass *class)
{
    GObjectClass *gobject_class;

    gobject_class = G_OBJECT_CLASS (class);
    gobject_class->finalize = peony_open_with_dialog_finalize;
                     ...
                     gpointer user_data)
{
    ...
    dialog = PEONY_OPEN_WITH_DIALOG (user_data);
...
static gboolean
peony_open_with_dialog_add_icon_idle (PeonyOpenWithDialog *dialog)
{
    GtkTreeIter   iter;
    GtkTreePath  *path;
    GdkPixbuf    *pixbuf;
...
static gboolean
peony_open_with_search_equal_func (GtkTreeModel *model,
                                  int column,
                                  const char *key,
                                  GtkTreeIter *iter,
                                  gpointer user_data)
...
static gboolean
peony_open_with_dialog_add_items_idle (PeonyOpenWithDialog *dialog)
{
    ...
    gtk_tree_view_set_search_equal_func (GTK_TREE_VIEW (dialog->details->program_list),
                                         peony_open_with_search_equal_func,
                                         NULL, NULL);
    ...
    if (!dialog->details->add_icons_idle_id)
    {
        dialog->details->add_icons_idle_id =
            g_idle_add_full (G_PRIORITY_DEFAULT_IDLE, (GSourceFunc) peony_open_with_dialog_add_icon_idle,
                             dialog, NULL);
    }
...
static void
expander_toggled (GtkWidget *expander, PeonyOpenWithDialog *dialog)
{
...
static void
peony_open_with_dialog_init (PeonyOpenWithDialog *dialog)
{
    GtkWidget *hbox;
    GtkWidget *vbox;
    GtkWidget *vbox2;
    ...
    dialog->details = g_new0 (PeonyOpenWithDialogDetails, 1);
    ...
    dialog->details->add_items_idle_id = g_idle_add_full (G_PRIORITY_DEFAULT_IDLE,
                                         (GSourceFunc) peony_open_with_dialog_add_items_idle,
                                         dialog, NULL);
...
static void
set_uri_and_type (PeonyOpenWithDialog *dialog,
                  const char *uri,
...
static GtkWidget *
real_peony_open_with_dialog_new (const char *uri,
                                const char *mime_type,
                                ...
                                gboolean add_mode)
{
    ...
    dialog = gtk_widget_new (PEONY_TYPE_OPEN_WITH_DIALOG, NULL);

    set_uri_and_type (PEONY_OPEN_WITH_DIALOG (dialog), uri, mime_type, extension, add_mode);

    return dialog;
}

GtkWidget *
peony_open_with_dialog_new (const char *uri,
                           const char *mime_type,
                           const char *extension)
{
    return real_peony_open_with_dialog_new (uri, mime_type, extension, FALSE);
}

GtkWidget *
peony_add_application_dialog_new (const char *uri,
                                 const char *mime_type)
{
    PeonyOpenWithDialog *dialog;

    dialog = PEONY_OPEN_WITH_DIALOG (real_peony_open_with_dialog_new (uri, mime_type, NULL, TRUE));

    return GTK_WIDGET (dialog);
}

GtkWidget *
peony_add_application_dialog_new_for_multiple_files (const char *extension,
        const char *mime_type)
{
    PeonyOpenWithDialog *dialog;

    dialog = PEONY_OPEN_WITH_DIALOG (real_peony_open_with_dialog_new (NULL, mime_type, extension, TRUE));
