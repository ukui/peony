...
/* peony-mime-actions.c - uri-specific versions of mime action functions
...
#include "peony-program-choosing.h"
...
typedef struct
{
    ...
    PeonyFileListHandle *files_handle;
...
{
    peony_file_unref (location->file);
    g_free (location->uri);
    g_free (location);
}
...
launch_location_from_file (PeonyFile *file)
{
    LaunchLocation *location;
    location = g_new (LaunchLocation, 1);
    location->file = peony_file_ref (file);
    location->uri = peony_file_get_uri (file);
                                 ...
                                 const char *uri)
{
    peony_file_unref (location->file);
    g_free (location->uri);
    location->file = peony_file_get_by_uri (uri);
...
static GList*
filter_peony_handler (GList *apps)
{
    GList *l, *next;
    GAppInfo *application;
    const char *id;
    ...
    while (l != NULL)
    {
        ...
        if (id != NULL &&
                strcmp (id,
                        "peony-folder-handler.desktop") == 0)
        {
...
static gboolean
peony_mime_actions_check_if_required_attributes_ready (PeonyFile *file)
{
    PeonyFileAttributes attributes;
    gboolean ready;

    attributes = peony_mime_actions_get_required_file_attributes ();
    ready = peony_file_check_if_ready (file, attributes);

    return ready;
}

PeonyFileAttributes
peony_mime_actions_get_required_file_attributes (void)
{
...
static gboolean
file_has_local_path (PeonyFile *file)
{
    GFile *location;
    char *path;
    gboolean res;
...
GAppInfo *
peony_mime_get_default_application_for_file (PeonyFile *file)
{
    GAppInfo *app;
    char *mime_type;
    char *uri_scheme;

    if (!peony_mime_actions_check_if_required_attributes_ready (file))
    {
        return NULL;
    }
    ...
    if (app == NULL)
    {
        uri_scheme = peony_file_get_uri_scheme (file);
        if (uri_scheme != NULL)
        {
...
static int
file_compare_by_mime_type (PeonyFile *file_a,
                           PeonyFile *file_b)
{
    char *mime_type_a, *mime_type_b;
    int ret;

    mime_type_a = peony_file_get_mime_type (file_a);
    mime_type_b = peony_file_get_mime_type (file_b);
...
static int
file_compare_by_parent_uri (PeonyFile *file_a,
                            PeonyFile *file_b)
{
    char *parent_uri_a, *parent_uri_b;
    int ret;

    parent_uri_a = peony_file_get_parent_uri (file_a);
    parent_uri_b = peony_file_get_parent_uri (file_b);
...
GList *
peony_mime_get_applications_for_file (PeonyFile *file)
{
    char *mime_type;
    char *uri_scheme;
    GList *result;
    ...
    return filter_peony_handler (result);
}

gboolean
peony_mime_has_any_applications_for_file (PeonyFile *file)
{
    GList *apps;
    char *mime_type;
    gboolean result;
    ...
    apps = filter_peony_handler (apps);
...
GAppInfo *
peony_mime_get_default_application_for_files (GList *files)
{
    ...
    for (l = sorted_files; l != NULL; l = l->next)
    {
        ...
        one_app = peony_mime_get_default_application_for_file (file);
...
GList *
peony_mime_get_applications_for_files (GList *files)
{
    GList *l, *sorted_files;
    PeonyFile *file;
    GList *one_ret, *ret;
    ...
    for (l = sorted_files; l != NULL; l = l->next)
    {
        ...
        one_ret = peony_mime_get_applications_for_file (file);
...
static void
report_broken_symbolic_link (GtkWindow *parent_window, PeonyFile *file)
{
    char *target_path;
    char *display_name;
    char *prompt;
    ...
    target_path = peony_file_get_symbolic_link_target_path (file);
    if (target_path == NULL)
    {
     ...
     * merged UkuiComponent items not to be un-merged. Maybe we need to unmerge
     * explicitly when disconnecting views instead of relying on the
     * unmerge in Destroy. But since UkuiComponentUIHandler is probably going
...
get_executable_text_file_action (GtkWindow *parent_window, PeonyFile *file)
{
    ...
    g_assert (peony_file_contains_text (file));

    preferences_value = g_settings_get_enum (peony_preferences,
                            PEONY_PREFERENCES_EXECUTABLE_TEXT_ACTIVATION);
    switch (preferences_value)
    {
    case PEONY_EXECUTABLE_TEXT_LAUNCH:
        return ACTIVATION_ACTION_LAUNCH;
    case PEONY_EXECUTABLE_TEXT_DISPLAY:
        return ACTIVATION_ACTION_OPEN_IN_APPLICATION;
    case PEONY_EXECUTABLE_TEXT_ASK:
        break;
    default:
        /* Complain non-fatally, since preference data can't be trusted */
        g_warning ("Unknown value %d for PEONY_PREFERENCES_EXECUTABLE_TEXT_ACTIVATION",
                   preferences_value);
...
get_activation_action (PeonyFile *file)
{
    ...
    if (peony_file_is_launchable (file))
    {
        ...
        else if (peony_file_contains_text (file))
        {
    ...
    {
        if (peony_mime_file_opens_in_view (file))
        {
...
gboolean
peony_mime_file_opens_in_external_app (PeonyFile *file)
{
 ...
 *
 * Construct a list of ApplicationLaunchParameters from a list of PeonyFiles,
                            ...
                            GList **unhandled_uris)
{
    ...
    *unhandled_uris = NULL;
    ...
    for (l = uris; l != NULL; l = l->next)
    {
        ...
        app = peony_mime_get_default_application_for_file (file);
        if (app != NULL)
        {
            app_uris = NULL;
...
static gboolean
file_was_cancelled (PeonyFile *file)
{
    GError *error;
...
static gboolean
file_was_not_mounted (PeonyFile *file)
{
    GError *error;
...
activation_parameters_free (ActivateParameters *parameters)
{
    ...
    peony_file_list_free (parameters->mountables);
    peony_file_list_free (parameters->start_mountables);
    peony_file_list_free (parameters->not_mounted);
...
cancel_activate_callback (gpointer callback_data)
{
    ...
    {
        peony_file_list_cancel_call_when_ready (parameters->files_handle);
...
activate_parameters_install_free (ActivateParametersInstall *parameters_install)
{
    ...
    peony_file_unref (parameters_install->file);
    peony_file_list_free (parameters_install->files);
...
static char *
get_application_no_mime_type_handler_message (PeonyFile *file, char *uri)
{
...
choose_program (GtkDialog *message_dialog, int response, gpointer callback_data)
{
    ...
    g_object_set_data_full (G_OBJECT (dialog),
                            "mime-action:file",
                            peony_file_ref (file),
                            (GDestroyNotify)peony_file_unref);
...
show_unhandled_type_error (ActivateParametersInstall *parameters)
{
    ...
    char *mime_type = peony_file_get_mime_type (parameters->file);
    ...
    g_object_set_data_full (G_OBJECT (dialog),
                            "mime-action:file",
                            peony_file_ref (parameters->file),
                            (GDestroyNotify)peony_file_unref);
        ...
        gpointer      user_data)
{
    ...
    peony_mime_activate_files (parameters_install->parent_window,
                                    ...
                                    ActivateParametersInstall *parameters_install)
{
    ...
    {
        mime_type = peony_file_get_mime_type (parameters_install->file);
...
application_unhandled_uri (ActivateParameters *parameters, char *uri)
{
    ...
    show_install_mime = g_settings_get_boolean (peony_preferences, PEONY_PREFERENCES_INSTALL_MIME_ACTIVATION);
#else
...
activate_parameters_desktop_free (ActivateParametersDesktop *parameters_desktop)
{
    ...
    peony_file_unref (parameters_desktop->file);
                                      ...
                                      ActivateParametersDesktop *parameters)
{
    ...
    case RESPONSE_RUN:
        ...
        uri = peony_file_get_uri (parameters->file);
        peony_debug_log (FALSE, PEONY_DEBUG_LOG_DOMAIN_USER,
                        "directory view activate_callback launch_desktop_file window=%p: %s",
                        parameters->parent_window, uri);
        peony_launch_desktop_file (screen, uri, NULL,
                                  parameters->parent_window);
        g_free (uri);
        break;
    case RESPONSE_MARK_TRUSTED:
        file = peony_file_get_location (parameters->file);
        peony_file_mark_desktop_file_trusted (file,
                                             parameters->parent_window,
                                             TRUE,
                                             NULL, NULL);
        g_object_unref (file);
                       ...
                       PeonyFile *file)
{
    ...
    if (!peony_file_is_trusted_link (file))
    {
        ...
        parameters_desktop->file = peony_file_ref (file);
...
activate_files (ActivateParameters *parameters)
{
    ...
    for (l = launch_files; l != NULL; l = l->next)
    {
        ...
        name = peony_file_get_name (file);

        peony_debug_log (FALSE, PEONY_DEBUG_LOG_DOMAIN_USER,
                        "directory view activate_callback launch_file window=%p: %s",
                        parameters->parent_window, quoted_path);

        peony_launch_application_from_command (screen, name, quoted_path, FALSE, NULL);
        g_free (name);
        g_free (quoted_path);
        g_free (executable_path);
        g_free (uri);
    ...
    for (l = launch_in_terminal_files; l != NULL; l = l->next)
    {
        ...
        peony_launch_application_from_command (screen, name, quoted_path, TRUE, NULL);
        g_free (name);
        g_free (quoted_path);
        g_free (executable_path);
        g_free (uri);
    ...
    if (count > 1)
    {
        if ((parameters->flags & PEONY_WINDOW_OPEN_FLAG_NEW_WINDOW) == 0)
        {
                                       ...
                                       (flags & PEONY_WINDOW_OPEN_FLAG_NEW_TAB) != 0)))
    {
        ...
        if ((flags & PEONY_WINDOW_OPEN_FLAG_NEW_TAB) != 0 &&
                g_settings_get_enum (peony_preferences, PEONY_PREFERENCES_NEW_TAB_POSITION) ==
        ...
        for (l = open_in_view_files; l != NULL; l = l->next)
        {
            ...
            peony_window_slot_info_open_location (parameters->slot_info,
    ...
    for (l = open_in_app_parameters; l != NULL; l = l->next)
    {
        ...
        peony_launch_application_by_uri (one_parameters->application,
                                        one_parameters->uris,
                                        parameters->parent_window);
    ...
    if (parameters->slot_info != NULL)
    {
        window_info = peony_window_slot_info_get_window (parameters->slot_info);
    }

    if (open_in_app_parameters != NULL ||
            unhandled_open_in_app_uris != NULL)
    {
        if ((parameters->flags & PEONY_WINDOW_OPEN_FLAG_CLOSE_BEHIND) != 0 &&
                window_info != NULL &&
                peony_window_info_get_window_type (window_info) == PEONY_WINDOW_SPATIAL)
        {
            peony_window_info_close (window_info);
        }
    }
...
activation_mount_not_mounted (ActivateParameters *parameters)
{
    ...
    for (l = parameters->locations; l != NULL; l = next)
    {
        loc = l->data;
        next = l->next;
        peony_file_invalidate_all_attributes (loc->file);
    }

    files = get_file_list_for_launch_locations (parameters->locations);
    peony_file_list_call_when_ready
    (files,
     peony_mime_actions_get_required_file_attributes () | PEONY_FILE_ATTRIBUTE_LINK_INFO,
        ...
        gpointer callback_data)
{
    ...
    for (l = parameters->locations; l != NULL; l = next)
    {
        ...
        if (peony_file_get_file_type (file) == G_FILE_TYPE_MOUNTABLE &&
                !peony_file_has_activation_uri (file))
        {
    ...
    for (l = parameters->locations; l != NULL; l = l->next)
    {
         ...
         */
        uri = peony_file_get_activation_uri (location->file);
        if (uri != NULL)
        {
...
static void
activation_mountable_mounted (PeonyFile  *file,
                              ...
                              gpointer       callback_data)
{
    ...
    if (error == NULL)
    {
        ...
        target_file = peony_file_get (result_location);
...
static void
activation_mountable_started (PeonyFile  *file,
                              GFile         *gfile_of_file,
                              GError        *error,
                              gpointer       callback_data)
{
 ...
 * @files: A GList of PeonyFiles to activate.
 *
 **/
void
peony_mime_activate_files (GtkWindow *parent_window,
                          PeonyWindowSlotInfo *slot_info,
                          GList *files,
                          ...
                          gboolean user_confirmation)
{
    ...
    peony_debug_log_with_file_list (FALSE, PEONY_DEBUG_LOG_DOMAIN_USER, files,
                                   "peony_mime_activate_files window=%p",
                                   parent_window);
    ...
    if (file_count == 1)
    {
        file_name = peony_file_get_display_name (files->data);
 ...
 * @view: FMDirectoryView in question.
 * @file: A PeonyFile representing the file in this view to activate.
 * @use_new_window: Should this item be opened in a new window?
 *
 **/

void
peony_mime_activate_file (GtkWindow *parent_window,
                         PeonyWindowSlotInfo *slot_info,
                         PeonyFile *file,
                         const char *launch_directory,
                         PeonyWindowOpenMode mode,
                         PeonyWindowOpenFlags flags)
{
    ...
    peony_mime_activate_files (parent_window, slot_info, files, launch_directory, mode, flags, FALSE);
    g_list_free (files);
}
