...
static void
menu_provider_iface_init (PeonyMenuProviderIface *iface);

static GList*
gksu_context_menu_get_file_items (PeonyMenuProvider *provider,
				  GtkWidget *window,
				  GList *files);
static void
gksu_context_menu_activate (PeonyMenuItem *item,
			    PeonyFileInfo *file);
...
static void menu_provider_iface_init (PeonyMenuProviderIface *iface)
{
				  ...
				  GList *files)
{
    ...
    /* ... and if it is not a peony special item */
    {
      gchar *uri_scheme = NULL;

      uri_scheme = peony_file_info_get_uri_scheme (file);
      if (!strncmp (uri_scheme, "x-peony-desktop", 18))
	{
	  g_free (uri_scheme);
	  return NULL;
	}
    ...
    item = peony_menu_item_new ("Gksu::open_as_root",
