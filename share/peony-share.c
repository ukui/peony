...
typedef struct {
  ...
  PeonyFileInfo *fileinfo; /* Peony file to which this page refers */
...
message_confirm_missing_permissions (GtkWidget *widget, const char *path, mode_t need_mask)
{
  ...
  dialog = gtk_message_dialog_new (toplevel ? GTK_WINDOW (toplevel) : NULL,
				   ...
				   GTK_BUTTONS_NONE,
				   _("Peony needs to add some permissions to your folder \"%s\" in order to share it"),
				   display_name);
  ...
  gtk_message_dialog_format_secondary_text (GTK_MESSAGE_DIALOG (dialog),
					    _("The folder \"%s\" needs the following extra permissions for sharing to work:\n"
					      "%s%s%s"
					      "Do you want Peony to add these permissions to the folder automatically?"),
					    display_name,
...
property_page_commit (PropertyPage *page)
{
  ...
  if (!retval)
  ...
  else
    {
      ...
      peony_file_info_invalidate_extension_info (page->fileinfo);
    }
...
/*--------------------------------------------------------------------------*/
static gchar *
get_fullpath_from_fileinfo(PeonyFileInfo *fileinfo)
{
  GFile *file;
  gchar *fullpath;
  ...
  file = peony_file_info_get_location(fileinfo);
...
/*--------------------------------------------------------------------------*/
static PropertyPage *
create_property_page (PeonyFileInfo *fileinfo)
{
  PropertyPage *page;
  GError *error;
  ShareInfo *share_info;
...
/* Implementation of the PeonyInfoProvider interface */

/* peony_info_provider_update_file_info 
 * This function is called by Peony when it wants the extension to 
 * fill in data about the file.  It passes a PeonyFileInfo object,
 ...
 * the extension can do so, and return PEONY_OPERATION_COMPLETE.  
 ...
 * PEONY_OPERATION_IN_PROGRESS.  The function must also set the 
...
static PeonyShareStatus 
get_share_status_and_free_share_info (ShareInfo *share_info)
{
  PeonyShareStatus result;

  if (!share_info)
    result = PEONY_SHARE_NOT_SHARED;
  else
    {
      if (share_info->is_writable)
	result = PEONY_SHARE_SHARED_RW;
      else
	result = PEONY_SHARE_SHARED_RO;
...
/*--------------------------------------------------------------------------*/
static void
get_share_info_for_file_info (PeonyFileInfo *file, ShareInfo **share_info, gboolean *is_shareable)
{
  char		*uri;
  ...
  *is_shareable = FALSE;
  ...
  f = peony_file_info_get_location(file);
  if (!uri)
    goto out;
...
#define NETWORK_SHARE_PREFIX "network:///share-"
  ...
  if (!peony_file_info_is_directory(file))
    goto out;
...
/*--------------------------------------------------------------------------*/
static PeonyShareStatus 
file_get_share_status_file(PeonyFileInfo *file)
{
  ShareInfo *share_info;
  ...
  if (!is_shareable)
    return PEONY_SHARE_NOT_SHARED;

  return get_share_status_and_free_share_info (share_info);
}

static PeonyOperationResult
peony_share_update_file_info (PeonyInfoProvider *provider,
				 PeonyFileInfo *file,
				 GClosure *update_complete,
...
/*   gchar *share_status = NULL; */
  ...
  case PEONY_SHARE_SHARED_RO:
    peony_file_info_add_emblem (file, "shared");
/*     share_status = _("shared (read only)"); */
    break;

  case PEONY_SHARE_SHARED_RW:
    peony_file_info_add_emblem (file, "shared");
/*     share_status = _("shared (read and write)"); */
    break;

  case PEONY_SHARE_NOT_SHARED:
...
/*   peony_file_info_add_string_attribute (file, */
/* 					   "PeonyShare::share_status", */
/* 					   share_status); */
...
static void
peony_share_cancel_update (PeonyInfoProvider *provider,
			      PeonyOperationHandle *handle)
{
  PeonyShareHandle *share_handle;
	
  share_handle = (PeonyShareHandle*)handle;
  share_handle->cancelled = TRUE;
}

static void 
peony_share_info_provider_iface_init (PeonyInfoProviderIface *iface)
{
  iface->update_file_info = peony_share_update_file_info;
  iface->cancel_update = peony_share_cancel_update;
}
 ...
 * 
 * The function should return a GList of allocated PeonyPropertyPage
 * items.
 */
static GList *
peony_share_get_property_pages (PeonyPropertyPageProvider *provider,
				   GList *files)
{
  PropertyPage *page;
  GList *pages;
  PeonyPropertyPage *np_page;
  ...
  fileinfo = PEONY_FILE_INFO (files->data);
  ...
  pages = NULL;
  np_page = peony_property_page_new
    ("PeonyShare::property_page",
...
/*--------------------------------------------------------------------------*/
static void 
peony_share_property_page_provider_iface_init (PeonyPropertyPageProviderIface *iface)
{
  iface->get_pages = peony_share_get_property_pages;
}

/*--------------------------------------------------------------------------*/
static void
peony_share_instance_init (PeonyShare *share)
{
}

/*--------------------------------------------------------------------------*/
static void
peony_share_class_init (PeonyShareClass *class)
{
 ...
 *  
 * This function is called by Peony when it wants context menu
 * items from the extension.
 *
 * This function is called in the main thread before a context menu
 * is shown, so it should return quickly.
 * 
 * The function should return a GList of allocated PeonyMenuItem
 * items.
 */
...
static void
share_this_folder_callback (PeonyMenuItem *item,
			    gpointer user_data)
{
  PeonyFileInfo *fileinfo;
  PropertyPage *page;
  ...
  fileinfo = PEONY_FILE_INFO (user_data);
  g_assert (fileinfo != NULL);
...
static GList *
peony_share_get_file_items (PeonyMenuProvider *provider,
			     GtkWidget *window,
			     GList *files)
{
  GList *items;
  ...
  item = peony_menu_item_new ("PeonyShare::share",
				 _("Sharing Options"),
				 _("Share this Folder"),
				 "folder-remote");
...
/*--------------------------------------------------------------------------*/
static void 
peony_share_menu_provider_iface_init (PeonyMenuProviderIface *iface)
{
	iface->get_file_items = peony_share_get_file_items;
}
...
#define PEONY_TYPE_SHARE  (peony_share_get_type ())

static GType
peony_share_get_type (void) 
{
  return share_type;
}

static void
peony_share_register_type (GTypeModule *module)
{
  static const GTypeInfo info = {
    sizeof (PeonyShareClass),
    (GBaseInitFunc) NULL,
    (GBaseFinalizeFunc) NULL,
    (GClassInitFunc) peony_share_class_init,
    NULL, 
    NULL,
    sizeof (PeonyShare),
    0,
    (GInstanceInitFunc) peony_share_instance_init,
  };
  ...
  static const GInterfaceInfo property_page_provider_iface_info = {
    (GInterfaceInitFunc) peony_share_property_page_provider_iface_init,
    NULL,
    NULL
  };
  ...
  static const GInterfaceInfo info_provider_iface_info = {
    (GInterfaceInitFunc) peony_share_info_provider_iface_init,
    NULL,
    NULL
  };
  ...
  static const GInterfaceInfo menu_provider_iface_info = {
    (GInterfaceInitFunc) peony_share_menu_provider_iface_init,
    NULL,
    NULL
  };
 ...
 * peony-extensions-types.h, and must be implemented by all 
 * extensions. */
...
{
  /*g_print ("Initializing peony-share extension\n");*/

  bindtextdomain(GETTEXT_PACKAGE, MATELOCALEDIR);
  bind_textdomain_codeset(GETTEXT_PACKAGE, "UTF-8");

  peony_share_register_type (module);
}
...
{
  /*g_print ("Shutting down peony-share extension\n");*/
  /* FIXME freeing */
}
