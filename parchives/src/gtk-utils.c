...
#define PARCHIVES_RESOURCE_UI_PATH "/org/mate/Parchives/ui/"
		  ...
		  const char *section)
{
	...
	uri = g_strconcat ("help:parchives", section ? "/" : NULL, section, NULL);
...
_gtk_builder_new_from_resource (const char *resource_path)
{
	...
	builder = gtk_builder_new ();
	full_path = g_strconcat (PARCHIVES_RESOURCE_UI_PATH, resource_path, NULL);
