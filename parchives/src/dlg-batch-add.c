...
help_clicked_cb (GtkWidget  *widget,
		 DialogData *data)
{
	show_help_dialog (GTK_WINDOW (GET_WIDGET ("dialog")), "parchives-fmgr-add");
}
		     ...
		     GList    *file_list)
{
	...
	data->settings = g_settings_new (PARCHIVES_SCHEMA_BATCH_ADD);
	data->settings_general = g_settings_new (PARCHIVES_SCHEMA_GENERAL);
