...
#define COT_SCHEMA "org.mate.peony-open-terminal"
#define COT_DESKTOP_KEY "desktop-opens-home-dir"
#define PEONY_SCHEMA "org.ukui.peony.preferences"
#define PEONY_DESKTOP_KEY "desktop-is-home-dir"
#define TERM_SCHEMA "org.mate.applications-terminal"
#define TERM_EXEC_KEY "exec"

static void peony_open_terminal_instance_init (PeonyOpenTerminal      *cvs);
static void peony_open_terminal_class_init    (PeonyOpenTerminalClass *class);
...
get_terminal_file_info (PeonyFileInfo *file_info)
{
	TerminalFileInfo  ret;
	char             *uri;
	char             *uri_scheme;

	uri = peony_file_info_get_activation_uri (file_info);
	uri_scheme = g_uri_parse_scheme (uri);

	if (strcmp (uri_scheme, "file") == 0) {
		ret = FILE_INFO_LOCAL;
	} else if (strcmp (uri_scheme, "x-peony-desktop") == 0) {
...
desktop_is_home_dir (void)
{
	...
	settings = g_settings_new (PEONY_SCHEMA);
	result = g_settings_get_boolean (settings, PEONY_DESKTOP_KEY);
	g_object_unref (settings);
	return result;
}
		  ...
		  PeonyFileInfo *file_info)
{
	...
	vfs_uri = g_file_new_for_uri (peony_file_info_get_activation_uri (file_info));
...
static void
open_terminal_callback (PeonyMenuItem *item,
			PeonyFileInfo *file_info)
{
	gchar *display_str;
	...
	screen = g_object_get_data (G_OBJECT (item), "PeonyOpenTerminal::screen");
	if (screen != NULL) {
	...
	if (dfile != NULL) {
		...
		if (orig_cwd == -1) {
			g_message ("PeonyOpenTerminal: Failed to open current Peony working directory.");
		} else if (working_directory != NULL) {

			if (chdir (working_directory) == -1) {
				int ret;

				g_message ("PeonyOpenTerminal: Failed to change Peony working directory to \"%s\".",
					   working_directory);
				...
				if (ret == -1) {
					g_message ("PeonyOpenTerminal: Failed to close() current Peony working directory.");
				}
		...
		if (orig_cwd != -1) {
			...
			ret = fchdir (orig_cwd);
			if (ret == -1) {
				g_message ("PeonyOpenTerminal: Failed to change back Peony working directory to original location after changing it to \"%s\".",
					   working_directory);
			}
			...
			if (ret == -1) {
				g_message ("PeonyOpenTerminal: Failed to close Peony working directory.");
			}
		}
	} else {	
...
static PeonyMenuItem *
open_terminal_menu_item_new (PeonyFileInfo	  *file_info,
                             ...
                             gboolean          is_file_item)
{
	...
	ret = peony_menu_item_new ("PeonyOpenTerminal::open_terminal",
				      name, tooltip, "terminal");

	g_object_set_data (G_OBJECT (ret),
			   "PeonyOpenTerminal::screen",
			   screen);
...
static GList *
peony_open_terminal_get_background_items (PeonyMenuProvider *provider,
                                         GtkWidget        *window,
                                         PeonyFileInfo     *file_info)
{
	PeonyMenuItem *item;
...
GList *
peony_open_terminal_get_file_items (PeonyMenuProvider *provider,
                                   GtkWidget        *window,
                                   GList            *files)
{
	PeonyMenuItem *item;
	...
	if (g_list_length (files) != 1 ||
	    (!peony_file_info_is_directory (files->data) &&
	     peony_file_info_get_file_type (files->data) != G_FILE_TYPE_SHORTCUT &&
	     peony_file_info_get_file_type (files->data) != G_FILE_TYPE_MOUNTABLE)) {
		return NULL;
	}
...
static void
peony_open_terminal_menu_provider_iface_init (PeonyMenuProviderIface *iface)
{
	iface->get_background_items = peony_open_terminal_get_background_items;
	iface->get_file_items = peony_open_terminal_get_file_items;
}

static void 
peony_open_terminal_instance_init (PeonyOpenTerminal *cvs)
{
}

static void
peony_open_terminal_class_init (PeonyOpenTerminalClass *class)
{
}

GType
peony_open_terminal_get_type (void) 
{
	return terminal_type;
}

void
peony_open_terminal_register_type (GTypeModule *module)
{
	static const GTypeInfo info = {
		sizeof (PeonyOpenTerminalClass),
		(GBaseInitFunc) NULL,
		(GBaseFinalizeFunc) NULL,
		(GClassInitFunc) peony_open_terminal_class_init,
		NULL, 
		NULL,
		sizeof (PeonyOpenTerminal),
		0,
		(GInstanceInitFunc) peony_open_terminal_instance_init,
	};

	static const GInterfaceInfo menu_provider_iface_info = {
		(GInterfaceInitFunc) peony_open_terminal_menu_provider_iface_init,
		NULL,
		NULL
	};
