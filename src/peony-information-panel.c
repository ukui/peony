...
#include <libpeony-private/peony-keep-last-vertical-box.h>
...
struct PeonyInformationPanelDetails
{
    GtkWidget *container;
    PeonyWindowInfo *window;
    PeonySidebarTitle *title;
...
static gboolean peony_information_panel_press_event           (GtkWidget                    *widget,
        GdkEventButton               *event);
static void     peony_information_panel_finalize              (GObject                      *object);
static void     peony_information_panel_drag_data_received    (GtkWidget                    *widget,
        ...
        guint                         info,
        guint                         time);
static void     peony_information_panel_read_defaults         (PeonyInformationPanel     *information_panel);
#if GTK_CHECK_VERSION (3, 0, 0)
static void     peony_information_panel_style_updated         (GtkWidget                    *widget);
#else
static void     peony_information_panel_style_set             (GtkWidget                    *widget,
        GtkStyle                     *previous_style);
#endif
static void     peony_information_panel_theme_changed         (GSettings   *settings,
                                                              const gchar *key,
                                                              gpointer     user_data);
static void     peony_information_panel_update_appearance     (PeonyInformationPanel     *information_panel);
static void     peony_information_panel_update_buttons        (PeonyInformationPanel     *information_panel);
static void     background_metadata_changed_callback             (PeonyInformationPanel     *information_panel);
static void     peony_information_panel_iface_init            (PeonySidebarIface         *iface);
static void     peony_information_panel_iface_init            (PeonySidebarIface         *iface);
static void     sidebar_provider_iface_init                      (PeonySidebarProviderIface *iface);
static GType    peony_information_panel_provider_get_type     (void);
...
static const GtkTargetEntry target_table[] =
{
    ...
    { "x-special/ukui-reset-background", 0, TARGET_BACKGROUND_RESET },
...
{
    GObject parent;
} PeonyInformationPanelProvider;

typedef struct
{
    GObjectClass parent;
} PeonyInformationPanelProviderClass;


G_DEFINE_TYPE_WITH_CODE (PeonyInformationPanel, peony_information_panel, EEL_TYPE_BACKGROUND_BOX,
                         G_IMPLEMENT_INTERFACE (PEONY_TYPE_SIDEBAR,
                                 peony_information_panel_iface_init));

G_DEFINE_TYPE_WITH_CODE (PeonyInformationPanelProvider, peony_information_panel_provider, G_TYPE_OBJECT,
                         G_IMPLEMENT_INTERFACE (PEONY_TYPE_SIDEBAR_PROVIDER,
                                 sidebar_provider_iface_init));


static const char *
peony_information_panel_get_sidebar_id (PeonySidebar *sidebar)
{
    return PEONY_INFORMATION_PANEL_ID;
}

static char *
peony_information_panel_get_tab_label (PeonySidebar *sidebar)
{
    return g_strdup (_("Information"));
}

static char *
peony_information_panel_get_tab_tooltip (PeonySidebar *sidebar)
{
    return g_strdup (_("Show Information"));
}

static GdkPixbuf *
peony_information_panel_get_tab_icon (PeonySidebar *sidebar)
{
    return NULL;
}

static void
peony_information_panel_is_visible_changed (PeonySidebar *sidebar,
        gboolean         is_visible)
{
    /* Do nothing */
}
...
static void
peony_information_panel_iface_init (PeonySidebarIface *iface)
{
    iface->get_sidebar_id = peony_information_panel_get_sidebar_id;
    iface->get_tab_label = peony_information_panel_get_tab_label;
    iface->get_tab_tooltip = peony_information_panel_get_tab_tooltip;
    iface->get_tab_icon = peony_information_panel_get_tab_icon;
    iface->is_visible_changed = peony_information_panel_is_visible_changed;
}

/* initializing the class object by installing the operations we override */
static void
peony_information_panel_class_init (PeonyInformationPanelClass *klass)
{
    ...
    gobject_class->finalize = peony_information_panel_finalize;

    widget_class->drag_data_received  = peony_information_panel_drag_data_received;
    widget_class->button_press_event  = peony_information_panel_press_event;
#if GTK_CHECK_VERSION (3, 0, 0)
    widget_class->style_updated = peony_information_panel_style_updated;
#else
    widget_class->style_set = peony_information_panel_style_set;
#endif
    ...
    signals[LOCATION_CHANGED] = g_signal_new
                                ("location_changed",
                                 ...
                                 G_SIGNAL_RUN_LAST,
                                 G_STRUCT_OFFSET (PeonyInformationPanelClass,
                                         location_changed),
                                 NULL, NULL,
                                 g_cclosure_marshal_VOID__STRING,
                                 G_TYPE_NONE, 1, G_TYPE_STRING);

    g_type_class_add_private (klass, sizeof (PeonyInformationPanelDetails));
}

/* utility routine to allocate the box the holds the command buttons */
static void
make_button_box (PeonyInformationPanel *information_panel)
{
#if GTK_CHECK_VERSION (3, 0, 0)
    ...
    information_panel->details->button_box = peony_keep_last_vertical_box_new (4);
...
static void
peony_information_panel_init (PeonyInformationPanel *information_panel)
{
    information_panel->details = G_TYPE_INSTANCE_GET_PRIVATE (information_panel,
    							      PEONY_TYPE_INFORMATION_PANEL,
    							      PeonyInformationPanelDetails);

    /* load the default background */
    peony_information_panel_read_defaults (information_panel);
    ...
    information_panel->details->title = PEONY_SIDEBAR_TITLE (peony_sidebar_title_new ());
    ...
    g_signal_connect (peony_preferences,
              "changed::" PEONY_PREFERENCES_SIDE_PANE_BACKGROUND_SET,
              G_CALLBACK(peony_information_panel_theme_changed),
              information_panel);
    g_signal_connect (peony_preferences,
              "changed::" PEONY_PREFERENCES_SIDE_PANE_BACKGROUND_COLOR,
              G_CALLBACK(peony_information_panel_theme_changed),
              information_panel);
    g_signal_connect (peony_preferences,
              "changed::" PEONY_PREFERENCES_SIDE_PANE_BACKGROUND_URI,
              G_CALLBACK(peony_information_panel_theme_changed),
              information_panel);
...
static void
peony_information_panel_finalize (GObject *object)
{
    PeonyInformationPanel *information_panel;

    information_panel = PEONY_INFORMATION_PANEL (object);

    if (information_panel->details->file != NULL)
    {
        peony_file_monitor_remove (information_panel->details->file, information_panel);
        peony_file_unref (information_panel->details->file);
    }
    ...
    g_signal_handlers_disconnect_by_func (peony_preferences,
                                          peony_information_panel_theme_changed,
                                          information_panel);

    G_OBJECT_CLASS (peony_information_panel_parent_class)->finalize (object);
}
...
static gboolean
information_panel_has_background (PeonyInformationPanel *information_panel)
{
    EelBackground *background;
...
static GtkWidget *
peony_information_panel_create_context_menu (PeonyInformationPanel *information_panel)
{
...
static void
peony_information_panel_read_defaults (PeonyInformationPanel *information_panel)
{
    gboolean background_set;
    char *background_color, *background_image;

    background_set = g_settings_get_boolean (peony_preferences, PEONY_PREFERENCES_SIDE_PANE_BACKGROUND_SET);

    background_color = NULL;
    background_image = NULL;
    if (background_set)
    {
        background_color = g_settings_get_string (peony_preferences, PEONY_PREFERENCES_SIDE_PANE_BACKGROUND_COLOR);
        background_image = g_settings_get_string (peony_preferences, PEONY_PREFERENCES_SIDE_PANE_BACKGROUND_URI);
    }
...
static void
peony_information_panel_theme_changed (GSettings   *settings,
                                      const gchar *key,
                                      gpointer user_data)
{
    PeonyInformationPanel *information_panel;

    information_panel = PEONY_INFORMATION_PANEL (user_data);
    peony_information_panel_read_defaults (information_panel);
    peony_information_panel_update_appearance (information_panel);
...
hit_test (PeonyInformationPanel *information_panel,
          int x, int y)
{
    GtkAllocation *allocation;
    gboolean bg_hit;

    if (peony_sidebar_title_hit_test_icon (information_panel->details->title, x, y))
    {
        return ICON_PART;
    }
...
static void
receive_dropped_uri_list (PeonyInformationPanel *information_panel,
                          ...
                          GtkSelectionData *selection_data)
{
    ...
    case BACKGROUND_PART:
        ...
        if (exactly_one && uri_is_local_image (uris[0]))
        {
            ...
            {
                action = peony_drag_drop_background_ask (GTK_WIDGET (information_panel),
    ...
    case ICON_PART:
        ...
        if (uri_is_local_image (uris[0]))
        {
            if (information_panel->details->file != NULL)
            {
                peony_file_set_metadata (information_panel->details->file,
...
static void
receive_dropped_color (PeonyInformationPanel *information_panel,
                       GdkDragAction action,
                       int x, int y,
                       GtkSelectionData *selection_data)
{
...
static void
receive_dropped_keyword (PeonyInformationPanel *information_panel,
                         int x, int y,
                         GtkSelectionData *selection_data)
{
    peony_drag_file_receive_dropped_keyword (information_panel->details->file,
...
static void
peony_information_panel_drag_data_received (GtkWidget *widget, GdkDragContext *context,
        int x, int y,
        GtkSelectionData *selection_data,
        guint info, guint time)
{
    ...
    g_return_if_fail (PEONY_IS_INFORMATION_PANEL (widget));

    information_panel = PEONY_INFORMATION_PANEL (widget);

    switch (info)
    {
    case TARGET_UKUI_URI_LIST:
...
static gboolean
peony_information_panel_press_event (GtkWidget *widget, GdkEventButton *event)
{
    PeonyInformationPanel *information_panel;
    GtkWidget *menu;
    ...
    {
        menu = peony_information_panel_create_context_menu (information_panel);
 ...
 */
static void
background_settings_changed_callback (EelBackground *background, GdkDragAction action, PeonyInformationPanel *information_panel)
{
    char *image;
    char *color;

    g_assert (EEL_IS_BACKGROUND (background));
    g_assert (PEONY_IS_INFORMATION_PANEL (information_panel));
    ...
    if (action != (GdkDragAction) PEONY_DND_ACTION_SET_AS_BACKGROUND)
    {
        peony_file_set_metadata (information_panel->details->file,
        ...
        g_settings_set_string (peony_preferences,
                       PEONY_PREFERENCES_SIDE_PANE_BACKGROUND_COLOR, color ? color : "");
        g_settings_set_string (peony_preferences,
                       PEONY_PREFERENCES_SIDE_PANE_BACKGROUND_URI, image ? image : "");
        g_settings_set_boolean (peony_preferences,
                    PEONY_PREFERENCES_SIDE_PANE_BACKGROUND_SET, TRUE);
...
static void
background_reset_callback (EelBackground *background, PeonyInformationPanel *information_panel)
{
    char *color;
    char *image;
    g_assert (EEL_IS_BACKGROUND (background));
    ...
    color = peony_file_get_metadata (information_panel->details->file,
                                    PEONY_METADATA_KEY_SIDEBAR_BACKGROUND_COLOR,
                                    NULL);

    image = peony_file_get_metadata (information_panel->details->file,
                                    PEONY_METADATA_KEY_SIDEBAR_BACKGROUND_IMAGE,
                                    NULL);
    if (color || image)
    {
    ...
    else
    {
        g_signal_handlers_block_by_func (peony_preferences,
                         ...
                         information_panel);
        g_settings_set_boolean (peony_preferences,
                    PEONY_PREFERENCES_SIDE_PANE_BACKGROUND_SET, FALSE);
        g_signal_handlers_unblock_by_func (peony_preferences,
                         G_CALLBACK(peony_information_panel_theme_changed),
                         information_panel);
    }
...
static GtkWindow *
peony_information_panel_get_window (PeonyInformationPanel *information_panel)
{
    GtkWidget *result;
...
command_button_callback (GtkWidget *button, GAppInfo *application)
{
    ...
    information_panel = PEONY_INFORMATION_PANEL (g_object_get_data (G_OBJECT (button), "user_data"));

    files.next = NULL;
    files.prev = NULL;
    files.data = information_panel->details->file;
    peony_launch_application (application, &files,
                             peony_information_panel_get_window (information_panel));
}
...
{
    //PeonyInformationPanel *self = PEONY_INFORMATION_PANEL (g_object_get_data (G_OBJECT (button), "user_data"));
}

/* utility routine that allocates the command buttons from the command list */

static void
add_command_button (PeonyInformationPanel *information_panel, GAppInfo *application)
{
    char *temp_str;
...
static void
add_buttons_from_metadata (PeonyInformationPanel *information_panel, const char *button_data)
{
    char **terms;
...
/*
 * peony_information_panel_update_buttons:
 *
 * Update the list of program-launching buttons based on the current uri.
 */
static void
peony_information_panel_update_buttons (PeonyInformationPanel *information_panel)
{
    char *button_data;
    ...
    button_data = peony_file_get_metadata (information_panel->details->file,
    ...
    if (peony_mime_has_any_applications_for_file (information_panel->details->file) &&
            !peony_file_is_directory (information_panel->details->file))
    {
        default_app =
            peony_mime_get_default_application_for_file (information_panel->details->file);
...
static void
peony_information_panel_update_appearance (PeonyInformationPanel *information_panel)
{
    ...
    g_return_if_fail (PEONY_IS_INFORMATION_PANEL (information_panel));
    ...
    background_color = peony_file_get_metadata (information_panel->details->file,
                       PEONY_METADATA_KEY_SIDEBAR_BACKGROUND_COLOR,
                       NULL);
    background_image = peony_file_get_metadata (information_panel->details->file,
                       PEONY_METADATA_KEY_SIDEBAR_BACKGROUND_IMAGE,
                       NULL);
            ...
            value_different (information_panel->details->current_background_image, background_image))
    {
        ...
        peony_sidebar_title_select_text_color (information_panel->details->title, background);
    }
...
static void
background_metadata_changed_callback (PeonyInformationPanel *information_panel)
{
    PeonyFileAttributes attributes;
    gboolean ready;

    attributes = peony_mime_actions_get_required_file_attributes ();
    ready = peony_file_check_if_ready (information_panel->details->file, attributes);
    ...
    if (ready)
    {
        ...
        peony_information_panel_update_buttons (information_panel);
    }
}
...
static void
peony_information_panel_set_uri (PeonyInformationPanel *information_panel,
                                const char* new_uri,
                                const char* initial_title)
{
    ...
    if (information_panel->details->file != NULL &&
            peony_file_matches_uri (information_panel->details->file, new_uri))
    {
        return;
    }
    ...
    file = peony_file_get_by_uri (new_uri);
    ...
    attributes = peony_mime_actions_get_required_file_attributes ();
    peony_file_monitor_add (information_panel->details->file, information_panel, attributes);

    background_metadata_changed_callback (information_panel);

    /* tell the title widget about it */
    peony_sidebar_title_set_file (information_panel->details->title,
                                 information_panel->details->file,
                                 initial_title);
}

static void
title_changed_callback (PeonyWindowInfo *window,
                        char               *new_title,
                        PeonyInformationPanel *panel)
{
    peony_sidebar_title_set_text (panel->details->title,
                                 new_title);
}
...
static void
#if GTK_CHECK_VERSION (3, 0, 0)
peony_information_panel_style_updated (GtkWidget *widget)
#else
peony_information_panel_style_set (GtkWidget *widget, GtkStyle *previous_style)
#endif
{
    PeonyInformationPanel *information_panel;
    ...
    peony_information_panel_theme_changed (NULL, NULL, information_panel);
}
                      ...
                      PeonyInformationPanel *panel)
{
    ...
    title = peony_window_slot_info_get_title (slot);
    peony_information_panel_set_uri (panel,
                                    uri,
                                    title);
    g_free (title);
}
...
static void
selection_changed_callback (PeonyWindowInfo *window,
                            PeonyInformationPanel *panel)
{
    int selection_count;
    GList *selection;
    ...
    selection = peony_window_info_get_selection (window);
    ...
    if (selection_count == 1)
    {
        ...
        file = peony_file_get_existing (selected);
    ...
    {
        uri = peony_window_info_get_current_location (window);
        name = peony_window_info_get_title (window);
    }

    peony_information_panel_set_uri (panel, uri, name);
...
static void
peony_information_panel_set_parent_window (PeonyInformationPanel *panel,
        PeonyWindowInfo *window)
{
    gpointer slot;
    char *title, *location;
...
static PeonySidebar *
peony_information_panel_create (PeonySidebarProvider *provider,
                               PeonyWindowInfo *window)
{
    PeonyInformationPanel *panel;

    panel = g_object_new (peony_information_panel_get_type (), NULL);
    peony_information_panel_set_parent_window (panel, window);
    g_object_ref_sink (panel);

    return PEONY_SIDEBAR (panel);
}

static void
sidebar_provider_iface_init (PeonySidebarProviderIface *iface)
{
    iface->create = peony_information_panel_create;
}

static void
peony_information_panel_provider_init (PeonyInformationPanelProvider *sidebar)
{
}

static void
peony_information_panel_provider_class_init (PeonyInformationPanelProviderClass *class)
{
}

void
peony_information_panel_register (void)
{
    peony_module_add_type (peony_information_panel_provider_get_type ());
}
