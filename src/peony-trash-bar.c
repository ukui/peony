...
#define PEONY_TRASH_BAR_GET_PRIVATE(o)\
	(G_TYPE_INSTANCE_GET_PRIVATE ((o), PEONY_TYPE_TRASH_BAR, PeonyTrashBarPrivate))
...
G_DEFINE_TYPE (PeonyTrashBar, peony_trash_bar, GTK_TYPE_BOX);
#else
G_DEFINE_TYPE (PeonyTrashBar, peony_trash_bar, GTK_TYPE_HBOX);
#endif
                           ...
                           PeonyTrashBar *bar)
{
    ...
    locations = peony_window_info_get_selection (PEONY_WINDOW_INFO  (bar->priv->window));
    files = NULL;

    for (l = locations; l != NULL; l = l->next)
    {
        files = g_list_prepend (files, peony_file_get (l->data));
    }

    peony_restore_files_from_trash (files, GTK_WINDOW (gtk_widget_get_toplevel (button)));
...
static void
selection_changed_cb (PeonyWindow *window,
                      PeonyTrashBar *bar)
{
    int count;

    count = peony_window_info_get_selection_count (PEONY_WINDOW_INFO (window));

    gtk_widget_set_sensitive (bar->priv->restore_button, (count > 0));
}

static void
connect_window_and_update_button (PeonyTrashBar *bar)
{
...
static void
peony_trash_bar_set_property (GObject      *object,
                             guint         prop_id,
                             const GValue *value,
                             GParamSpec   *pspec)
{
...
static void
peony_trash_bar_finalize (GObject *obj)
{
    PeonyTrashBar *bar;

    bar = PEONY_TRASH_BAR (obj);
    ...
    G_OBJECT_CLASS (peony_trash_bar_parent_class)->finalize (obj);
}

static void
peony_trash_bar_trash_state_changed (PeonyTrashMonitor *trash_monitor,
                                    gboolean              state,
                                    gpointer              data)
{
    PeonyTrashBar *bar;

    bar = PEONY_TRASH_BAR (data);
...
static void
peony_trash_bar_class_init (PeonyTrashBarClass *klass)
{
    GObjectClass *object_class;

    object_class = G_OBJECT_CLASS (klass);

    object_class->set_property = peony_trash_bar_set_property;
    object_class->finalize = peony_trash_bar_finalize;
    ...
    g_type_class_add_private (klass, sizeof (PeonyTrashBarPrivate));
}
...
empty_trash_callback (GtkWidget *button, gpointer data)
{
    ...
    peony_file_operations_empty_trash (window);
}

static void
peony_trash_bar_init (PeonyTrashBar *bar)
{
    GtkWidget *label;
    GtkWidget *hbox;

    bar->priv = PEONY_TRASH_BAR_GET_PRIVATE (bar);
    ...
    g_signal_connect_object (peony_trash_monitor_get (),
                             "trash_state_changed",
                             G_CALLBACK (peony_trash_bar_trash_state_changed),
                             bar,
                             0);
}

GtkWidget *
peony_trash_bar_new (PeonyWindow *window)
{
    GObject *bar;

    bar = g_object_new (PEONY_TYPE_TRASH_BAR, "window", window, NULL);
