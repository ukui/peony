...
struct PeonyEmblemSidebarDetails
{
...
static void peony_emblem_sidebar_populate          (PeonyEmblemSidebar        *emblem_sidebar);
static void peony_emblem_sidebar_refresh           (PeonyEmblemSidebar        *emblem_sidebar);
static void peony_emblem_sidebar_iface_init        (PeonySidebarIface         *iface);
static void sidebar_provider_iface_init           (PeonySidebarProviderIface *iface);
static GType peony_emblem_sidebar_provider_get_type (void);
...
{
    GObject parent;
} PeonyEmblemSidebarProvider;

typedef struct
{
    GObjectClass parent;
} PeonyEmblemSidebarProviderClass;

#if GTK_CHECK_VERSION (3, 0, 0)
G_DEFINE_TYPE_WITH_CODE (PeonyEmblemSidebar, peony_emblem_sidebar, GTK_TYPE_BOX,
#else
G_DEFINE_TYPE_WITH_CODE (PeonyEmblemSidebar, peony_emblem_sidebar, GTK_TYPE_VBOX,
#endif
                         G_IMPLEMENT_INTERFACE (PEONY_TYPE_SIDEBAR,
                                 peony_emblem_sidebar_iface_init));

G_DEFINE_TYPE_WITH_CODE (PeonyEmblemSidebarProvider, peony_emblem_sidebar_provider, G_TYPE_OBJECT,
                         G_IMPLEMENT_INTERFACE (PEONY_TYPE_SIDEBAR_PROVIDER,
                                 sidebar_provider_iface_init));

static void
peony_emblem_sidebar_drag_data_get_cb (GtkWidget *widget,
                                      GdkDragContext *context,
                                      GtkSelectionData *data,
                                      guint info,
                                      guint time,
                                      PeonyEmblemSidebar *emblem_sidebar)
{
    char *keyword;
...
static void
peony_emblem_sidebar_enter_notify_cb (GtkWidget *widget,
                                     PeonyEmblemSidebar *emblem_sidebar)
{
    GdkPixbuf *pixbuf;
...
static void
peony_emblem_sidebar_leave_notify_cb (GtkWidget *widget,
                                     PeonyEmblemSidebar *emblem_sidebar)
{
    GdkPixbuf *pixbuf;
    EelLabeledImage *image;
...
static gboolean
peony_emblem_sidebar_button_press_cb (GtkWidget *widget,
                                     ...
                                     PeonyEmblemSidebar *emblem_sidebar)
{
    ...
    if (event->button == 3)
    {
        ...
        gtk_widget_set_sensitive (emblem_sidebar->details->popup_remove,
                                  peony_emblem_can_remove_emblem (keyword));
        gtk_widget_set_sensitive (emblem_sidebar->details->popup_rename,
                                  peony_emblem_can_rename_emblem (keyword));
...
emblems_changed_callback (GObject *signaller,
                          PeonyEmblemSidebar *emblem_sidebar)
{
    peony_emblem_sidebar_refresh (emblem_sidebar);
}

static void
peony_emblem_sidebar_delete_cb (GtkWidget *menu_item,
                               PeonyEmblemSidebar *emblem_sidebar)
{
    char *error;

    if (peony_emblem_remove_emblem (emblem_sidebar->details->popup_emblem_keyword))
    {
                           ...
                           PeonyEmblemSidebar *emblem_sidebar)
{
    ...
    if (peony_emblem_rename_emblem (keyword, name))
    {
        send_emblems_changed ();
    }
    else
...
static GtkWidget *
create_rename_emblem_dialog (PeonyEmblemSidebar *emblem_sidebar,
...
static void
peony_emblem_sidebar_rename_cb (GtkWidget *menu_item,
                               PeonyEmblemSidebar *emblem_sidebar)
{
    GtkWidget *dialog;
...
static void
create_popup_menu (PeonyEmblemSidebar *emblem_sidebar)
{
    ...
    g_signal_connect (menu_item, "activate",
                      G_CALLBACK (peony_emblem_sidebar_rename_cb),
                      emblem_sidebar);
    ...
    menu_item = gtk_image_menu_item_new_from_stock (GTK_STOCK_DELETE,
                NULL);
    g_signal_connect (menu_item, "activate",
                      G_CALLBACK (peony_emblem_sidebar_delete_cb),
                      emblem_sidebar);
    gtk_widget_show (menu_item);
    gtk_menu_shell_append (GTK_MENU_SHELL (popup), menu_item);
...
static GtkWidget *
create_emblem_widget_with_pixbuf (PeonyEmblemSidebar *emblem_sidebar,
                                  const char *keyword,
                                  ...
                                  GdkPixbuf *pixbuf)
{
    ...
    g_signal_connect (event_box, "button_press_event",
                      G_CALLBACK (peony_emblem_sidebar_button_press_cb),
                      emblem_sidebar);
    g_signal_connect (event_box, "drag-data-get",
                      G_CALLBACK (peony_emblem_sidebar_drag_data_get_cb),
                      emblem_sidebar);
    g_signal_connect (event_box, "enter-notify-event",
                      G_CALLBACK (peony_emblem_sidebar_enter_notify_cb),
                      emblem_sidebar);
    g_signal_connect (event_box, "leave-notify-event",
                      G_CALLBACK (peony_emblem_sidebar_leave_notify_cb),
                      emblem_sidebar);
...
static GtkWidget *
create_emblem_widget (PeonyEmblemSidebar *emblem_sidebar,
                      const char *name)
{
    GtkWidget *ret;
    const char *display_name;
    ...
    info = peony_icon_info_lookup_from_name (name, PEONY_ICON_SIZE_STANDARD);

    pixbuf = peony_icon_info_get_pixbuf_at_size (info, PEONY_ICON_SIZE_STANDARD);

    display_name = peony_icon_info_get_display_name (info);

    keyword = peony_emblem_get_keyword_from_icon_name (name);
...
static GtkWidget *
create_add_emblems_dialog (PeonyEmblemSidebar *emblem_sidebar,
                           GSList *emblems)
{
...
static void
peony_emblem_sidebar_refresh (PeonyEmblemSidebar *emblem_sidebar)
{
    ...
    peony_emblem_sidebar_populate (emblem_sidebar);
}
                                ...
                                PeonyEmblemSidebar *emblem_sidebar)
{
    ...
    case GTK_RESPONSE_OK:
        ...
        for (l = emblems; l; l = l->next)
        {
            ...
            keyword = peony_emblem_create_unique_keyword (emblem->name);
            if (!peony_emblem_verify_keyword
        ...
        for (l = emblems; l; l = l->next)
        {
            ...
            peony_emblem_install_custom_emblem (emblem->pixbuf,
                                               emblem->keyword,
                                               emblem->name,
                                               GTK_WINDOW (dialog));
        }
...
static void
show_add_emblems_dialog (PeonyEmblemSidebar *emblem_sidebar,
                         GSList *emblems)
{
    GtkWidget *dialog;
...
static void
peony_emblem_sidebar_drag_received_cb (GtkWidget *widget,
                                      ...
                                      PeonyEmblemSidebar *emblem_sidebar)
{
    ...
    case TARGET_URI_LIST:
        ...
        for (i = 0; uris[i] != NULL; ++i)
        {
            ...
            pixbuf = peony_emblem_load_pixbuf_for_emblem (f);
...
static GtkWidget *
peony_emblem_sidebar_create_container (PeonyEmblemSidebar *emblem_sidebar)
{
    ...
    g_signal_connect (scroller, "drag-data-received",
                      G_CALLBACK (peony_emblem_sidebar_drag_received_cb),
                      emblem_sidebar);
...
static void
peony_emblem_sidebar_populate (PeonyEmblemSidebar *emblem_sidebar)
{
    ...
    while (l != NULL)
    {
        ...
        if (!peony_emblem_should_show_in_list (name))
        {
            continue;
        }
...
static void
peony_emblem_sidebar_init (PeonyEmblemSidebar *emblem_sidebar)
{
    GtkWidget *widget;

    emblem_sidebar->details = g_new0 (PeonyEmblemSidebarDetails, 1);

    create_popup_menu (emblem_sidebar);

    widget = peony_emblem_sidebar_create_container (emblem_sidebar);
    peony_emblem_sidebar_populate (emblem_sidebar);
...
static void
peony_emblem_sidebar_finalize (GObject *object)
{
    PeonyEmblemSidebar *emblem_sidebar;

    g_assert (PEONY_IS_EMBLEM_SIDEBAR (object));
    emblem_sidebar = PEONY_EMBLEM_SIDEBAR (object);
    ...
    G_OBJECT_CLASS (peony_emblem_sidebar_parent_class)->finalize (object);
}

static void
peony_emblem_sidebar_class_init (PeonyEmblemSidebarClass *object_klass)
{
    GObjectClass *gobject_class;

    PeonyEmblemSidebarClass *klass;

    klass = PEONY_EMBLEM_SIDEBAR_CLASS (object_klass);
    gobject_class = G_OBJECT_CLASS (object_klass);

    gobject_class->finalize = peony_emblem_sidebar_finalize;
}

static const char *
peony_emblem_sidebar_get_sidebar_id (PeonySidebar *sidebar)
{
    return PEONY_EMBLEM_SIDEBAR_ID;
}

static char *
peony_emblem_sidebar_get_tab_label (PeonySidebar *sidebar)
{
    return g_strdup (_("Emblems"));
}

static char *
peony_emblem_sidebar_get_tab_tooltip (PeonySidebar *sidebar)
{
    return g_strdup (_("Show Emblems"));
}

static GdkPixbuf *
peony_emblem_sidebar_get_tab_icon (PeonySidebar *sidebar)
{
    return NULL;
}

static void
peony_emblem_sidebar_is_visible_changed (PeonySidebar *sidebar,
                                        gboolean         is_visible)
{
    /* Do nothing */
}
...
static void
peony_emblem_sidebar_iface_init (PeonySidebarIface *iface)
{
    iface->get_sidebar_id = peony_emblem_sidebar_get_sidebar_id;
    iface->get_tab_label = peony_emblem_sidebar_get_tab_label;
    iface->get_tab_tooltip = peony_emblem_sidebar_get_tab_tooltip;
    iface->get_tab_icon = peony_emblem_sidebar_get_tab_icon;
    iface->is_visible_changed = peony_emblem_sidebar_is_visible_changed;
}

static void
peony_emblem_sidebar_set_parent_window (PeonyEmblemSidebar *sidebar,
                                       PeonyWindowInfo *window)
{
...
static PeonySidebar *
peony_emblem_sidebar_create (PeonySidebarProvider *provider,
                            PeonyWindowInfo *window)
{
    PeonyEmblemSidebar *sidebar;

    sidebar = g_object_new (peony_emblem_sidebar_get_type (), NULL);
    peony_emblem_sidebar_set_parent_window (sidebar, window);
...
{
    iface->create = peony_emblem_sidebar_create;
}

static void
peony_emblem_sidebar_provider_init (PeonyEmblemSidebarProvider *sidebar)
{
}

static void
peony_emblem_sidebar_provider_class_init (PeonyEmblemSidebarProviderClass *class)
{
}

void
peony_emblem_sidebar_register (void)
{
    peony_module_add_type (peony_emblem_sidebar_provider_get_type ());
}
