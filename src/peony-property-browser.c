...
#include <gio/gio.h>
#include <libpeony-private/peony-customization-data.h>
...
{
    PEONY_PROPERTY_NONE,
    PEONY_PROPERTY_PATTERN,
    PEONY_PROPERTY_COLOR,
    PEONY_PROPERTY_EMBLEM
} PeonyPropertyType;

struct PeonyPropertyBrowserDetails
{
    GtkWidget *container;
    ...
    PeonyPropertyType category_type;
...
static void     peony_property_browser_update_contents       (PeonyPropertyBrowser       *property_browser);
static void     peony_property_browser_set_category          (PeonyPropertyBrowser       *property_browser,
        const char                    *new_category);
static void     peony_property_browser_set_dragged_file      (PeonyPropertyBrowser       *property_browser,
        const char                    *dragged_file_name);
static void     peony_property_browser_set_drag_type         (PeonyPropertyBrowser       *property_browser,
        const char                    *new_drag_type);
static void     add_new_button_callback                         (GtkWidget                     *widget,
        PeonyPropertyBrowser       *property_browser);
static void     cancel_remove_mode                              (PeonyPropertyBrowser       *property_browser);
...
static void     remove_button_callback                          (GtkWidget                     *widget,
        PeonyPropertyBrowser       *property_browser);
static gboolean peony_property_browser_delete_event_callback (GtkWidget                     *widget,
        GdkEvent                      *event,
        gpointer                       user_data);
static void     peony_property_browser_hide_callback         (GtkWidget                     *widget,
        gpointer                       user_data);
static void     peony_property_browser_drag_end              (GtkWidget                     *widget,
        GdkDragContext                *context);
static void     peony_property_browser_drag_begin            (GtkWidget                     *widget,
        GdkDragContext                *context);
static void     peony_property_browser_drag_data_get         (GtkWidget                     *widget,
        GdkDragContext                *context,
...
static GdkPixbuf * make_drag_image                              (PeonyPropertyBrowser       *property_browser,
        const char                    *file_name);
static GdkPixbuf * make_color_drag_image                        (PeonyPropertyBrowser       *property_browser,
...
G_DEFINE_TYPE (PeonyPropertyBrowser, peony_property_browser, GTK_TYPE_WINDOW)


/* Destroy the three dialogs for adding patterns/colors/emblems if any of them
   exist. */
static void
peony_property_browser_destroy_dialogs (PeonyPropertyBrowser *property_browser)
{
...
static void
peony_property_browser_dispose (GObject *object)
{
    PeonyPropertyBrowser *property_browser;

    property_browser = PEONY_PROPERTY_BROWSER (object);

    peony_property_browser_destroy_dialogs (property_browser);
    ...
    G_OBJECT_CLASS (peony_property_browser_parent_class)->dispose (object);
}

/* initializing the class object by installing the operations we override */
static void
peony_property_browser_class_init (PeonyPropertyBrowserClass *klass)
{
    ...
    widget_class->drag_data_get  = peony_property_browser_drag_data_get;
    widget_class->drag_end  = peony_property_browser_drag_end;
...
static void
peony_property_browser_init (PeonyPropertyBrowser *property_browser)
{
    ...
    property_browser->details = G_TYPE_INSTANCE_GET_PRIVATE (property_browser,
    							     PEONY_TYPE_PROPERTY_BROWSER,
    							     PeonyPropertyBrowserDetails);
    ...
    gtk_window_set_wmclass (GTK_WINDOW (widget), "property_browser", "Peony");
    ...
    gtk_style_context_add_class (context, "peony-property-browser");
    ...
    g_signal_connect (property_browser, "delete_event",
                      G_CALLBACK (peony_property_browser_delete_event_callback), NULL);
    g_signal_connect (property_browser, "hide",
                      G_CALLBACK (peony_property_browser_hide_callback), NULL);
    ...
    peony_property_browser_set_path(property_browser, BROWSER_CATEGORIES_FILE_NAME);
}

/* create a new instance */
PeonyPropertyBrowser *
peony_property_browser_new (GdkScreen *screen)
{
    PeonyPropertyBrowser *browser;

    browser = PEONY_PROPERTY_BROWSER
              (gtk_widget_new (peony_property_browser_get_type (), NULL));
...
void
peony_property_browser_show (GdkScreen *screen)
{
    static GtkWindow *browser = NULL;

    if (browser == NULL)
    {
        browser = GTK_WINDOW (peony_property_browser_new (screen));
...
static gboolean
peony_property_browser_delete_event_callback (GtkWidget *widget,
        GdkEvent  *event,
        gpointer   user_data)
{
...
static void
peony_property_browser_hide_callback (GtkWidget *widget,
                                     gpointer   user_data)
{
    PeonyPropertyBrowser *property_browser;

    property_browser = PEONY_PROPERTY_BROWSER (widget);
...
static void
peony_property_browser_set_dragged_file (PeonyPropertyBrowser *property_browser,
                                        const char *dragged_file_name)
{
    g_free (property_browser->details->dragged_file);
...
static void
peony_property_browser_set_drag_type (PeonyPropertyBrowser *property_browser,
                                     const char *new_drag_type)
{
    g_free (property_browser->details->drag_type);
...
static void
peony_property_browser_drag_begin (GtkWidget *widget,
                                  GdkDragContext *context)
{
    PeonyPropertyBrowser *property_browser;
    GtkWidget *child;
...
static void
peony_property_browser_drag_data_get (GtkWidget *widget,
                                     GdkDragContext *context,
                                     GtkSelectionData *selection_data,
                                     guint info,
                                     guint32 time)
{
    ...
    gboolean is_reset;
    PeonyPropertyBrowser *property_browser = PEONY_PROPERTY_BROWSER(widget);
    GdkAtom target;
...
static void
peony_property_browser_drag_end (GtkWidget *widget, GdkDragContext *context)
{
    PeonyPropertyBrowser *property_browser = PEONY_PROPERTY_BROWSER(widget);
...
static GdkPixbuf *
make_drag_image (PeonyPropertyBrowser *property_browser, const char* file_name)
{
    ...
    if (property_browser->details->category_type == PEONY_PROPERTY_EMBLEM)
    {
        if (strcmp (file_name, "erase") == 0)
        {
            pixbuf = NULL;

            image_file_name = peony_pixmap_file (ERASE_OBJECT_NAME);
        ...
        {
            icon_name = peony_emblem_get_icon_name_from_keyword (file_name);
            info = peony_icon_info_lookup_from_name (icon_name, PEONY_ICON_SIZE_STANDARD);
            pixbuf = peony_icon_info_get_pixbuf_at_size (info, PEONY_ICON_SIZE_STANDARD);
            g_object_unref (info);
            g_free (icon_name);
        }
        return pixbuf;
    ...
    {
        pixbuf = peony_customization_make_pattern_chit (orig_pixbuf, property_browser->details->property_chit, TRUE, is_reset);
    }
    else
    {
...
static GdkPixbuf*
make_color_drag_image (PeonyPropertyBrowser *property_browser, const char *color_spec, gboolean trim_edges)
{
    ...
    {
        ret = peony_customization_make_pattern_chit (color_square,
...
category_toggled_callback (GtkWidget *widget, char *category_name)
{
    ...
    property_browser = PEONY_PROPERTY_BROWSER (g_object_get_data (G_OBJECT (widget), "user_data"));
    ...
    peony_property_browser_set_category (property_browser, category_name);
}

static xmlDocPtr
read_browser_xml (PeonyPropertyBrowser *property_browser)
{
    char *path;
    xmlDocPtr document;

    path = peony_get_data_file_path (property_browser->details->path);
    if (path == NULL)
    {
        return NULL;
    }
...
static void
write_browser_xml (PeonyPropertyBrowser *property_browser,
                   xmlDocPtr document)
{
...
static void
remove_color (PeonyPropertyBrowser *property_browser, const char* color_name)
{
...
static void
remove_pattern(PeonyPropertyBrowser *property_browser, const char* pattern_name)
{
    char *pattern_path;
    char *user_directory;
...
static void
remove_emblem (PeonyPropertyBrowser *property_browser, const char* emblem_name)
{
    /* delete the emblem from the emblem directory */
    if (peony_emblem_remove_emblem (emblem_name) == FALSE)
    {
...
static void
peony_property_browser_remove_element (PeonyPropertyBrowser *property_browser, EelLabeledImage *child)
{
...
static GtkWidget*
peony_emblem_dialog_new (PeonyPropertyBrowser *property_browser)
{
    GtkWidget *widget;
    GtkWidget *button;
    GtkWidget *dialog;
...
static GtkWidget*
peony_color_selection_dialog_new (PeonyPropertyBrowser *property_browser)
{
    GtkWidget *widget;
    GtkWidget *dialog;
...
add_pattern_to_browser (GtkDialog *dialog, gint response_id, gpointer *data)
{
    ...
    PeonyPropertyBrowser *property_browser = PEONY_PROPERTY_BROWSER (data);
    ...
    peony_property_browser_update_contents (property_browser);
...
static void
add_new_pattern (PeonyPropertyBrowser *property_browser)
{
    GtkWidget *dialog;
    GtkFileFilter *filter;
    GtkWidget *preview;
...
static void
add_color_to_file (PeonyPropertyBrowser *property_browser, const char *color_spec, const char *color_name)
{
...
show_color_selection_window (GtkWidget *widget, gpointer *data)
{
    ...
    property_browser->details->colors_dialog = peony_color_selection_dialog_new (property_browser);
...
static void
add_new_color (PeonyPropertyBrowser *property_browser)
{
    if (property_browser->details->colors_dialog)
    {
...
static void
emblem_dialog_clicked (GtkWidget *dialog, int which_button, PeonyPropertyBrowser *property_browser)
{
    ...
    if (which_button == GTK_RESPONSE_OK)
    {
        ...
        pixbuf = peony_emblem_load_pixbuf_for_emblem (emblem_file);
        g_object_unref (emblem_file);
        ...
        peony_emblem_install_custom_emblem (pixbuf,
                                           stripped_keyword,
                                           stripped_keyword,
                                           GTK_WINDOW (property_browser));
        if (pixbuf != NULL)
...
static void
add_new_emblem (PeonyPropertyBrowser *property_browser)
{
    if (property_browser->details->emblems_dialog)
    {
    ...
    {
        property_browser->details->emblems_dialog = peony_emblem_dialog_new (property_browser);
...
/* cancelremove mode */
static void
cancel_remove_mode (PeonyPropertyBrowser *property_browser)
{
...
static void
add_new_button_callback(GtkWidget *widget, PeonyPropertyBrowser *property_browser)
{
...
{
    cancel_remove_mode (PEONY_PROPERTY_BROWSER (property_browser));
...
static void
remove_button_callback(GtkWidget *widget, PeonyPropertyBrowser *property_browser)
{
    if (property_browser->details->remove_mode)
    {
        return;
                          ...
                          gpointer callback_data)
{
    ...
    g_return_if_fail (PEONY_IS_PROPERTY_BROWSER (callback_data));
    g_return_if_fail (g_object_get_data (G_OBJECT (child), "property-name") != NULL);

    element_name = g_object_get_data (G_OBJECT (child), "property-name");
    property_browser = PEONY_PROPERTY_BROWSER (callback_data);

    /* handle remove mode by removing the element */
    if (property_browser->details->remove_mode)
    {
        peony_property_browser_remove_element (property_browser, EEL_LABELED_IMAGE (child));
        property_browser->details->remove_mode = FALSE;
        peony_property_browser_update_contents (property_browser);
        gtk_widget_show (property_browser->details->help_label);
        return;
    ...
    {
        drag_types[0].target = "x-special/ukui-reset-background";
    }

    target_list = gtk_target_list_new (drag_types, G_N_ELEMENTS (drag_types));
    peony_property_browser_set_dragged_file(property_browser, element_name);
...
static void
make_properties_from_directories (PeonyPropertyBrowser *property_browser)
{
    PeonyCustomizationData *customization_data;
    char *object_name;
    char *object_label;
    ...
    g_return_if_fail (PEONY_IS_PROPERTY_BROWSER (property_browser));
    ...
    if (property_browser->details->category_type == PEONY_PROPERTY_EMBLEM)
    {
        ...
        while (l != NULL)
        {
            ...
            if (!peony_emblem_should_show_in_list (icon_name))
            {
                continue;
            }

            object_name = peony_emblem_get_keyword_from_icon_name (icon_name);
            if (peony_emblem_can_remove_emblem (object_name))
            {
            ...
            info = peony_icon_info_lookup_from_name (icon_name, PEONY_ICON_SIZE_STANDARD);
            object_pixbuf = peony_icon_info_get_pixbuf_at_size (info, PEONY_ICON_SIZE_STANDARD);
            object_label = g_strdup (peony_icon_info_get_display_name (info));
            g_object_unref (info);
    ...
    {
        customization_data = peony_customization_data_new (property_browser->details->category,
        ...
        while (peony_customization_data_get_next_element_for_display (customization_data,
                &object_name,
                &object_pixbuf,
                &object_label))
        {
            ...
            if (property_browser->details->category_type == PEONY_PROPERTY_PATTERN
        ...
        property_browser->details->has_local = peony_customization_data_private_data_was_displayed (customization_data);
        peony_customization_data_destroy (customization_data);
    }
    ...
    if (property_browser->details->category_type == PEONY_PROPERTY_EMBLEM)
    {
        ...
        path = peony_pixmap_file (ERASE_OBJECT_NAME);
        if (path != NULL)
        {
...
static void
add_reset_property (PeonyPropertyBrowser *property_browser)
{
    ...
    reset_image_file_name = g_strdup_printf ("%s/%s/%s", PEONY_DATADIR, "patterns", RESET_IMAGE_NAME);
    ...
    {
        reset_chit = peony_customization_make_pattern_chit (reset_pixbuf, property_browser->details->property_chit, FALSE, TRUE);
    }
...
static void
make_properties_from_xml_node (PeonyPropertyBrowser *property_browser,
                               xmlNodePtr node)
{
...
static void
make_category(PeonyPropertyBrowser *property_browser, const char* path, const char* mode, xmlNodePtr node, const char *description)
{
                                      ...
                                      const char *image)
{
    ...
    file_name = peony_pixmap_file (image);
    if (file_name != NULL)
    {
...
static void
make_category_link (PeonyPropertyBrowser *property_browser,
                    const char *name,
...
void
peony_property_browser_update_contents (PeonyPropertyBrowser *property_browser)
{
    xmlNodePtr cur_node;
    xmlDocPtr document;
    GtkWidget *viewport;
        ...
        if (strcmp (cur_node->name, "category") == 0)
        {
                    ...
                    && strcmp (property_browser->details->category, name) == 0)
            {
                ...
                make_category (property_browser,
                               ...
                               cur_node,
                               description);
                peony_property_browser_set_drag_type (property_browser, type);
...
static void
peony_property_browser_set_category (PeonyPropertyBrowser *property_browser,
                                    const char *new_category)
{
    ...
    {
        property_browser->details->category_type = PEONY_PROPERTY_COLOR;
    }
    else if (g_strcmp0 (new_category, "emblems") == 0)
    {
        property_browser->details->category_type = PEONY_PROPERTY_EMBLEM;
    }
    else
    {
        property_browser->details->category_type = PEONY_PROPERTY_NONE;
    }
...
void
peony_property_browser_set_path (PeonyPropertyBrowser *property_browser,
                                const char *new_path)
{
...
{
    g_signal_emit_by_name (peony_signaller_get_current (), "emblems_changed");
}
