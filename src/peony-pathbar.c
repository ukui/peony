...
#define PEONY_PATH_BAR_ICON_SIZE 16
...
G_DEFINE_TYPE (PeonyPathBar,
               peony_path_bar,
               GTK_TYPE_CONTAINER);

static void     peony_path_bar_finalize                 (GObject         *object);
static void     peony_path_bar_dispose                  (GObject         *object);
#if GTK_CHECK_VERSION(3,0,0)
static void     peony_path_bar_get_preferred_width      (GtkWidget        *widget,
        						gint             *minimum,
        						gint             *natural);
static void     peony_path_bar_get_preferred_height     (GtkWidget        *widget,
        						gint             *minimum,
        						gint             *natural);
#else
static void     peony_path_bar_size_request             (GtkWidget       *widget,
        						GtkRequisition  *requisition);
#endif
static void     peony_path_bar_unmap                    (GtkWidget       *widget);
static void     peony_path_bar_size_allocate            (GtkWidget       *widget,
        GtkAllocation   *allocation);
static void     peony_path_bar_add                      (GtkContainer    *container,
        GtkWidget       *widget);
static void     peony_path_bar_remove                   (GtkContainer    *container,
        GtkWidget       *widget);
static void     peony_path_bar_forall                   (GtkContainer    *container,
        gboolean         include_internals,
        GtkCallback      callback,
        gpointer         callback_data);
static void     peony_path_bar_scroll_up                (PeonyPathBar *path_bar);
static void     peony_path_bar_scroll_down              (PeonyPathBar *path_bar);
static gboolean peony_path_bar_scroll                   (GtkWidget       *path_bar,
        GdkEventScroll  *scroll);
static void     peony_path_bar_stop_scrolling           (PeonyPathBar *path_bar);
static gboolean peony_path_bar_slider_button_press      (GtkWidget       *widget,
        GdkEventButton  *event,
        PeonyPathBar *path_bar);
static gboolean peony_path_bar_slider_button_release    (GtkWidget       *widget,
        GdkEventButton  *event,
        PeonyPathBar *path_bar);
static void     peony_path_bar_grab_notify              (GtkWidget       *widget,
        gboolean         was_grabbed);
static void     peony_path_bar_state_changed            (GtkWidget       *widget,
        GtkStateType     previous_state);
#if GTK_CHECK_VERSION (3, 0, 0)
static void     peony_path_bar_style_updated            (GtkWidget       *widget);
#else
static void     peony_path_bar_style_set                (GtkWidget       *widget,
        GtkStyle        *previous_style);
#endif
static void     peony_path_bar_screen_changed           (GtkWidget       *widget,
        GdkScreen       *previous_screen);
static void     peony_path_bar_check_icon_theme         (PeonyPathBar *path_bar);
static void     peony_path_bar_update_button_appearance (ButtonData      *button_data);
static void     peony_path_bar_update_button_state      (ButtonData      *button_data,
        gboolean         current_dir);
static gboolean peony_path_bar_update_path              (PeonyPathBar *path_bar,
        GFile           *file_path,
        gboolean         emit_signal);

#if GTK_CHECK_VERSION(3,0,0)
static GtkWidget *
get_slider_button (PeonyPathBar  *path_bar,
...
static void
update_button_types (PeonyPathBar *path_bar)
{
    GList *list;
    GFile *path = NULL;
    ...
    if (path != NULL)
    {
        peony_path_bar_update_path (path_bar, path, TRUE);
    }
}
...
desktop_location_changed_callback (gpointer user_data)
{
    ...
    path_bar = PEONY_PATH_BAR (user_data);

    g_object_unref (path_bar->desktop_path);
    g_object_unref (path_bar->home_path);
    path_bar->desktop_path = peony_get_desktop_location ();
                        ...
                        PeonyPathBar *path_bar)
{
    ...
    for (list = path_bar->button_list; list; list = list->next)
    {
        ...
        if (g_file_equal (file, button_data->path))
        {
            ...
            icon_info = peony_icon_info_lookup (icon, PEONY_PATH_BAR_ICON_SIZE);
            pixbuf = peony_icon_info_get_pixbuf_at_size (icon_info, PEONY_PATH_BAR_ICON_SIZE);
...
slider_timeout (gpointer user_data)
{
    ...
    if (gtk_widget_get_visible (GTK_WIDGET (path_bar)))
    {
        ...
        {
            peony_path_bar_scroll_up (path_bar);
        }
        else
        {
            peony_path_bar_scroll_down (path_bar);
        }
    }
...
static void
peony_path_bar_slider_drag_motion (GtkWidget      *widget,
...
static void
peony_path_bar_slider_drag_leave (GtkWidget      *widget,
                                 GdkDragContext *context,
                                 unsigned int    time,
                                 gpointer        user_data)
{
...
static void
peony_path_bar_init (PeonyPathBar *path_bar)
{
    char *p;
#if GTK_CHECK_VERSION(3, 0, 0)
    ...
    gtk_style_context_add_class (context, "peony-pathbar");
#endif
...
#endif
    path_bar->icon_size = PEONY_PATH_BAR_ICON_SIZE;
    ...
    g_signal_connect_swapped (path_bar->up_slider_button, "clicked", G_CALLBACK (peony_path_bar_scroll_up), path_bar);
    g_signal_connect_swapped (path_bar->down_slider_button, "clicked", G_CALLBACK (peony_path_bar_scroll_down), path_bar);

    g_signal_connect (path_bar->up_slider_button, "button_press_event", G_CALLBACK (peony_path_bar_slider_button_press), path_bar);
    g_signal_connect (path_bar->up_slider_button, "button_release_event", G_CALLBACK (peony_path_bar_slider_button_release), path_bar);
    g_signal_connect (path_bar->down_slider_button, "button_press_event", G_CALLBACK (peony_path_bar_slider_button_press), path_bar);
    g_signal_connect (path_bar->down_slider_button, "button_release_event", G_CALLBACK (peony_path_bar_slider_button_release), path_bar);
    ...
    g_signal_connect (path_bar->up_slider_button,
                      "drag-motion",
                      G_CALLBACK (peony_path_bar_slider_drag_motion),
                      path_bar);
    g_signal_connect (path_bar->up_slider_button,
                      "drag-leave",
                      G_CALLBACK (peony_path_bar_slider_drag_leave),
                      path_bar);
    ...
    g_signal_connect (peony_trash_monitor_get (),
                      "trash_state_changed",
...
static void
peony_path_bar_class_init (PeonyPathBarClass *path_bar_class)
{
    ...
    gobject_class->finalize = peony_path_bar_finalize;
    gobject_class->dispose = peony_path_bar_dispose;

#if GTK_CHECK_VERSION(3,0,0)
    widget_class->get_preferred_height = peony_path_bar_get_preferred_height;
    widget_class->get_preferred_width = peony_path_bar_get_preferred_width;
#else
    widget_class->size_request = peony_path_bar_size_request;
#endif
    widget_class->unmap = peony_path_bar_unmap;
    widget_class->size_allocate = peony_path_bar_size_allocate;
#if GTK_CHECK_VERSION (3, 0, 0)
    widget_class->style_updated = peony_path_bar_style_updated;
#else
    widget_class->style_set = peony_path_bar_style_set;
#endif
    widget_class->screen_changed = peony_path_bar_screen_changed;
    widget_class->grab_notify = peony_path_bar_grab_notify;
    widget_class->state_changed = peony_path_bar_state_changed;
    widget_class->scroll_event = peony_path_bar_scroll;

    container_class->add = peony_path_bar_add;
    container_class->forall = peony_path_bar_forall;
    container_class->remove = peony_path_bar_remove;

    path_bar_signals [PATH_CLICKED] =
        g_signal_new ("path-clicked",
                      G_OBJECT_CLASS_TYPE (path_bar_class),
                      G_SIGNAL_RUN_FIRST,
                      G_STRUCT_OFFSET (PeonyPathBarClass, path_clicked),
                      NULL, NULL,
    ...
    path_bar_signals [PATH_SET] =
        g_signal_new ("path-set",
                      G_OBJECT_CLASS_TYPE (path_bar_class),
                      G_SIGNAL_RUN_FIRST,
                      G_STRUCT_OFFSET (PeonyPathBarClass, path_set),
                      NULL, NULL,
                      g_cclosure_marshal_VOID__OBJECT,
                      G_TYPE_NONE, 1,
                      G_TYPE_FILE);
...
static void
peony_path_bar_finalize (GObject *object)
{
    PeonyPathBar *path_bar;

    path_bar = PEONY_PATH_BAR (object);

    peony_path_bar_stop_scrolling (path_bar);
    ...
    g_signal_handlers_disconnect_by_func (peony_trash_monitor_get (),
                                          trash_state_changed_cb, path_bar);
    g_signal_handlers_disconnect_by_func (peony_preferences,
                                          desktop_location_changed_callback,
                                          path_bar);

    G_OBJECT_CLASS (peony_path_bar_parent_class)->finalize (object);
}

/* Removes the settings signal handler.  It's safe to call multiple times */
static void
remove_settings_signal (PeonyPathBar *path_bar,
                        GdkScreen  *screen)
{
...
static void
peony_path_bar_dispose (GObject *object)
{
    remove_settings_signal (PEONY_PATH_BAR (object), gtk_widget_get_screen (GTK_WIDGET (object)));

    G_OBJECT_CLASS (peony_path_bar_parent_class)->dispose (object);
}
...
#if GTK_CHECK_VERSION(3,0,0)
static void
peony_path_bar_get_preferred_width (GtkWidget *widget,
    			       gint      *minimum,
    			       gint      *natural)
{
    ButtonData *button_data;
    ...
    path_bar = PEONY_PATH_BAR (widget);
...
static void
peony_path_bar_get_preferred_height (GtkWidget *widget,
    				gint      *minimum,
    				gint      *natural)
{
    ButtonData *button_data;
...
static void
peony_path_bar_size_request (GtkWidget      *widget,
                            GtkRequisition *requisition)
{
    ButtonData *button_data;
    PeonyPathBar *path_bar;
...
static void
peony_path_bar_update_slider_buttons (PeonyPathBar *path_bar)
{
    if (path_bar->button_list)
    {
...
static void
peony_path_bar_unmap (GtkWidget *widget)
{
    peony_path_bar_stop_scrolling (PEONY_PATH_BAR (widget));

    GTK_WIDGET_CLASS (peony_path_bar_parent_class)->unmap (widget);
}

/* This is a tad complicated */
static void
peony_path_bar_size_allocate (GtkWidget     *widget,
                             GtkAllocation *allocation)
{
    GtkWidget *child;
    PeonyPathBar *path_bar;
    ...
    if (need_sliders)
    {
        ...
        peony_path_bar_update_slider_buttons (path_bar);
    }
    else
    {
...
static void
#if GTK_CHECK_VERSION (3, 0, 0)
peony_path_bar_style_updated (GtkWidget *widget)
{
    if (GTK_WIDGET_CLASS (peony_path_bar_parent_class)->style_updated)
    {
        GTK_WIDGET_CLASS (peony_path_bar_parent_class)->style_updated (widget);
#else
peony_path_bar_style_set (GtkWidget *widget,	GtkStyle  *previous_style)
{
    if (GTK_WIDGET_CLASS (peony_path_bar_parent_class)->style_set)
    {
        GTK_WIDGET_CLASS (peony_path_bar_parent_class)->style_set (widget, previous_style);
#endif
    }

    peony_path_bar_check_icon_theme (PEONY_PATH_BAR (widget));
}

static void
peony_path_bar_screen_changed (GtkWidget *widget,
                              GdkScreen *previous_screen)
{
    if (GTK_WIDGET_CLASS (peony_path_bar_parent_class)->screen_changed)
    {
        GTK_WIDGET_CLASS (peony_path_bar_parent_class)->screen_changed (widget, previous_screen);
    }
    /* We might nave a new settings, so we remove the old one */
    if (previous_screen)
    {
        remove_settings_signal (PEONY_PATH_BAR (widget), previous_screen);
    }
    peony_path_bar_check_icon_theme (PEONY_PATH_BAR (widget));
}

static gboolean
peony_path_bar_scroll (GtkWidget      *widget,
...
static void
peony_path_bar_add (GtkContainer *container,
                   GtkWidget    *widget)
{
...
static void
peony_path_bar_remove_1 (GtkContainer *container,
                        GtkWidget    *widget)
{
...
static void
peony_path_bar_remove (GtkContainer *container,
                      GtkWidget    *widget)
{
    PeonyPathBar *path_bar;
    GList *children;

    path_bar = PEONY_PATH_BAR (container);
...
static void
peony_path_bar_forall (GtkContainer *container,
                      gboolean      include_internals,
                      GtkCallback   callback,
                      gpointer      callback_data)
{
...
static void
peony_path_bar_scroll_down (PeonyPathBar *path_bar)
{
    GList *list;
    GList *down_button;
    GList *up_button;
...
static void
peony_path_bar_scroll_up (PeonyPathBar *path_bar)
{
    GList *list;
...
static gboolean
peony_path_bar_scroll_timeout (PeonyPathBar *path_bar)
{
    ...
    if (path_bar->timer)
    {
        ...
        if (path_bar->need_timer)
        {
            ...
            path_bar->timer = g_timeout_add (SCROLL_TIMEOUT,
                                             (GSourceFunc)peony_path_bar_scroll_timeout,
                                             path_bar);
...
static void
peony_path_bar_stop_scrolling (PeonyPathBar *path_bar)
{
    if (path_bar->timer)
    {
...
static gboolean
peony_path_bar_slider_button_press (GtkWidget       *widget,
                                   GdkEventButton  *event,
                                   PeonyPathBar *path_bar)
{
...
static gboolean
peony_path_bar_slider_button_release (GtkWidget      *widget,
                                     GdkEventButton *event,
                                     PeonyPathBar     *path_bar)
{
...
static void
peony_path_bar_grab_notify (GtkWidget *widget,
                           gboolean   was_grabbed)
{
    if (!was_grabbed)
    {
...
static void
peony_path_bar_state_changed (GtkWidget    *widget,
                             GtkStateType  previous_state)
{
...
static void
reload_icons (PeonyPathBar *path_bar)
{
    GList *list;
    ...
    for (list = path_bar->button_list; list; list = list->next)
    {
        ...
        {
            peony_path_bar_update_button_appearance (button_data);
        }
...
static void
change_icon_theme (PeonyPathBar *path_bar)
{
    path_bar->icon_size = PEONY_PATH_BAR_ICON_SIZE;
...
static void
peony_path_bar_check_icon_theme (PeonyPathBar *path_bar)
{
    GtkSettings *settings;
...
void
peony_path_bar_clear_buttons (PeonyPathBar *path_bar)
{
                   ...
                   gpointer   data)
{
    ...
    path_bar = PEONY_PATH_BAR (gtk_widget_get_parent (button));
...
get_custom_user_icon_info (ButtonData *button_data)
{
    ...
    {
        custom_icon_uri = peony_file_get_custom_icon (button_data->file);
...
get_type_icon_info (ButtonData *button_data)
{
    ...
    {
    case ROOT_BUTTON:
        return peony_icon_info_lookup_from_name (PEONY_ICON_FILESYSTEM,
                                                PEONY_PATH_BAR_ICON_SIZE);

    case HOME_BUTTON:
        return peony_icon_info_lookup_from_name (PEONY_ICON_HOME,
                                                PEONY_PATH_BAR_ICON_SIZE);

    case DESKTOP_BUTTON:
        return peony_icon_info_lookup_from_name (PEONY_ICON_DESKTOP,
                                                PEONY_PATH_BAR_ICON_SIZE);

    case NORMAL_BUTTON:
        if (button_data->is_base_dir)
        {
            return peony_file_get_icon (button_data->file,
                                       PEONY_PATH_BAR_ICON_SIZE,
                                       PEONY_FILE_ICON_FLAGS_NONE);
        }
...
button_data_free (ButtonData *button_data)
{
    ...
    if (button_data->file != NULL)
    {
        ...
        peony_file_monitor_remove (button_data->file, button_data);
        peony_file_unref (button_data->file);
    }
...
static void
peony_path_bar_update_button_appearance (ButtonData *button_data)
{
    PeonyIconInfo *icon_info;
    GdkPixbuf *pixbuf;
    const gchar *dir_name = get_dir_name (button_data);
...
static void
peony_path_bar_update_button_state (ButtonData *button_data,
                                   gboolean    current_dir)
{
    if (button_data->label != NULL)
    {
    ...
    for (l = mounts; l != NULL; l = l->next)
    {
        ...
        if (g_file_equal (location, root))
        {
            ...
            if (button_data)
            {
                ...
                info = peony_icon_info_lookup (icon, PEONY_PATH_BAR_ICON_SIZE);
                g_object_unref (icon);
                button_data->custom_icon = peony_icon_info_get_pixbuf_at_size (info, PEONY_PATH_BAR_ICON_SIZE);
                g_object_unref (info);
                         ...
                         gpointer            user_data)
{
    ...
    if (info == PEONY_ICON_DND_UKUI_ICON_LIST)
    {
    ...
    else if (info == PEONY_ICON_DND_URI_LIST)
    {
...
setup_button_drag_source (ButtonData *button_data)
{
    ...
    gtk_target_list_add_uri_targets (target_list, PEONY_ICON_DND_URI_LIST);
...
static void
button_data_file_changed (PeonyFile *file,
                          ButtonData *button_data)
{
    ...
    path_bar = (PeonyPathBar *) gtk_widget_get_ancestor (button_data->button,
               PEONY_TYPE_PATH_BAR);
    if (path_bar == NULL)
    {
        return;
    ...
    if (!g_file_equal (button_data->path, location))
    {
        ...
        else
        {
            ...
            else
            {
                 ...
                 */
                current_location = peony_file_get_location (current_button_data->file);
            }

            peony_path_bar_update_path (path_bar, location, FALSE);
            peony_path_bar_set_path (path_bar, current_location);
            g_object_unref (location);
            g_object_unref (current_location);
            return;
        }
    }
    else if (peony_file_is_gone (file))
    {
        gint idx, position;
...
static ButtonData *
make_directory_button (PeonyPathBar  *path_bar,
                       PeonyFile     *file,
                       gboolean          current_dir,
                       gboolean          base_dir,
                       gboolean          file_is_hidden)
{
    ...
    path = peony_file_get_location (file);
    ...
    */
    ...
    {
        button_data->dir_name = peony_file_get_display_name (file);
    }
    if (button_data->file == NULL)
    {
        button_data->file = peony_file_ref (file);
        peony_file_monitor_add (button_data->file, button_data,
    ...
    peony_path_bar_update_button_state (button_data, current_dir);
    ...
    peony_drag_slot_proxy_init (button_data->button,
...
static gboolean
peony_path_bar_check_parent_path (PeonyPathBar *path_bar,
                                 GFile *location,
                                 ButtonData **current_button_data)
{
    GList *list;
    ...
    if (current_path)
    {
        ...
        for (list = path_bar->button_list; list; list = list->next)
        {
            ...
            peony_path_bar_update_button_state (BUTTON_DATA (list->data),
...
static gboolean
peony_path_bar_update_path (PeonyPathBar *path_bar,
                           GFile *file_path,
                           gboolean emit_signal)
{
    PeonyFile *file, *parent_file;
    gboolean first_directory, last_directory;
    gboolean result;
    GList *new_buttons, *l, *fake_root;
    ButtonData *button_data, *current_button_data;

    g_return_val_if_fail (PEONY_IS_PATH_BAR (path_bar), FALSE);
    ...
    peony_path_bar_clear_buttons (path_bar);
...
gboolean
peony_path_bar_set_path (PeonyPathBar *path_bar, GFile *file_path)
{
    ButtonData *button_data;
    ...
    if (peony_path_bar_check_parent_path (path_bar, file_path, &button_data))
    {
        if (path_bar->current_path != NULL)
        {
            g_object_unref (path_bar->current_path);
    ...
    return peony_path_bar_update_path (path_bar, file_path, TRUE);
}

GFile *
peony_path_bar_get_path_for_button (PeonyPathBar *path_bar,
                                   GtkWidget       *button)
{
    GList *list;

    g_return_val_if_fail (PEONY_IS_PATH_BAR (path_bar), NULL);
 ...
 **/
void
peony_path_bar_down (PeonyPathBar *path_bar)
{
    GList *l;
...
GtkWidget *
peony_path_bar_get_button_from_button_list_entry (gpointer entry)
{
