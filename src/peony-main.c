...
void peony_main_event_loop_quit (gboolean explicit)
{
    if (explicit)
    {
        ...
        {
            /* If this is reached, peony must run in "daemon" mode
        ...
        /* TODO: With the old session we needed to set restart
           style to UKUI_RESTART_IF_RUNNING here, but i don't think we need
           that now since ukui-session doesn't restart apps except on startup. */
    }
...
main (int argc, char *argv[])
{
    ...
    /* If in autostart mode (aka started by ukui-session), we need to ensure
     * peony starts with the correct options.
     */
    if (autostart_mode)
    {
    ...
    else if (running_as_root () || !running_in_ukui ())
    {
    ...
    if (perform_self_check && remaining != NULL)
    {
        ...
        fprintf (stderr, _("peony: %s cannot be used with URIs.\n"),
                 "--check");
        return EXIT_FAILURE;
    }
    if (perform_self_check && kill_shell)
    {
        fprintf (stderr, _("peony: --check cannot be used with other options.\n"));
        return EXIT_FAILURE;
    }
    ...
    {
        fprintf (stderr, _("peony: --geometry cannot be used with more than one URI.\n"));
        return EXIT_FAILURE;
    }
     ...
     * on other desktops or when running peony as root. Otherwise, we read the value
     * from the configuration.
     */
    if (running_in_ukui () && !running_as_root())
    {
        exit_with_last_window = g_settings_get_boolean (peony_preferences, PEONY_PREFERENCES_EXIT_WITH_LAST_WINDOW);
    }
    ...
    else
    {
        ...
        if (remaining != NULL)
        {
                    ...
                    if (uri)
                    {
                        ...
                        else
                        {
                            ...
                            if (fileinfo)
                            {
                                ...
                                if (appinfo)
                                {
                                    if (g_strcmp0 (g_app_info_get_executable (appinfo), "peony") != 0)
                                    {
                                        g_app_info_launch_default_for_uri (uri, NULL, NULL);
                                    }
                                    else
                                    {
                                        fprintf (stderr, _("peony: set erroneously as default application for '%s' content type.\n"),
            ...
            if (uris_array->len == 0)
            {
                /* Peony is being used only to open files (not directories), so closing */
