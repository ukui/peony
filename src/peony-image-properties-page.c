...
{
    GObject parent;
} PeonyImagePropertiesPageProvider;

typedef struct
{
    GObjectClass parent;
} PeonyImagePropertiesPageProviderClass;
...
static void
peony_image_properties_page_finalize (GObject *object)
{
    PeonyImagePropertiesPage *page;

    page = PEONY_IMAGE_PROPERTIES_PAGE (object);
    ...
    G_OBJECT_CLASS (peony_image_properties_page_parent_class)->finalize (object);
}
                     ...
                     gpointer      data)
{
    ...
    page = PEONY_IMAGE_PROPERTIES_PAGE (data);
...
static gboolean
append_tag_value_pair (PeonyImagePropertiesPage *page,
                       ExifData *data,
                       ExifTag   tag,
                       char     *description)
{
...
static void
append_exifdata_string (ExifData *exifdata, PeonyImagePropertiesPage *page)
{
...
#ifdef HAVE_EXEMPI
static void
append_xmp_value_pair (PeonyImagePropertiesPage *page,
                       XmpPtr      xmp,
                       const char *ns,
                       const char *propname,
                       char       *descr)
...
static void
append_xmpdata_string (XmpPtr xmp, PeonyImagePropertiesPage *page)
{
    if (xmp != NULL)
    {
...
static void
load_finished (PeonyImagePropertiesPage *page)
{
                        ...
                        gpointer         callback_data)
{
    ...
    page = PEONY_IMAGE_PROPERTIES_PAGE (callback_data);
...
static void
load_location (PeonyImagePropertiesPage *page,
               const char                  *location)
{
    GFile *file;

    g_assert (PEONY_IS_IMAGE_PROPERTIES_PAGE (page));
...
#ifdef HAVE_EXEMPI
    {
        /* Current Exempi does not support setting custom IO to be able to use Ukui-vfs */
...
static void
peony_image_properties_page_class_init (PeonyImagePropertiesPageClass *class)
{
    GObjectClass *object_class;

    object_class = G_OBJECT_CLASS (class);

    object_class->finalize = peony_image_properties_page_finalize;

    g_type_class_add_private (object_class, sizeof(PeonyImagePropertiesPageDetails));
}

static void
peony_image_properties_page_init (PeonyImagePropertiesPage *page)
{
                    ...
                    GList *files)
{
    ...
    if (!
            (peony_file_info_is_mime_type (file, "image/x-bmp") ||
             peony_file_info_is_mime_type (file, "image/x-ico") ||
             peony_file_info_is_mime_type (file, "image/jpeg") ||
             peony_file_info_is_mime_type (file, "image/gif") ||
             peony_file_info_is_mime_type (file, "image/png") ||
             peony_file_info_is_mime_type (file, "image/pnm") ||
             peony_file_info_is_mime_type (file, "image/ras") ||
             peony_file_info_is_mime_type (file, "image/tga") ||
             peony_file_info_is_mime_type (file, "image/tiff") ||
             peony_file_info_is_mime_type (file, "image/wbmp") ||
             peony_file_info_is_mime_type (file, "image/x-xbitmap") ||
             peony_file_info_is_mime_type (file, "image/x-xpixmap")))
    {
        return NULL;
    }
    ...
    page = g_object_new (peony_image_properties_page_get_type (), NULL);
    load_location (page, uri);

    g_free (uri);

    real_page = peony_property_page_new
                ("PeonyImagePropertiesPage::property_page",
...
static void
peony_image_properties_page_provider_init (PeonyImagePropertiesPageProvider *sidebar)
{
}

static void
peony_image_properties_page_provider_class_init (PeonyImagePropertiesPageProviderClass *class)
{
}

void
peony_image_properties_page_register (void)
{
    peony_module_add_type (peony_image_properties_page_provider_get_type ());
}
