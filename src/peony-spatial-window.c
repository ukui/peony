...
#include "peony-location-dialog.h"
...
G_DEFINE_TYPE(PeonySpatialWindow, peony_spatial_window, PEONY_TYPE_WINDOW)
#define parent_class peony_spatial_window_parent_class

static void peony_spatial_window_save_geometry (PeonySpatialWindow *window,
						   PeonyFile *viewed_file);

static gboolean
save_window_geometry_timeout (gpointer callback_data)
{
    PeonySpatialWindow *window;
    PeonyWindowSlot *slot;

    window = PEONY_SPATIAL_WINDOW (callback_data);
    slot = peony_window_get_active_slot (PEONY_WINDOW (window));

    if (slot != NULL)
    {
        peony_spatial_window_save_geometry (window, slot->viewed_file);
    }
...
static gboolean
peony_spatial_window_configure_event (GtkWidget *widget,
                                     GdkEventConfigure *event)
{
    PeonySpatialWindow *window;

    window = PEONY_SPATIAL_WINDOW (widget);

    GTK_WIDGET_CLASS (peony_spatial_window_parent_class)->configure_event (widget, event);
...
static void
peony_spatial_window_unrealize (GtkWidget *widget)
{
    PeonySpatialWindow *window;
    PeonyWindowSlot *slot;
    ...
    GTK_WIDGET_CLASS (peony_spatial_window_parent_class)->unrealize (widget);
...
static gboolean
peony_spatial_window_state_event (GtkWidget *widget,
                                 GdkEventWindowState *event)
{
    ...
    if (!PEONY_IS_DESKTOP_WINDOW (widget))
    {

        if (event->changed_mask & GDK_WINDOW_STATE_MAXIMIZED &&
                viewed_file != NULL)
        {
            peony_file_set_boolean_metadata (viewed_file,
    ...
    if (GTK_WIDGET_CLASS (peony_spatial_window_parent_class)->window_state_event != NULL)
    {
        return GTK_WIDGET_CLASS (peony_spatial_window_parent_class)->window_state_event (widget, event);
    }
...
static void
peony_spatial_window_finalize (GObject *object)
{
    PeonySpatialWindow *window;

    window = PEONY_SPATIAL_WINDOW (object);

    g_free (window->details->last_geometry);

    G_OBJECT_CLASS (peony_spatial_window_parent_class)->finalize (object);
}

static void
peony_spatial_window_save_geometry (PeonySpatialWindow *window,
        		   PeonyFile *viewed_file)
{
    ...
    if (gtk_widget_get_window (GTK_WIDGET (window)) &&
        ...
        peony_file_set_metadata (viewed_file,
...
static void
peony_spatial_window_save_scroll_position (PeonySpatialWindow *window,
					  PeonyWindowSlot *slot)
{
    char *scroll_string;
    ...
    scroll_string = peony_view_get_first_visible_file (slot->content_view);
    peony_file_set_metadata (slot->viewed_file,
...
static void
peony_spatial_window_save_show_hidden_files_mode (PeonySpatialWindow *window,
						 PeonyFile *viewed_file)
{
    ...
    mode = PEONY_WINDOW (window)->details->show_hidden_files_mode;
...
static void
peony_spatial_window_show (GtkWidget *widget)
{
    PeonyWindow *window;
    PeonyWindowSlot *slot;
    ...
    GTK_WIDGET_CLASS (peony_spatial_window_parent_class)->show (widget);

    if (slot != NULL && slot->query_editor != NULL)
    {
        peony_query_editor_grab_focus (PEONY_QUERY_EDITOR (slot->query_editor));
    }
}
...
action_close_parent_folders_callback (GtkAction *action,
                                      gpointer user_data)
{
    peony_application_close_parent_windows (PEONY_SPATIAL_WINDOW (user_data));
}
...
action_close_all_folders_callback (GtkAction *action,
                                   gpointer user_data)
{
    peony_application_close_all_spatial_windows ();
}

static void
real_prompt_for_location (PeonyWindow *window,
                          const char     *initial)
{
    GtkWidget *dialog;

    dialog = peony_location_dialog_new (window);
    if (initial != NULL)
    {
        peony_location_dialog_set_location (PEONY_LOCATION_DIALOG (dialog),
                                           initial);
    }
...
static void
sync_window_title (PeonyWindow *window)
{
    PeonyWindowSlot *slot;
                   ...
                   guint *min_width, guint *min_height)
{
    if (min_width)
    {
        *min_width = PEONY_SPATIAL_WINDOW_MIN_WIDTH;
    }
    if (min_height)
    {
        *min_height = PEONY_SPATIAL_WINDOW_MIN_HEIGHT;
    }
}
                       ...
                       guint *default_width, guint *default_height)
{
    if (default_width)
    {
        *default_width = PEONY_SPATIAL_WINDOW_DEFAULT_WIDTH;
    }
    if (default_height)
    {
        *default_height = PEONY_SPATIAL_WINDOW_DEFAULT_HEIGHT;
    }
}
...
static void
real_set_allow_up (PeonyWindow *window, gboolean allow)
{
    PeonySpatialWindow *spatial;
    GtkAction *action;

    spatial = PEONY_SPATIAL_WINDOW (window);

    action = gtk_action_group_get_action (spatial->details->spatial_action_group,
                                          SPATIAL_ACTION_CLOSE_PARENT_FOLDERS);
    gtk_action_set_sensitive (action, allow);

    PEONY_WINDOW_CLASS (peony_spatial_window_parent_class)->set_allow_up (window, allow);
}
                ...
                PeonyWindowOpenSlotFlags flags)
{
    ...
    g_assert (peony_window_get_active_slot (pane->window) == NULL);

    slots = peony_window_get_slots (pane->window);
    g_assert (slots == NULL);
    g_list_free (slots);

    slot = g_object_new (PEONY_TYPE_WINDOW_SLOT, NULL);
    slot->pane = pane;
    gtk_container_add (GTK_CONTAINER (PEONY_SPATIAL_WINDOW (pane->window)->details->content_box),
                       slot->content_box);
...
static void
save_spatial_data (PeonySpatialWindow *window,
		   PeonyWindowSlot *slot)
{
    peony_spatial_window_save_geometry (window, slot->viewed_file);
    peony_spatial_window_save_scroll_position (window, slot);
    peony_spatial_window_save_show_hidden_files_mode (window, slot->viewed_file);
}
                 ...
                 PeonyWindowSlot *slot)
{
    ...
    window = PEONY_SPATIAL_WINDOW (pane->window);
    ...
    PEONY_WINDOW_CLASS (peony_spatial_window_parent_class)->close_slot (pane, slot);
}
...
{
    PeonyWindowSlot *slot;
    PeonySpatialWindow *self;

    self = PEONY_SPATIAL_WINDOW (window);
    ...
    if (PEONY_WINDOW_CLASS (peony_spatial_window_parent_class)->close != NULL) {
        PEONY_WINDOW_CLASS (peony_spatial_window_parent_class)->close (window);
    }
}
                                       ...
                                       PeonyWindow *window)
{
	...
	slot = peony_window_get_active_slot (window);
	current = peony_window_slot_get_location (slot);
    ...
    if (!g_file_equal (current, dest))
    {
        ...
        peony_window_slot_open_location_with_selection
...
static void
got_file_info_for_location_menu_callback (PeonyFile *file,
        gpointer callback_data)
{
    ...
    pixbuf = peony_file_get_icon_pixbuf (file,
...
static int
get_dnd_icon_size (PeonySpatialWindow *window)
{
	PeonyWindowSlot *active_slot;
    PeonyView *view;
    PeonyZoomLevel zoom_level;

	active_slot = peony_window_get_active_slot (PEONY_WINDOW (window));
	view = active_slot->content_view;

    if (view == NULL)
    {
        return PEONY_ICON_SIZE_STANDARD;
    }
    else
    {
        zoom_level = peony_view_get_zoom_level (view);
        return peony_get_icon_size_for_zoom_level (zoom_level);
    }
}
                                     ...
                                     PeonySpatialWindow *window)
{
    ...
    pixbuf = peony_file_get_icon_pixbuf (slot->viewed_file,
                                        get_dnd_icon_size (window),
                                        FALSE,
                                        PEONY_FILE_ICON_FLAGS_IGNORE_VISITING | PEONY_FILE_ICON_FLAGS_FOR_DRAG_ACCEPT);
...
/* build UKUI icon list, which only contains the window's URI.
 * If we just used URIs, moving the folder to trash
 * wouldn't work */
static void
get_data_binder (PeonyDragEachSelectedItemDataGet iteratee,
                 ...
                 gpointer                            data)
{
    ...
    g_assert (PEONY_IS_SPATIAL_WINDOW (iterator_context));
    window = PEONY_SPATIAL_WINDOW (iterator_context);

    slot = PEONY_WINDOW (window)->details->active_pane->active_slot;

    location = peony_window_slot_get_location_uri (slot);
...
void
peony_spatial_window_set_location_button  (PeonySpatialWindow *window,
        GFile                 *location)
{
    if (location != NULL)
    {
                              ...
                              gpointer user_data)
{
    ...
    if (!PEONY_IS_DESKTOP_WINDOW (window))   /* don't bookmark x-peony-desktop:/// */
    {
        peony_window_add_bookmark_for_current_location (window);
    }
}
...
static void
peony_spatial_window_init (PeonySpatialWindow *window)
{
    GtkWidget *arrow;
    GtkWidget *hbox, *vbox;
    ...
    gtk_box_pack_start (GTK_BOX (PEONY_WINDOW (window)->details->statusbar),
                        window->details->location_button,
                        FALSE, TRUE, 0);

    gtk_box_reorder_child (GTK_BOX (PEONY_WINDOW (window)->details->statusbar),
    ...
    ui = peony_ui_string_get ("peony-spatial-window-ui.xml");
...
static void
peony_spatial_window_class_init (PeonySpatialWindowClass *klass)
{
    GtkBindingSet *binding_set;
	PeonyWindowClass *nclass = PEONY_WINDOW_CLASS (klass);
	GtkWidgetClass *wclass = GTK_WIDGET_CLASS (klass);

	nclass->window_type = PEONY_WINDOW_SPATIAL;
	...
	wclass->show = peony_spatial_window_show;
	wclass->configure_event = peony_spatial_window_configure_event;
	wclass->unrealize = peony_spatial_window_unrealize;
	wclass->window_state_event = peony_spatial_window_state_event;

	G_OBJECT_CLASS (klass)->finalize = peony_spatial_window_finalize;
	...
	g_type_class_add_private (klass, sizeof(PeonySpatialWindowDetails));
}
