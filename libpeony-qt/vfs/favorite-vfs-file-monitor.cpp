...
{
//    g_type_class_add_private (self, sizeof (FavoriteVFSFileMonitorPrivate));
...
static void vfs_favorite_file_monitor_start (FavoriteVFSFileMonitor* monitor, const gchar* filename, gboolean isDir, GFileMonitorFlags flags, GMainContext* ctx)
{
    ...
    return;
    VFS_FAVORITE_FILE_MONITOR_GET_CLASS (monitor)->start (monitor, source->dirname, source->basename, source->filename, source);
}
