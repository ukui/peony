...
static void vfs_favorites_file_enumerator_init (FavoritesVFSFileEnumerator* self)
{
    ...
    self->priv->enumerate_queue->enqueue("favorite:///recent");
    self->priv->enumerate_queue->enqueue("favorite:///trash");
    self->priv->enumerate_queue->enqueue(QString("favorite://%1").arg(QStandardPaths::writableLocation(QStandardPaths::DesktopLocation)));
