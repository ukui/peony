...
GFileInfo* vfs_favorites_file_query_info(GFile *file, const char *attributes, GFileQueryInfoFlags flags, GCancellable *cancellable, GError **error)
{
    ...
    if ("favorite:///computer" == url.toString() || "favorite:///trash" == url.toString() || "favorite:///recent" == url.toString()) {
        trueUri = QString("%1:///").arg(url.path().split("/").last()).toUtf8().constData();
    } else if ("favorite:///" != url.toString()) {
        trueUri = QString("file://%1").arg(url.path()).toUtf8().constData();
    }
