...
struct _PeonyPropertyPageDetails {
    char *name;
    GtkWidget *label;
    GtkWidget *page;
};
...
/**
 * SECTION:peony-property-page
 * @title: PeonyPropertyPage
 * @short_description: Property page descriptor object
 * @include: libpeony-extension/peony-property-page.h
 *
 * #PeonyPropertyPage is an object that describes a page in the file
 * properties dialog. Extensions can provide #PeonyPropertyPage objects
 * by registering a #PeonyPropertyPageProvider and returning them from
 * peony_property_page_provider_get_pages(), which will be called by the
 ...
 *
 * Returns: a newly created #PeonyPropertyPage
 */
PeonyPropertyPage *
                        ...
                        GtkWidget *page_widget)
{
    ...
    page = g_object_new (PEONY_TYPE_PROPERTY_PAGE,
                         "name", name,
                         "label", label,
                         "page", page_widget,
                         NULL);
...
static void
peony_property_page_get_property (GObject *object,
                                 guint param_id,
                                 GValue *value,
                                 GParamSpec *pspec)
{
    ...
    page = PEONY_PROPERTY_PAGE (object);
...
static void
peony_property_page_set_property (GObject *object,
                                 guint param_id,
                                 const GValue *value,
                                 GParamSpec *pspec)
{
...
static void
peony_property_page_dispose (GObject *object)
{
    PeonyPropertyPage *page;
...
static void
peony_property_page_finalize (GObject *object)
{
    PeonyPropertyPage *page;
...
static void
peony_property_page_instance_init (PeonyPropertyPage *page)
{
    page->details = g_new0 (PeonyPropertyPageDetails, 1);
}

static void
peony_property_page_class_init (PeonyPropertyPageClass *class)
{
    parent_class = g_type_class_peek_parent (class);

    G_OBJECT_CLASS (class)->finalize = peony_property_page_finalize;
    G_OBJECT_CLASS (class)->dispose = peony_property_page_dispose;
    G_OBJECT_CLASS (class)->get_property = peony_property_page_get_property;
    G_OBJECT_CLASS (class)->set_property = peony_property_page_set_property;
...
GType
peony_property_page_get_type (void)
{
    static GType type = 0;

    if (!type) {
        const GTypeInfo info = {
            sizeof (PeonyPropertyPageClass),
            NULL,
            NULL,
            (GClassInitFunc)peony_property_page_class_init,
            NULL,
            NULL,
            sizeof (PeonyPropertyPage),
            0,
            (GInstanceInitFunc)peony_property_page_instance_init
        };
