/*
 *  peony-menu-item.c - Menu items exported by PeonyMenuProvider
 *                         objects.
 *
 ...
 * peony_menu_provider_get_file_items(), or
 * peony_menu_provider_get_background_items(), which will be called by the
 ...
 *
 * Returns: a newly create #PeonyMenuItem
 */
PeonyMenuItem *
                    ...
                    const char *icon)
{
    ...
    item = g_object_new (PEONY_TYPE_MENU_ITEM,
                         "name", name,
                         "label", label,
                         "tip", tip,
                         "icon", icon,
 ...
 * @item: pointer to a #PeonyMenuItem
 *
 ...
 * @item: pointer to a #PeonyMenuItem
 * @menu: pointer to a #PeonyMenu to attach to the button
 *
 * Attachs a menu to the given #PeonyMenuItem.
 */
void
peony_menu_item_set_submenu (PeonyMenuItem *item, PeonyMenu *menu)
{
    g_object_set (item, "menu", menu, NULL);
}

static void
peony_menu_item_get_property (GObject *object,
                             guint param_id,
                             GValue *value,
                             GParamSpec *pspec)
{
...
static void
peony_menu_item_set_property (GObject *object,
                             guint param_id,
                             const GValue *value,
                             GParamSpec *pspec)
{
...
static void
peony_menu_item_finalize (GObject *object)
{
    PeonyMenuItem *item;
...
static void
peony_menu_item_instance_init (PeonyMenuItem *item)
{
    item->details = g_new0 (PeonyMenuItemDetails, 1);
    item->details->sensitive = TRUE;
    item->details->menu = NULL;
}

static void
peony_menu_item_class_init (PeonyMenuItemClass *class)
{
    parent_class = g_type_class_peek_parent (class);

    G_OBJECT_CLASS (class)->finalize = peony_menu_item_finalize;
    G_OBJECT_CLASS (class)->get_property = peony_menu_item_get_property;
    G_OBJECT_CLASS (class)->set_property = peony_menu_item_set_property;

    signals[ACTIVATE] =
        g_signal_new ("activate",
                      G_TYPE_FROM_CLASS (class),
                      G_SIGNAL_RUN_LAST,
                      G_STRUCT_OFFSET (PeonyMenuItemClass,
                                       activate),
                      NULL, NULL,
...
peony_menu_item_get_type (void)
{
    ...
    if (!type) {
        const GTypeInfo info = {
            sizeof (PeonyMenuItemClass),
            NULL,
            NULL,
            (GClassInitFunc)peony_menu_item_class_init,
            NULL,
            NULL,
            sizeof (PeonyMenuItem),
            0,
            (GInstanceInitFunc)peony_menu_item_instance_init
        };
