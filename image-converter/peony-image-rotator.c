...
typedef struct _PeonyImageRotatorPrivate PeonyImageRotatorPrivate;

struct _PeonyImageRotatorPrivate {
	GList *files;
...
#define PEONY_IMAGE_ROTATOR_GET_PRIVATE(o) (G_TYPE_INSTANCE_GET_PRIVATE ((o), PEONY_TYPE_IMAGE_ROTATOR, PeonyImageRotatorPrivate))

G_DEFINE_TYPE (PeonyImageRotator, peony_image_rotator, G_TYPE_OBJECT)
...
typedef enum {
	...
	SIGNAL_TYPE_EXAMPLE,
	LAST_SIGNAL
} PeonyImageRotatorSignalType;

static void
peony_image_rotator_finalize(GObject *object)
{
	PeonyImageRotator *dialog = PEONY_IMAGE_ROTATOR (object);
	PeonyImageRotatorPrivate *priv = PEONY_IMAGE_ROTATOR_GET_PRIVATE (dialog);
	
	g_free (priv->suffix);
		
	G_OBJECT_CLASS(peony_image_rotator_parent_class)->finalize(object);
}

static void
peony_image_rotator_set_property (GObject      *object,
                        guint         property_id,
                        const GValue *value,
                        GParamSpec   *pspec)
{
...
static void
peony_image_rotator_get_property (GObject      *object,
                        guint         property_id,
                        GValue       *value,
                        GParamSpec   *pspec)
{
	PeonyImageRotator *self = PEONY_IMAGE_ROTATOR (object);
	PeonyImageRotatorPrivate *priv = PEONY_IMAGE_ROTATOR_GET_PRIVATE (self);
...
static void
peony_image_rotator_class_init(PeonyImageRotatorClass *klass)
{
	g_type_class_add_private (klass, sizeof (PeonyImageRotatorPrivate));

	GObjectClass *object_class = G_OBJECT_CLASS(klass);
	GParamSpec *files_param_spec;

	object_class->finalize = peony_image_rotator_finalize;
	object_class->set_property = peony_image_rotator_set_property;
	object_class->get_property = peony_image_rotator_get_property;
...
static void run_op (PeonyImageRotator *rotator);

static GFile *
peony_image_rotator_transform_filename (PeonyImageRotator *rotator, GFile *orig_file)
{
	PeonyImageRotatorPrivate *priv = PEONY_IMAGE_ROTATOR_GET_PRIVATE (rotator);
...
{
	PeonyImageRotator *rotator = PEONY_IMAGE_ROTATOR (data);
	PeonyImageRotatorPrivate *priv = PEONY_IMAGE_ROTATOR_GET_PRIVATE (rotator);
	...
	if (status != 0) {
		...
		GFile *new_location = peony_image_rotator_transform_filename (rotator, orig_location);
...
static void
run_op (PeonyImageRotator *rotator)
{
	PeonyImageRotatorPrivate *priv = PEONY_IMAGE_ROTATOR_GET_PRIVATE (rotator);
...
static void
peony_image_rotator_response_cb (GtkDialog *dialog, gint response_id, gpointer user_data)
{
	PeonyImageRotator *rotator = PEONY_IMAGE_ROTATOR (user_data);
	PeonyImageRotatorPrivate *priv = PEONY_IMAGE_ROTATOR_GET_PRIVATE (rotator);
...
static void
peony_image_rotator_init(PeonyImageRotator *rotator)
{
	PeonyImageRotatorPrivate *priv = PEONY_IMAGE_ROTATOR_GET_PRIVATE (rotator);
	...
	path = g_build_filename (DATADIR, PACKAGE, "peony-image-rotate.ui", NULL);
	...
	g_signal_connect (G_OBJECT (priv->rotate_dialog), "response",
			  (GCallback) peony_image_rotator_response_cb,
			  rotator);
}

PeonyImageRotator *
peony_image_rotator_new (GList *files)
{
	return g_object_new (PEONY_TYPE_IMAGE_ROTATOR, "files", files, NULL);
}

void
peony_image_rotator_show_dialog (PeonyImageRotator *rotator)
{
	PeonyImageRotatorPrivate *priv = PEONY_IMAGE_ROTATOR_GET_PRIVATE (rotator);
