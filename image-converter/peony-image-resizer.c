...
typedef struct _PeonyImageResizerPrivate PeonyImageResizerPrivate;

struct _PeonyImageResizerPrivate {
	GList *files;
...
#define PEONY_IMAGE_RESIZER_GET_PRIVATE(o) (G_TYPE_INSTANCE_GET_PRIVATE ((o), PEONY_TYPE_IMAGE_RESIZER, PeonyImageResizerPrivate))

G_DEFINE_TYPE (PeonyImageResizer, peony_image_resizer, G_TYPE_OBJECT)
...
typedef enum {
	...
	SIGNAL_TYPE_EXAMPLE,
	LAST_SIGNAL
} PeonyImageResizerSignalType;

static void
peony_image_resizer_finalize(GObject *object)
{
	PeonyImageResizer *dialog = PEONY_IMAGE_RESIZER (object);
	PeonyImageResizerPrivate *priv = PEONY_IMAGE_RESIZER_GET_PRIVATE (dialog);
	
	g_free (priv->suffix);
		
	G_OBJECT_CLASS(peony_image_resizer_parent_class)->finalize(object);
}

static void
peony_image_resizer_set_property (GObject      *object,
                        guint         property_id,
                        const GValue *value,
                        GParamSpec   *pspec)
{
...
static void
peony_image_resizer_get_property (GObject      *object,
                        guint         property_id,
                        GValue       *value,
                        GParamSpec   *pspec)
{
	PeonyImageResizer *self = PEONY_IMAGE_RESIZER (object);
	PeonyImageResizerPrivate *priv = PEONY_IMAGE_RESIZER_GET_PRIVATE (self);
...
static void
peony_image_resizer_class_init(PeonyImageResizerClass *klass)
{
	g_type_class_add_private (klass, sizeof (PeonyImageResizerPrivate));

	GObjectClass *object_class = G_OBJECT_CLASS(klass);
	GParamSpec *files_param_spec;

	object_class->finalize = peony_image_resizer_finalize;
	object_class->set_property = peony_image_resizer_set_property;
	object_class->get_property = peony_image_resizer_get_property;
...
static void run_op (PeonyImageResizer *resizer);

static GFile *
peony_image_resizer_transform_filename (PeonyImageResizer *resizer, GFile *orig_file)
{
	PeonyImageResizerPrivate *priv = PEONY_IMAGE_RESIZER_GET_PRIVATE (resizer);
...
{
	PeonyImageResizer *resizer = PEONY_IMAGE_RESIZER (data);
	PeonyImageResizerPrivate *priv = PEONY_IMAGE_RESIZER_GET_PRIVATE (resizer);
	
	gboolean retry = TRUE;
	
	PeonyFileInfo *file = PEONY_FILE_INFO (priv->files->data);
	
	if (status != 0) {
		/* resizing failed */
		char *name = peony_file_info_get_name (file);
		...
		GFile *orig_location = peony_file_info_get_location (file);
		GFile *new_location = peony_image_resizer_transform_filename (resizer, orig_location);
...
static void
run_op (PeonyImageResizer *resizer)
{
	PeonyImageResizerPrivate *priv = PEONY_IMAGE_RESIZER_GET_PRIVATE (resizer);
...
static void
peony_image_resizer_response_cb (GtkDialog *dialog, gint response_id, gpointer user_data)
{
	PeonyImageResizer *resizer = PEONY_IMAGE_RESIZER (user_data);
	PeonyImageResizerPrivate *priv = PEONY_IMAGE_RESIZER_GET_PRIVATE (resizer);
...
static void
peony_image_resizer_init(PeonyImageResizer *resizer)
{
	PeonyImageResizerPrivate *priv = PEONY_IMAGE_RESIZER_GET_PRIVATE (resizer);
	...
	path = g_build_filename (DATADIR, PACKAGE, "peony-image-resize.ui", NULL);
	...
	g_signal_connect (G_OBJECT (priv->resize_dialog), "response",
			  (GCallback) peony_image_resizer_response_cb,
			  resizer);
}

PeonyImageResizer *
peony_image_resizer_new (GList *files)
{
	return g_object_new (PEONY_TYPE_IMAGE_RESIZER, "files", files, NULL);
}

void
peony_image_resizer_show_dialog (PeonyImageResizer *resizer)
{
	PeonyImageResizerPrivate *priv = PEONY_IMAGE_RESIZER_GET_PRIVATE (resizer);
